...
| `p05_neural_road_generation` | `Active POC / 成果模块` | `modules/p05_neural_road_generation` | 当前执行目标 A 联合神经 RoadGraph 决策 POC：冻结 T01 业务骨架，以 T07 为前置证据、T10 为编排，模型依次完成语义路口锚定、普通 Segment 完整 Road/Node 方案、条件化 `ADVANCE_RIGHT`、RealityChangeClue/作用域与冲突组组合；旧 T03–T06 终态只作标签和验收。第一版 Movement 冻结，不替代 T07，不修改 T01–T12 正式实现或接口，未接正式主链或生产。P13-P0 `SELECTION_NO_GO` 与更早模型只作历史实验。 |
