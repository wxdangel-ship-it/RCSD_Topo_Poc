...
- 2026-05-29 更新：用户修正 Step2 / Step3 口径：`kind_2 = 64 / 128` 在 Step2 写 `no / NULL`；`kind_2 = 2048` 不满足全组同一 `RCSDIntersection` 时写 `no / NULL`。其中 `2048` 口径已被 2026-06-08 T 型路口取消关系映射修正覆盖。
- 2026-06-04 更新：用户修正 Step2 一面多 SWSD 语义路口口径：若同一个 `RCSDIntersection` 面包含多个 SWSD 语义路口，代表 node `kind_2 in {4, 8, 16, 64, 128, 2048}` 均记录为 `fail2`。其中 `2048` 覆盖口径已被 2026-06-08 T 型路口取消关系映射修正覆盖。
- 2026-06-04 更新：用户重新梳理 Step3 口径，Step3 处理范围曾恢复为代表 node `kind_2 in {4, 8, 16, 2048}`；该口径已被 2026-06-08 T 型路口取消关系映射修正覆盖。
- 2026-06-08 更新：用户曾修正 Step3 T05 relation 补充候选口径为排除 `2048`；该口径已被同日 T 型路口取消关系映射修正进一步覆盖为 Step3 仅处理 `{4, 8, 16}`。
- 2026-06-08 更新：用户要求整体取消 T07 中 T 型路口 `kind_2 = 2048` 的 SWSD-RCSD 关系映射，T07 统一写 `is_anchor = no / anchor_reason = NULL`，不参与或接收 `fail2`，后续由 T03 虚拟锚定。
...
**Independent Test**: 构造多组语义路口与 `RCSDIntersection` 面，覆盖单面命中、未命中、多面命中、同一面多组命中、`kind_2 = 64 / 128` 专项基础写 `no`、T 型路口 `kind_2 = 2048` 统一写 `no / NULL` 且不建立关系，以及同一面多组命中对 `{4,8,16,64,128}` 的 `fail2` 覆盖。
...
- **FR-011**: T07 Step2 MUST set representative `kind_2 = 2048` to `is_anchor = no / anchor_reason = NULL`, MUST NOT build a T07 SWSD-RCSD relation for it, and MUST NOT include it in `fail1 / fail2`; T-junction virtual anchoring is handled by T03.
...
- **FR-017**: T07 Step3 MUST process representative `kind_2 in {4, 8, 16}`.
- **FR-017a**: T07 Step3 MUST exclude representative `kind_2 = 2048` from both Step2 surface 1V1 relation building and T05 relation backfill.
...
- **SC-002**: A synthetic Step2 test suite covers `yes / no / fail1 / fail2 / NULL`, including `kind_2 = 64 / 128` base `no / NULL`, `kind_2 = 2048` unified `no / NULL` with no T07 relation, and one shared `RCSDIntersection` overriding `kind_2 in {4, 8, 16, 64, 128}` to `fail2` while leaving `2048` out.
...
- **SC-006**: Step3 tests cover successful T05 relation, missing relation, failed relation, RCSD `base_id` missing, and `kind_2 = 64 / 128 / 2048` exclusion from T05 backfill; `2048` is also excluded from Step2 surface relation building.
