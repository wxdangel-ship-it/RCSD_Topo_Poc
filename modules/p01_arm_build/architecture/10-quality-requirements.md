...
- P01-Final synthetic 覆盖规则级 full_allowed 投影到全部目标退出 Road、主干 / 平行支路部分覆盖报错、advance-left 与 uturn 特例、混源 Arm 规则源选择、RCSD 目标 Arm 缺失 fallback、精确源 Road 匹配缺失仍可规则生成、Source + CRS-normalized rounded exact mapping 审计、缺失 / 多匹配 issue 与 duplicate 防护。
