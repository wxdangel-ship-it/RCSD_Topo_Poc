...
| `t10_run_e2e_cases.sh` | `scripts/t10_run_e2e_cases.sh` | repo 级 | T10 Case 级端到端执行入口；从已生成或已解包的 T10 Case package 读取局部外部输入切片，串联 `T01 -> T07 -> T03 -> T04 -> T05 -> T06 -> T09` 的既有脚本或模块 callable，并输出阶段审计与 T06 数据漏斗；可通过 `T10_FEEDBACK_ITERATIONS` 执行 T06 upstream feedback 的 side-group endpoint / pair-anchor endpoint cluster 回灌迭代并启用 replaced Segment 不回退 guard | `active` | 否 |
