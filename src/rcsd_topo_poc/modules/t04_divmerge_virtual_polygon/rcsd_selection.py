...
def _published_rcsd_subset(
    *,
    ...
    if selected_aggregated.consistency_level == "A":
        ...
        keep_missing_arms_aggregate_component = bool(
            required_unit is not None
            and required_unit.unit_id
            and selected_aggregated.decision_reason == "role_mapping_partial_missing_arms"
        )
        if (
            required_unit is not None
            and required_unit.unit_id
            and not keep_partial_aggregate_component
            and not keep_missing_arms_aggregate_component
        ):
        ...
        elif keep_missing_arms_aggregate_component:
            published_member_unit_ids = (required_unit.unit_id,)
            publish_mode = "aggregated_partial_missing_arms_component"
        if selected_aggregated.required_node_id is not None and first_hit_road_ids:
            for road_id in first_hit_road_ids:
