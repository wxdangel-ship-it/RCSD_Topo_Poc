...
| T12 | 对原始 1V1 F-RCSD 执行 audit-only 质量检查，验证 SWSD 通行性等价假设，以 raw endpoint topology 为主、portal-constrained semantic carrier 为误报排除门禁，结合标准路口 portal 和锚点可信度自动发布确认与排除层，人工复核仅作可选 QA 覆盖，不执行修复。 |
...
- F-RCSD QA：T06 Step3 结果继续由 T06 正式审计；外部 1V1 匹配生成的 F-RCSD 由 T12 检查 SWSD 等价可达性、road-node integrity、canonical 候选图、raw verdict 图、portal-constrained semantic carrier、标准路口 portal 和自动高置信发布；人工复核仅作可选 QA 覆盖。
