...
| `t08_tool11_patch_data_organization.py` | `scripts/t08_tool11_patch_data_organization.py` | repo 级 | T08 Tool11 Patch 级数据整理；参数化扫描全量 Patch，递归复制 SWSD/RCSD、仅复制三个 FRCSD 白名单 GeoJSON，逐文件 SHA-256 校验后发布统一全量根和指定实验 Patch 独立根，默认拒绝覆盖并输出 `_tool11` summary | `active` | 否 |
| `t08_tool11_run_innernet.sh` | `scripts/t08_tool11_run_innernet.sh` | repo 级 | T08 Tool11 内网 WSL 固定场景入口；默认使用已确认的原始根、全量根、实验根与 6 个实验 Patch，自动将 Windows 路径转换为 WSL 路径，调用正式 Tool11 Python 入口并持久记录控制台日志；支持环境变量覆盖路径，只有 `OVERWRITE=1` 才替换已有成果 | `active` | 否 |
