...
| T12 quality hypothesis | SWSD 与原始 1V1 F-RCSD 在方向通行性上应等价：SWSD 必需方向不应缺失，单向 Segment 也不应无证据地增加反向载体。1V1/T05 mainNode 锚定只展开选中 `base_id` canonical group 的 raw subNode/alias；其它显式 grouped raw node 保留但不递归扩组，随后在 identity endpoint 图上按 Road Direction 检查实际 carrier。锚定 alias 距离仅审计，非锚定 spatial portal 仍受范围/标准面门禁。该语义用于 raw endpoint topology、portal-constrained semantic carrier、标准路口 portal、反向双端锚点区间、逐 raw RCSD Road Segment 唯一归属、SWSD 反向替代路径和锚点可信度联合质检；semantic carrier 只排除 raw 假断裂，SWSD 等价反向路径、其它 Segment 更强覆盖、归属并列和锚点区间未证明只排除反向误报，完成各自排除门禁且通过高置信锚点规则的记录才可进入正式问题层。T12 另以 T03 rejected 为 Junction candidate、原始 FRCSD 为复核 target，重算 target projection、局部 support ownership、endpoint/component 与替代 carrier；T07 1:N/N:1 稳定失败直接形成独立 Junction Point 问题。Segment 与 Junction 几何、计数和 evidence 分层，任何质量结论都不得直接提升为修复规则。 |
