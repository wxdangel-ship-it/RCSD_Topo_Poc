...
- 当前真实执行入口共 `79` 个。
- 分布概览：
  - repo 级入口文件：`55`（`Makefile` 1 + `scripts/` 53 + `.venv/bin/python -m rcsd_topo_poc` 1）
  - CLI 稳定子命令：`24`
- 维护口径：
...
| `t08_tool4_junction_type_repair.py` | `scripts/t08_tool4_junction_type_repair.py` | repo 级 | T08 Tool4 T 型路口错误修复，校验 `kind_2=2048` T 型语义路口并 copy-on-write 输出完整 Nodes/audit Nodes GPKG 与 summary，不改写输入 Nodes/Roads | `active` | 否 |
| `t08_tool5_complex_junction_preprocess.py` | `scripts/t08_tool5_complex_junction_preprocess.py` | repo 级 | T08 Tool5 复杂路口预处理，构建复杂分歧 / 合流路口，并可基于 `RCSDIntersection` 识别和处理错误 1 对多路口，copy-on-write 输出 `EPSG:3857` Nodes/Roads/audit Nodes GPKG 与 summary | `active` | 否 |
