...
- `no_surface_reference`：防御性异常兜底；表示合法 SWSD section reference 未能从输入 / Step3 / Step4 恢复出来，或上游输入已不满足 T04 case 的基本语义前提。Anchor_2 正常业务 case 不应因为“无主证据 + 无 RCSD 语义路口”落入该场景；该类 case 应优先进入 `no_main_evidence_with_swsd_only`，若存在局部 RCSDRoad fallback 则进入 `no_main_evidence_with_rcsdroad_fallback_and_swsd`。`no_surface_reference` 无 Reference Point、无 section reference，不生成实体路口面，不伪造 fake final polygon，并必须作为异常原因进入审计。
