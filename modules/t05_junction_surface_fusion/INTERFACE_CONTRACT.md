...
若某个 T03/T04 SWSD 语义路口 evidence 明确满足“前置构面成功 / 锚定成功，但 `relation_state = no_related_rcsd` 且不存在可用 RCSDNode 或 RCSDRoad 候选”，同时 Phase 2 审计也确认该 target 无 RCSD 关系，且对应 SWSD node 原始 `has_evd = yes / is_anchor = yes`，`swsdnode_out.gpkg` 中这两个字段必须改写为 `yes_nr`，表示 not RCSD；输入 `nodes.gpkg` 不得原地修改，标记明细写入 `swsdnode_yes_nr_audit.csv/json`。summary 必须输出 yes_nr 候选、匹配与未匹配统计，便于审计 `yes_nr=0` 的原因。T04 fallback road-only 即使 evidence 中出现 `no_related_rcsd`，只要存在 `fallback_rcsdroad_ids / selected_rcsdroad_ids` 并进入 road split，就不得标记 `yes_nr`。
...
- 最终成功关系必须通过 cardinality QC：同一 `target_id` 不得挂接多个 `base_id`，同一 `base_id` 不得被多个 `target_id` 挂接；若发现 `1:N`、`N:1` 或重复 success target，写入 `relation_cardinality_errors.csv/json`，记录 `introduced_by_module / source_modules / source_case_ids / scenes / reasons`，相关错误 relation 不进入 `intersection_match_all.geojson` 主表，并令 `summary.passed = false`。
