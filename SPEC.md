...
- T06 Step1 正式启用 final `nodes.gpkg.kind_2` 的 pair-node 豁免语义：仅当 `pair_nodes` 中节点的 `kind_2 in {1,4096,8192}` 时，该 pair node 不参与 Step1 的 `has_evd / is_anchor` eligibility 判定并视为通过；该语义不适用于 `junc_nodes`，不改变 `pair_nodes / semantic_node_set` 输出，也不跳过 Step2 relation mapping。
...
- 当前已登记正式业务模块：`t01_data_preprocess`、`t02_junction_anchor`、`t03_virtual_junction_anchor`、`t04_divmerge_virtual_polygon`；当前已登记 P01 成果模块：`p01_arm_build`。
...
- T06 Step1 中 final `nodes.gpkg.kind_2 in {1,4096,8192}` 只作为 `pair_nodes` 的 eligibility 豁免字段：命中节点不参与 `has_evd / is_anchor` 判定并视为通过；`junc_nodes` 仍必须按原规则判定，Step2 仍必须校验 T05 relation。
