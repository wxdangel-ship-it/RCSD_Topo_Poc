...
- `t01_data_preprocess` 双向 Segment 构建允许穿越 `kind_2 = 128` 复杂分歧 / 合流路口组合，但必须输出 `kind_2_128_*` 审计字段；该审计不改变 `seed / terminate / hard-stop` 规则，也不扩展 `through_node_ids` 语义；Step2 对复杂穿越热点可启用 trunk search budget，预算耗尽时输出 `trunk_search_budget_exceeded` 并保留预算审计信息
