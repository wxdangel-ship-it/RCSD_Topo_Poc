...
T10 不执行 topology silent fix。空间切片只做窗口相交与裁剪，不修复输入几何；invalid geometry、empty-after-clip、缺失 handoff、目录型 handoff 或后续拓扑不一致必须进入 audit。
...
v1 记录 contract validation 计数、Case package 计数、slot 级 source / selected feature count 与 slice 文件大小。后续接入真实执行后，必须记录模块级输入规模、输出规模和阶段耗时。
