...
| `t12_run_frcsd_quality_audit.py` | `scripts/t12_run_frcsd_quality_audit.py` | repo 级 | T12 原始 1V1 F-RCSD Segment/Junction 质量审计正式 Python 入口；读取 SWSD、原始 1V1 F-RCSD、RCSDIntersection、T05/T06 交叉证据、可选 T03/T07 Junction 来源及 DriveZone/review decisions，输出 Segment LineString 与 Junction Point 的 candidate/confirmed/excluded/evidence/summary 分层结果；新增 Junction 参数均可选，不修改输入或执行修复 | `active` | 否 |
| `t12_rerun_frcsd_junction_quality_innernet.sh` | `scripts/t12_rerun_frcsd_junction_quality_innernet.sh` | repo 级 | T12-only 内网重跑入口；使用环境变量接收既有正式 T01/T03/T05/T06/T07、SWSD、原始 1V1 F-RCSD 和 RCSDIntersection 路径，做前置校验后调用 T12 Python 正式入口，验证 Segment/Junction 必要输出并打印进度、计数和耗时；默认输出到新的 `outputs/_work/t12_innernet_rerun/<RUN_ID>`，拒绝覆盖 | `active` | 否 |
