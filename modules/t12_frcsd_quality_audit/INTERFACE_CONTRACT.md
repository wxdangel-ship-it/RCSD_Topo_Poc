...
- `--t07-step3-run-root`：可选 T07 Step3 根；必须包含正式 `relation_cardinality_errors.csv/json`。`one_target_to_many_base` 与 `many_target_to_one_base` 直接发布，`duplicate_target_rows` 忽略并计数。
- `--processing-crs`：仅在输入 CRS 不一致时显式指定 projected metre CRS。

两个 Junction 参数均为新增可选参数；既有必选参数、默认值和旧调用方式不变。不提供时仍写结构完整的空 Junction 输出，Segment 结果不变。
...
| Junction `issue_type` | `junction_required_topology_missing / junction_reality_or_precision_gap / junction_relation_cardinality_mismatch` | T03 两类高置信根因或 T07 关系基数错误。 |
| Junction `decision_source` | `t03_raw_frcsd_reverification / t07_stable_failure_direct` | T03 由 T12 重验，T07 直接发布。 |
| Junction `decision_rule` | `raw_frcsd_shared_degree1_terminal_collapse_confirmed / raw_frcsd_multi_component_unmatched_support_confirmed / t07_stable_one_target_to_many_base_direct_publish / t07_stable_many_target_to_one_base_direct_publish` 或可审计 exclusion rule | 独立 Junction 决定规则。 |
...
Segment candidates/confirmed GPKG 主几何保持 `LineString`；Junction candidates/confirmed GPKG 主几何只写 `Point`。两者不得合并成同一正式问题层。Segment review CSV 本轮不覆盖 Junction 自动决定。
...
T10 启用 T12 时必须传入当前 T03 run root；若当前运行实际产生正式 T07 Step3 关系基数工件，则同时传入其 stage root。T12 在 T10 中位于 T11 后、T09 前，始终 audit-only；该执行顺序不表示 T12 消费 T11 输出。
...
- Junction candidates/confirmed/exclusions 独立计数守恒，正式主几何为 Point；T03 结果必须来自原始 FRCSD 重验，T07 仅直接消费 1:N/N:1 稳定失败。
