...
6. Step1 对处理范围内语义路口执行全组 node 的 `DriveZone ∪ RCSDIntersection` intersects/touches 判定，只有组内所有 node 均命中该合并 evidence 面才写 `has_evd = yes`。
7. Step2 仅对 `has_evd = yes` 的语义路口执行 `RCSDIntersection` 判定。
8. Step2 先将 `kind_2 = 64 / 128` 写为基础 `no / NULL`；将 `kind_2 = 2048` 统一写为 `no / NULL`，不建立 T07 SWSD-RCSD 关系并交由 T03 虚拟锚定。
...
14. Step3 再选择 `has_evd = yes / is_anchor = no` 且代表 node `kind_2 in {4, 8, 16, 128}` 的 SWSD 语义路口作为 T05 relation 补充候选；只接受 `intersection_match_all.geojson` 中 `status = 0 / base_id != 0`、`base_id` 存在于输入 `RCSDNode.id/mainnodeid` 且未被前段占用的 relation。`kind_2 = 128` 只走该 relation 补充路径，不生成 T07 Step2 surface。
