...
| T12 Junction 候选被误当正式问题 | T03 rejected 可能由跨层、无效几何、constraint split、support 不完整或邻近其它 Segment Road造成，最近距离和 endpoint 全局 degree 不能代表当前 Junction ownership | 只接受正式 eligibility 与完整 rejected 审计链；在原始 FRCSD 重算局部 support ownership、projection、support/global degree、component、Direction 和替代 carrier，仅发布两类完整高置信规则。距离只检索/审计，正式高置信跨层或无效几何保守排除；T07 只直接发布 1:N/N:1，duplicate 忽略；Point 主结果与根因 evidence 分层 |
