...
提前右转 road 分为两类：路口内且按 `direction` 进入当前语义路口的 bit7 road 是 Arm 组成部分，进入 seed / member 并作为非 trunk member 保留；路口前、不进入当前语义路口的 bit7 road 不进入 seed、connector、member 或 trunk，必须生成 `AdvanceRightTurnRelation` 或明确 issue。bit7 relation 候选范围包括 Arm 非特殊 inbound / bidirectional seed 外侧节点相邻的 bit7 road，避免提前右转接在路口前 seed 外侧时被漏掉。连续路口前 bit7 road 链按一条 Arm 级提前右转 relation 输出，链内 road segment 仍保留在 relation 的 `advance_right_turn_road_ids` 中。提前左转 road 可以进入 Arm member，但必须标记并从 trunk 中排除；未进入 Arm member 的外侧 bit8 road 不计为当前 Arm 的提前左转。
