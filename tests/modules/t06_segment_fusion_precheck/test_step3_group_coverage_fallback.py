...
from rcsd_topo_poc.modules.t06_segment_fusion_precheck.step3_group_coverage_fallback import (
    ...
    GROUP_DIRECTIONALITY_FALLBACK_REASON,
    GROUP_DIRECTIONALITY_FALLBACK_RISK,
...
def test_group_path_corridor_local_coverage_source_not_formal_becomes_retained_swsd() -> None:
    segment_id = "A_B"
    units = [
        _Unit(
            segment_id=segment_id,
            swsd_road_ids=["S1"],
            removed_swsd_node_ids=["A", "B"],
    ...
    assert stats["group_path_corridor_source_not_formal_retained_segment_count"] == 1
    assert stats["group_path_corridor_source_not_formal_retained_segments"] == [segment_id]
    assert relation["relation_status"] == "retained_swsd"
    assert relation["relation_reason"] == "path_corridor_source_segment_not_formal_replaceable"
    assert relation["frcsd_road_ids"] == ["S1"]
    assert relation["source_mix"] == "source_2"
    assert not any(row["properties"].get("id") == "R1" for row in frcsd_roads)
    assert any((row["properties"].get("source"), row["properties"].get("id")) == (2, "S1") for row in frcsd_roads)
    assert removed_roads == {}
    assert removed_nodes == {}
...
def test_group_path_corridor_local_coverage_requires_endpoint_relation_maps() -> None:
    ...
    units = [
        _Unit(
            segment_id=segment_id,
            swsd_road_ids=["S1", "S2"],
            removed_swsd_node_ids=["A", "X", "B"],
            risk_flags=["group_path_corridor_replacement", GROUP_SOURCE_NOT_FORMAL_REPLACEABLE_REASON],
        )
    ]
    swsd_segments = [
        feature(
    ...
    assert stats["group_path_corridor_source_not_formal_retained_segment_count"] == 0
    assert stats["group_path_corridor_source_not_formal_relation_risk_pruned_count"] == 1
    assert relation["relation_status"] == "replaced"
    assert relation["source_mix"] == "source_1"
    assert GROUP_SOURCE_NOT_FORMAL_REPLACEABLE_REASON not in relation["risk_flags"]
...
def test_group_path_corridor_dual_direction_gap_retains_swsd_carrier() -> None:
    segment_id = "A_B"
    units = [_Unit(segment_id=segment_id, swsd_road_ids=["S1"], removed_swsd_node_ids=["A", "B"])]
    swsd_segments = [
        feature(
    ...
    frcsd_roads = [
        feature(
            {"id": "R1", "snodeid": "RB", "enodeid": "RA", "source": 1, "direction": 2},
            LineString([(100, 0), (0, 0)]),
        )
    ]
    frcsd_nodes = [feature({"id": "RA", "mainnodeid": "RA", "source": 1}, Point(0, 0)), feature({"id": "RB", "mainnodeid": "RB", "source": 1}, Point(100, 0))]
    relation_rows = [
        feature(
            {
                "swsd_segment_id": segment_id,
    ...
    relation = relation_rows[0]["properties"]
    assert stats["group_path_corridor_coverage_fallback_segment_count"] == 1
    assert stats["group_path_corridor_directionality_fallback_segment_count"] == 1
    assert stats["group_path_corridor_directionality_fallback_segments"] == [segment_id]
    assert relation["relation_status"] == "retained_swsd"
    assert relation["relation_reason"] == GROUP_DIRECTIONALITY_FALLBACK_REASON
    assert relation["frcsd_road_ids"] == ["S1"]
    assert relation["frcsd_road_source_values"] == [2]
    assert relation["retained_detached_swsd_road_ids"] == ["S1"]
    assert relation["removed_swsd_road_ids"] == []
    assert relation["source_mix"] == "source_2"
    assert relation["rcsd_pair_nodes"] == []
    assert relation["swsd_to_frcsd_node_map"] == [
        {"swsd_node_id": "A", "frcsd_node_ids": ["A"], "node_role": "pair_node", "mapping_status": "identity"},
        {"swsd_node_id": "B", "frcsd_node_ids": ["B"], "node_role": "pair_node", "mapping_status": "identity"},
    ]
    assert GROUP_DIRECTIONALITY_FALLBACK_RISK in relation["risk_flags"]
    assert GROUP_DIRECTIONALITY_FALLBACK_RISK in units[0].risk_flags
    assert removed_roads == {}
    assert removed_nodes == {}
