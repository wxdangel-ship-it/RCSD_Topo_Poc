...
- Segment package 的 `spatial_slice` 必须以 T01 `segment.gpkg` 中目标 Segment 几何 bounds 扩展 `radius_m` 形成窗口，并从既有 T10 run root 记录 T06 evidence references。
...
- `radius_m`：Segment 范围外扩半径，单位米，当前切片 CRS 为 `EPSG:3857`。
- `include_files`：是否物化外部输入文件；`true` 时默认生成空间切片。
- `materialization_mode`：可选，允许值：
  - `spatial_slice`：正式默认；按 Segment 几何 bounds + radius 生成局部 GPKG 切片。
  - `manifest_only`：只写 manifest，不扫描或复制全量输入内容。
...
- 其它外部输入 slot、`RADIUS_M / INCLUDE_FILES / MATERIALIZATION_MODE / OUT_ROOT / BUNDLE_ROOT / MAX_TEXT_SIZE_BYTES / DECODE_AFTER_EXPORT / TESTDATA_ROOT` 与 Case 入口一致。
...
11. `spatial_slice` 模式不得复制全量外部输入；每个 Case 目录只能物化半径窗口内的局部 GPKG。
12. `spatial_slice` 必须审计道路端点节点依赖完整性，不允许 silent fix。
13. Segment package 必须从 T10 run root 记录 T01 Segment source 和 T06 evidence references，不得只按 SegmentID 猜测外部输入范围。
