...
T08 通过已登记 `scripts/t08_tool*.py` 执行 Tool1-Tool11；其中 Tool1 通过 `--input-shp / --input-geojson / --input-fgb / --input-gpkg` 提供基础矢量格式转换。Tool10 另提供 `scripts/t08_tool10_run_patches_innernet.sh` 作为多 Patch 参数化批处理入口。Tool11 的正式核心入口为 `scripts/t08_tool11_patch_data_organization.py`；内网 WSL 可通过 `scripts/t08_tool11_run_innernet.sh` 使用已确认的原始根和全量输出根、持久日志及显式覆盖开关，默认只整理全量 Patch。实验根仅在显式配置时启用。每个工具的参数、输入输出和约束以 `INTERFACE_CONTRACT.md` 与 `docs/repository-metadata/entrypoint-registry.md` 为准。
