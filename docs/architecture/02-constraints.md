...
- 输入字段治理约束：
  ...
  - `Road.kind` 已正式启用；当前确认其字段语义为道路种别，单个 token 使用 `XXXX` 形式表达，前两位为道路等级、后两位为道路类型，多个道路种别用 `|` 分隔。当前仅在 T08 Tool4 入出度异常豁免中启用后两位道路类型：任一 token 后两位为 `0a`（大小写不敏感）时，该 Road 视为辅路。
