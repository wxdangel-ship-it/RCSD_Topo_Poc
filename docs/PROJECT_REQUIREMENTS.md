...
T12 面向通过 1V1 匹配技术融合生成的原始 F-RCSD，不把它解释为 T06 Segment 替换结果。T12 以“SWSD 与 1V1 F-RCSD 的方向拓扑通行性应等价”为待验证质量假设，复用 T06 的 ID、方向、canonical node、carrier graph、局部 portal 与 RCSD Road—Segment 唯一归属排序语义，既检查已锚定 Segment 的 SWSD 必需方向在目标承载网是否存在可解释通行路径，也检查单向 Segment 是否在目标网的明确双端路口锚点之间出现唯一归属于当前 Segment 的几何等价非预期反向载体。1V1/T05 锚定到 FRCSD mainNode 时，只把选中 `base_id` 所代表 canonical group 的 raw subNode/alias 展开到实际 Road endpoint；其它显式 grouped raw node 保留但不递归展开其各自 group。source/target 分别按当前方向的 outgoing/incoming Road 过滤；锚定 alias 的几何距离只作审计，非锚定 spatial node 仍执行既有范围/标准面门禁。`RCSDIntersection` 是 T07/T10 标准输入和人工确定的现实路口证据；T05/T06 只提供交叉解释证据，不替代 T12 对原始目标网的判断。

T12 必须分离 candidate、confirmed、excluded 和 optional review override。candidate 不是正式质量问题；已锚定 canonical alias 只有在 raw identity node 图上按 Road Direction 形成物理 carrier 时才能证明等价，mainNode 折叠本身不能证明通行。原始 FRCSD Road endpoint 图证明 SWSD 必需方向缺少等价 carrier 后，还必须检查 portal-constrained semantic carrier 和 T07 Road-surface portal carrier。既有 semantic carrier 继续要求物理 Road、方向/长度/走廊、端点 portal 与内部 alias 门禁。Road-surface carrier 只在两端均为正确且唯一的 T07 `RCSDIntersection` 标准面锚定时启用：实际有向 Road 与对应标准面相交，或 Road frontier 可被锚点组一跳物理 Road 明确连接，均可形成 surface access；一跳 support Road 必须从 anchor 向 frontier 有向并与标准面相交或位于 `1m` 拓扑容差内，整条 carrier 至少一端必须存在实际 Road-surface contact，双端仅靠任意一跳邻接不得成立。方向、物理 Road、锚点唯一性和路径长度等价仍是强门禁；锚定 alias、Road-surface gap、SWSD portal gap、内部 alias gap 和走廊距离等距离指标仅作人工审计风险，不得单独拒绝。两类 carrier 都只能作为高置信误报排除证据，不能单独确认问题。完成两层排除检查并通过锚点可信度门禁后仍缺失的记录，才能自动进入 confirmed 层。对 SWSD 要求方向已经等价的单向 Segment，T12 还可反转门户检查 FRCSD raw 反向载体；若 SWSD 全图存在通过相同几何阈值的反向替代路径则自动排除。无替代路径时，反向路径第一/最后 Road 还必须在 `1m` 容差内接触当前 Segment 双端唯一 T07 标准面，并在扣除两端标准面后逐 raw RCSD Road 按 `20m coverage > 50m coverage > geometry distance` 证明当前 Segment 是唯一最优 owner；其它 Segment 更强覆盖、并列或区间未证明均自动排除。只有这些条件全部成立时才自动确认 S03 `segment_unexpected_reverse_passability`，T03/T03 等弱锚点只保留候选/排除证据。canonical 零长度折叠、无物理 Road 路径和任意近邻节点不得补出正式 carrier。外部 review decisions 只作可选 QA 覆盖，不再是 confirmed 的前置条件。排除原因必须可追溯，任何阶段都不得修改输入几何、自动补路或 silent fix。

T12 同时独立发布 Junction Point 质量问题。T03 Step7 rejected 只提供候选，必须满足 `has_evd=yes / is_anchor=no / kind_2 in {4,2048}` 且正式审计链完整；T12 在原始 1V1 FRCSD 中重算 target projection、局部 support ownership、terminal endpoint degree、support component、Direction 与替代 carrier，只在 `shared_degree1_terminal_collapse` 或 `multi_component_unmatched_support` 全部门禁成立且无无效几何、正式高置信跨层解释时确认 J01 `junction_required_topology_missing` 或 J02 `junction_unmatched_support_topology`。默认 `6m` endpoint tolerance 与 `50m` local scope 只用于投影、局部检索和审计，距离本身不得形成 verdict；同 endpoint 的其它 Segment Road 只进入全局 degree 审计，不自动并入当前 Junction support。T07 只以 Step2 代表路口 final `is_anchor=fail1/fail2` 为真值，分别发布 J03 `junction_anchor_one_to_many` 与 J04 `junction_anchor_many_to_one`；`fail2 > fail1`，J04 按冲突分量逐 Junction Point 输出并共享 conflict group。`node_error_1/2.gpkg`、Step2 summary 和 relation evidence 必须与 final state 一致；Step3 `relation_cardinality_errors.*` 导入数恒为 0。Segment 继续沿用 T01 Segment 线几何族（GeoPackage 可为 `LineString/MultiLineString`），Junction 为 Point；两者的 candidates/confirmed/exclusions/evidence、计数和根因必须分层，不修改 T03/T07 算法或回写任何输入。

### 3.7 编排与证据层

T10 负责组织端到端 Case package、Case replay、full pipeline manifest、T06 funnel、可选 T12 quality audit、T11 candidate audit、visual check 和 feedback package。T10 不定义或改写 T01-T09 / T11 / T12 的算法规则，不把 T06 feedback 直接作为 Step3 替换白名单。T10 v1 Case runner 默认在 T06 后、T09 前执行 T11；显式启用并提供原始 1V1 F-RCSD 时在 T11 后、T09 前执行 T12，并把当前 T03 run root 与同次 T07 Step1/2 run root 显式交给 T12。T12 的 T07 Junction 质量来源只允许是 Step2 final `fail1/fail2`，不得消费 Step3 cardinality。两者都不改变 T06 到 T09 的业务 handoff。T10 提供固定 `RUN_T08=0 / RUN_T12=1` 的 F-RCSD 质量检查专用流水线；普通 Case runner 不调用 T08，通用内网全量总控仍可把 T08 作为独立前置阶段串入。
...
| T12 | 原始 1V1 F-RCSD 质量审计；验证 SWSD 方向可达性等价假设，覆盖必需方向缺失与单向 Segment 的非预期反向载体。以 raw endpoint topology 为主，以 portal-constrained semantic carrier、T07 Road-surface portal carrier 和 SWSD 反向替代路径为误报排除门禁，结合标准路口和锚点可信度自动发布高置信问题与排除证据，人工 review 仅作可选 QA 覆盖，不执行修复。 |
