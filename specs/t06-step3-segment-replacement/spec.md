...
Step3 必须保持可审计性：每个被替换 Segment 删除了哪些 SWSDRoad / SWSDNode、引入了哪些 RCSDRoad / RCSDNode、影响了哪些语义路口 C、每个 C 如何选择新的 main node，都必须有审计输出。
...
- Step3 summary 记录输入数量、替换数量、删除数量、引入数量、重建 C 数量和失败原因。
