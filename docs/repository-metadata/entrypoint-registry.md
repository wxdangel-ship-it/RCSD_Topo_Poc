...
| `t10_run_frcsd_quality_pipeline.sh` | `scripts/t10_run_frcsd_quality_pipeline.sh` | repo 级 | T10 原始 1V1 F-RCSD 质量检查专用流水线；固定 `RUN_T08=0 / RUN_T12=1`，按 `T01 -> T07 -> T03 -> T04 -> T05 -> T06 -> T11 -> T12 -> T09` 复用通用 full runner，要求显式原始 1V1 F-RCSD Road/Node，混合 CRS 时接受显式 `T12_PROCESSING_CRS`。已有 T10/T06 run 可用 `RESUME_RUN_ROOT + RUN_STAGES=t12` 只重跑 T12，并默认生成新的 timestamped `T12_RUN_ID`，不重跑 T01-T11/T09、不覆盖旧 T12 输出；也支持显式 `T12_RUN_ID / T12_REVIEW_DECISIONS` 续跑发布 confirmed，沿用 manifest、summary、resume 和失败退出，不复制模块算法 | `active` | 否 |
