...
- `yes_nr` 适用于 Phase 2 审计确认 `no_related_rcsd`，且 SWSD node 原始 `has_evd = yes / is_anchor = yes` 的 target；T05 必须只在 copy-on-write `swsdnode_out.gpkg` 中改写为 `yes_nr`，并输出 `swsdnode_yes_nr_audit.csv/json`；不得原地修改输入 nodes。
- `summary.json` 必须记录 Phase2 审计 no-RCSD target 数、T03/T04 前置成功 no-RCSD 诊断 target 数、二者重叠数、节点匹配数、候选数与未匹配数，用于定位 `swsdnode_yes_nr_count = 0` 的原因。
