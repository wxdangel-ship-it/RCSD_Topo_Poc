...
提前右转 road 分为两类：路口前提前右转不进入当前语义路口，不进入 seed、connector、member 或 trunk，必须生成 `AdvanceRightTurnRelation` 或明确 issue；路口内提前右转按 `direction` 进入当前语义路口，进入 seed / member，作为非 trunk member 保留，不进入 `AdvanceRightTurnRelation`。bit7 relation 候选范围包括 Arm 非特殊 inbound / bidirectional seed 外侧节点相邻的 bit7 road，避免提前右转接在路口前 seed 外侧时被漏掉。连续路口前 bit7 road 链按一条 Arm 级提前右转 relation 输出，链内 road segment 保留在 relation 的 `advance_right_turn_road_ids` 中。提前左转 road 可以进入 Arm member，但必须标记并从 trunk 中排除；未进入 Arm member 的外侧 bit8 road 不计为当前 Arm 的提前左转。
...
为辅助识别 trace 在 T 型或 ambiguous boundary 前被过度切碎的 case，模块额外输出 `LocalArmCandidate`。该候选基于当前语义路口 seed road 的进入 / 退出角色、从 member node 指向外侧 node 的局部趋势、同向 inbound/outbound 配对、outbound 顺时针连接下一 inbound 的道路设计关系，以及 source InitialArm 唯一覆盖，形成方向性 seed corridor 分组。它不使用目标 case ID；当候选完整覆盖 InitialArm 时可进入 FinalArm 兜底聚合，并避免同一 InitialArm 被多个 FinalArm 复用。
...
A2 以 F-RCSD FinalArm 为目标承载体。每个 FinalArm 先归一化为 ArmProfile，汇总 InitialArm、LocalArmCandidate、ArmCorridorEvidence、ArmTrace 与 ThroughDecisionAudit 证据。候选边覆盖 FRCSD-SWSD、FRCSD-RCSD 与 SWSD-RCSD，评分由 seed role、走廊趋势、trace / terminal、road coverage 和 geometry 辅助证据组成。

几何只作为辅助分数；非几何证据不足时不得 high confidence。A2 形成跨三源 LogicalArmGroup，并显式区分 source_missing / source_partial 与 over_split / over_merged / conflict / uncertain。候选选择先按源数据集做 source Arm 互斥优先分配；当某个 F-RCSD Arm 存在可用替代候选时，不复用已被更优 F-RCSD Arm 占用的 source Arm。无法通过可用替代候选解除的 over_merged 不自动拆分，只输出 ArmBuildFeedback。

## RoadNextRoad-aware ArmMovement 与 P01-Final

RoadNextRoad 在 A1 阶段只表达 allowed evidence；缺失只输出 `no_allowed_evidence`，不代表禁止。movement_type 使用 same-arm、唯一 stable straight target、trunk / ArmCorridorEvidence / LocalArmCandidate 方向连续性和相对侧向关系判定，不使用 `turnType / turntype`。
...
P01-Final 中，SWSD / RCSD RoadNextRoad 先抽象为进入道路角色到目标 Arm 的通行规则。`full_allowed` 生成到目标 Arm 全部退出 Road；`prohibited` 不生成；主干道路 / 平行支路的无法解释部分目标覆盖进入 `data_error_partial_target_coverage`，不自动投影；如果主干 evidence 完整覆盖目标 trunk road，则输出 `trunk_only_allowed` 并只投影 trunk。advance-left left-receiving only 与 uturn trunk only 是合法特殊范围。F-RCSD Arm 可以混源，规则源按 SWSD 结构匹配、RCSD 结构匹配、SWSD basic rule 兜底选择；primary source 没有可生成规则时，可以使用 alternate source 的 Arm role / corridor ordinal 证据低置信投影；参考 RCSD 但目标 Arm 缺失时 fallback 到 SWSD basic rule。精确源 Road 映射、source map 与旧 SourceMovementPolicy 只用于审计和兼容解释。平行支路以 `parallel_branch_alignment.json` 独立记录 source missing、count matched ordered、count mismatch 与 insufficient geometry ordering 状态。
