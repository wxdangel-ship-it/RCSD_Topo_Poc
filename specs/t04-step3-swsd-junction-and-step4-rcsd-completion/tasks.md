...
- [x] 字节数自检：以上每个被改动的 `.md` 文件无体量约束（仅源码 / 脚本受 §3 约束），但仍记录修订摘要到 PR 描述。
...
- [ ] 字节数前置自检：`rcsd_alignment.py / _event_interpretation_core.py / step4_road_surface_fork_rcsd.py / case_models.py`。
- [ ] 在 `rcsd_alignment.py` 增加：
  - [ ] `@dataclass(frozen=True) class RCSDSemanticArm` —— 与 SWSDSemanticArm 对称结构（`inter_junction_connector_rcsdroad_ids / neighbor_rcsd_junction_id` 等）。
  - [ ] `@dataclass(frozen=True) class RCSDSemanticJunction` —— `junction_id / member_rcsdnode_ids / intra_junction_rcsdroad_ids / semantic_arms / paired_swsd_arm_mapping / alignment_partial_missing_swsd_arm_ids`。
- [ ] 在 `step4_road_surface_fork_rcsd.py` 或 `_event_interpretation_core.py`（按文件体量决定）实现：
  - [ ] `_build_rcsd_semantic_junction(unit, swsd_semantic_junction, rcsd_alignment_decision) -> RCSDSemanticJunction | None`。
  - [ ] 复用 `aggregated_rcsd_units / local_rcsd_units / required_rcsd_node` 已有信号；intra/connector 切分逻辑与 SWSD 完全对称（走 RCSDRoad / RCSDNode 图）。
  - [ ] `paired_swsd_arm_mapping`：基于 angle（30° 容差）+ direction role 与 SWSD `semantic_arms` 比对；多解落 `null` 并写 `audit.pairing_ambiguous_arm_ids`。
- [ ] `case_models.T04EventUnitResult` / `case_models.T04CandidateAuditEntry` 增加 `rcsd_semantic_junction: RCSDSemanticJunction | None`。
- [ ] `case_models.T04EventUnitResult.to_summary_doc` 与 `outputs.write_case_outputs / step4_event_interpretation.json / step4_candidates.json` 持久化新字段。
- [ ] 字节数复检；如 `case_models.py` 接近 100 KB 则按 §3 拆分。
- [ ] 单元测试扩展 `tests/modules/t04_divmerge_virtual_polygon/test_step4_rcsd_alignment_type.py`：
  - [ ] `rcsd_semantic_junction` → `paired_swsd_arm_mapping` 全配对。
  - [ ] `rcsd_junction_partial_alignment` → `alignment_partial_missing_swsd_arm_ids` 非空。
  - [ ] 多解 → 对应 arm 配对为 `null` 且 audit 记录歧义原因。
...
- [ ] 字节数前置自检：相同范围 `.py`。
- [ ] 在 `rcsd_alignment.py` 增加 `@dataclass(frozen=True) class RCSDRoadOnlyChain`：
  - [ ] `chain_road_ids / chain_endpoint_node_ids / chain_endpoint_kinds / closure_status / swsd_direction_consistent / swsd_direction_evidence / selection_uniqueness_proof`。
- [ ] 实现 `_build_rcsdroad_only_chain(unit, swsd_semantic_junction, candidate_rcsdroad_ids, rcsd_road_graph) -> RCSDRoadOnlyChain | None`：
  - [ ] 输入：`fallback_rcsdroad_ids ∪ first_hit_rcsdroad_ids` 候选集合；当前 SWSD `semantic_arms` 角度信号。
  - [ ] 拓扑序排序：从一端 RCSDNode 出发，沿 RCSDRoad 序列首尾相接构造 chain。
  - [ ] `closure_status` 取值：`closed_between_two_rcsd_junctions / open_dead_end / open_patch_boundary / unresolved`。
  - [ ] `swsd_direction_consistent`：chain head/tail tangent 与最近 SWSD arm `angle_deg` 比对（容差 30°）；记录 evidence。
  - [ ] 多候选 chain 时复用 `ambiguous_rcsd_alignment` 阻断逻辑；唯一选中需有 `selection_uniqueness_proof`。
- [ ] `T04EventUnitResult.rcsdroad_only_chain: RCSDRoadOnlyChain | None`。
- [ ] 持久化到 `step4_candidates.json / step4_event_interpretation.json`。
- [ ] 单元测试新增：
  - [ ] `closure_status = closed_between_two_rcsd_junctions` 用 synthetic 构造（**D4 决策**：39-case 实证可能为 0，但 synthetic 单测必须覆盖该状态分支）。
  - [ ] 单端 dead end → `open_dead_end`。
  - [ ] patch 边界 → `open_patch_boundary`。
  - [ ] `swsd_direction_consistent = true / false` 各一例（**D5 决策**：固定 30° 容差）。
  - [ ] 多候选 chain → 阻断或唯一选中带 proof。
- [ ] **D5 决策守门**：`_build_rcsdroad_only_chain` 内角度容差常量必须直接复用现有 `BRANCH_MATCH_TOLERANCE_DEG = 30.0`，不得新建独立常量；单元测试断言该来源。
- [ ] **D5 审计输出**：每个 RCSDRoad-only chain 的 `swsd_direction_evidence` 字典必须包含 `chain_head_angle_deg / chain_tail_angle_deg / matched_swsd_arm_id / angle_gap_deg / consistency_decision_reason` 五个字段，供 Phase 6 跑完 39-case 后审计角度差分布。
...
- [ ] 字节数前置自检：`rcsd_alignment.py / case_models.py / outputs.py / 各 binding 模块`。
- [ ] 在 `rcsd_alignment.py` 新增：
  - [ ] `class ConsistencyVerdict(StrEnum)` —— 取值 `strong_consistent / partial_consistent / direction_only_consistent / not_applicable / inconsistent / blocked`。
  - [ ] `RCSD_CONSISTENCY_RESULT_VALUES: tuple[str, ...]` —— §4.6 冻结值域。
  - [ ] `compute_consistency_verdict(rcsd_alignment_type, positive_rcsd_consistency_level, axis_polarity_inverted, rcsdroad_only_chain) -> ConsistencyVerdict` 纯函数。
- [ ] `T04EventUnitResult.swsd_rcsd_alignment_consistent: ConsistencyVerdict`；`to_summary_doc / to_csv_row` 同步。
- [ ] `outputs.REVIEW_INDEX_FIELDNAMES` 增加 `swsd_rcsd_alignment_consistent` 列；`T04ReviewIndexRow` 同步增字段。
- [ ] `outputs.write_review_summary` 增加 `swsd_rcsd_alignment_consistent_counts` 分布统计。
- [ ] 收口 `rcsd_consistency_result`：
  - [ ] 全仓 `rg "rcsd_consistency_result\s*=" --type py` 列出所有写入点（已知 ≥ 8 处）。
  - [ ] 每处写入前 `assert value in RCSD_CONSISTENCY_RESULT_VALUES`，越界字符串触发 `ValueError`（不静默写入）。
  - [ ] 现存字符串如 `"positive_rcsd_strong_consistent" / "positive_rcsd_partial_consistent" / "road_surface_fork_without_bound_target_rcsd" / "missing_positive_rcsd"` 全部纳入值域。
  - [ ] 新增 `"positive_rcsd_direction_only_consistent" / "positive_rcsd_inconsistent" / "none"` 显式补齐。
- [ ] 单元测试 `tests/modules/t04_divmerge_virtual_polygon/test_consistency_verdict.py`：
  - [ ] `compute_consistency_verdict` 真值表全覆盖。
  - [ ] 越界 `rcsd_consistency_result` 写入 → `ValueError`。
  - [ ] CSV 列序与 INTERFACE_CONTRACT 文档一致。
...
- [ ] 跑 30-case baseline gate：`pytest tests/modules/t04_divmerge_virtual_polygon/test_step7_final_publish.py::test_anchor2_full_20260426_baseline_gate -x`。
  - [ ] `accepted = 20 / rejected = 3` 不漂移；`857993 = fail4 / 699870 = yes`。
- [ ] 跑 30-case surface scenario gate：`...::test_anchor2_30case_surface_scenario_baseline_gate -x`。
  - [ ] `accepted = 26 / rejected = 4` 不漂移。

### 6.2 39-case 重新跑 + 新基线生成

- [ ] 跑 Anchor_2 39-case full baseline（手工命令或 `t04_run_internal_full_input_8workers.sh`）：
...
- [ ] 全 39-case 自动核查（必须全绿）：
  - [ ] 每 case `step3_status.json.swsd_semantic_junction.junction_id != ""`。
  - [ ] 每 case `step3_status.json.swsd_semantic_junction.intra_junction_road_ids ∩ Σ inter_junction_connector_road_ids = ∅`。
  - [ ] 每 case Step5 `related_swsd_road_ids` 与 Step3 派生集合（`intra ∪ Σ connector`）逐项相等。
  - [ ] 每 case `summary.json.performance.threshold_status = within_threshold`。
  - [ ] 全 39-case 总耗时较 30-case baseline `~180–184s` 上涨 ≤ 5%。
  - [ ] 每 case `swsd_rcsd_alignment_consistent` 字段非空且与 alignment_type / consistency_level / axis_polarity_inverted 推导一致。

### 6.3 命名回归 case（用户人工核对发现渲染缺失）

- [ ] `724067`：
  - [ ] `swsd_semantic_junction` 输出非空。
  - [ ] 派生道路集合（`intra ∪ Σ connector`）与 `final_review.png` 上 SWSD 路网图层逐条目视核对，**全部可见**。
  - [ ] 任何缺失道路：定位是 Step3 实体未召回还是渲染层未绘制；前者修 `_walk_arm_to_neighbor_semantic_junction`，后者修 `review_render`。
- [ ] `758784`：同上 3 项核查。
- [ ] `760213`：同上 3 项核查。
- [ ] 命名 case 全部通过后，把这三个 case 写入 `tests/modules/t04_divmerge_virtual_polygon/test_step3_swsd_semantic_junction.py` 作为 fixture 锁定（snapshot 比对：`junction_id / intra_junction_road_ids / Σ semantic_arms[].inter_junction_connector_road_ids` 全集）。

### 6.4 全 39-case 渲染缺失抽查（不限于命名 case）

- [ ] 对全 39-case 自动生成对照清单：每 case 输出 `(case_id, swsd_entity_road_count, render_visible_road_count, missing_road_ids)` 到 `outputs/_work/t04_swsd_render_audit/<run_root>/render_audit.csv`。
  - 实现策略：在 `review_render` 输出 PNG 同时，dump 一份"图层实际渲染的 SWSD road id 集合"到 audit JSON；与 Step3 实体派生集合做 set-difference。
- [ ] 任意 case 出现 `missing_road_ids != []` → 视为 Phase 6 失败项，回退到 Phase 1–2 修 `_walk_arm_to_neighbor_semantic_junction` 或 `review_render`，再跑 6.2 / 6.3 / 6.4。
- [ ] 渲染缺失修复后，把所有"Phase 6.4 阶段抽查发现遗漏的 case"也加入 6.3 的 fixture snapshot（命名 case + 抽查发现 case 共同构成 SWSD 实体回归 fixture 集）。

### 6.5 最终目视审计图

- [ ] 39-case 全部通过 6.2/6.3/6.4 后，`run_root/cases/<case_id>/final_review.png` 即作为**本轮新目视审计基线**；不与 2026-05-01 的 23-case PNG fingerprint 做对比。
- [ ] 把 run_root 路径登记到 `architecture/10-quality-requirements.md` 的 *Anchor_2 full baseline gate* 段落作为新参考。
- [ ] **不**新增 PNG fingerprint hash 断言（保持当前 23-case fingerprint gate 不动；本轮新基线作为人工审计参考存在，不进 pytest）。

### 6.6 D4 / D5 数字落地

- [ ] **D4 实证统计**：39-case 跑完后，把 `closure_status` 在 39-case 上的实际分布数字写入 `notes/run-log.md`（例如 `closed_between_two_rcsd_junctions: 0 / open_dead_end: 4 / open_patch_boundary: 7 / unresolved: 0`）。若 `closed_between_two_rcsd_junctions = 0`，同步在 `INTERFACE_CONTRACT.md §2.5` 末尾追加一句"当前 Anchor_2 39-case 数据集无该状态实证 case，作为契约预留状态保留"。
- [ ] **D5 角度差分布**：把所有 RCSDRoad-only case 的 `angle_gap_deg` 数字汇总到 `notes/run-log.md` 表格（case_id / chain_head / chain_tail / matched_swsd_arm / angle_gap_deg / consistent）；若发现密集落在 25–35° 边界区间，在 `notes/run-log.md` 末尾标注"建议下一轮 SpecKit 评估容差调整"，但**本轮不调整**。
...
- 新 run_root 路径与登记到 `10-quality-requirements.md` 的 commit hash。

---

## Phase 7 — QA / Documentation Closeout

- [ ] 验证 CRS：所有新增 `step3_status / step4_event_interpretation / step5_status` 写出未改 CRS。
- [ ] 验证 geometry valid：新增字段不引入 geometry；纯 ID + 元数据，无 valid 风险。
- [ ] 验证文件体量：再次运行 `Get-ChildItem`（PowerShell）或 `stat -c%s`（POSIX）自检，所有源码 `.py` 不跨 100 KB；如发生拆分，确认 `docs/repository-metadata/code-size-audit.md` 已同步。
- [ ] 验证契约一致性：`step4_review_index.csv` 列序与 `INTERFACE_CONTRACT.md §4.4` 一致；`step3_status.json` schema 与 §2.4 一致；`step4_event_interpretation.json` schema 与 §2.5 一致。
- [ ] 性能审计：`summary.json.performance` 字段齐全；`threshold_source = module_quality_requirement_default_or_env_override`；`threshold_status = within_threshold`。
- [ ] 视觉审计采样：从 39-case 中随机抽 5 个 case，人工核对 `final_review.png` 上 SWSD 路口道路集合 = `swsd_semantic_junction` 派生集合（采样核对作为 6.4 全量自动核查之上的人工抽样保险）。
- [ ] 生成 Release Notes 草稿：`已修改 / 已验证 / 待确认` 三档分明，置于 `specs/t04-step3-swsd-junction-and-step4-rcsd-completion/notes/release-notes.md`。
- [ ] 治理工件登记：在 `specs/t04-step3-swsd-junction-and-step4-rcsd-completion/notes/run-log.md` 写执行轨迹（每 Phase 起止时间 + commit hash + PR 链接 + run_root）。
...
读完每份文档后，在 PR 描述中**显式声明**"已阅读 ✅"。读不完就启动编码视为治理违规。

### 8.2 分支与 PR 模式（每 Phase 一 PR，强制）

- 主分支：`main`（受保护，不允许直接 push）。
- SpecKit 分支命名：`speckit/t04-step3-swsd-junction-phase<N>-<short-name>`，例：
  - Phase 0：`speckit/t04-step3-swsd-junction-phase0-contract-freeze`
  - Phase 1：`speckit/t04-step3-swsd-junction-phase1-swsd-junction-dataclass`
  - Phase 2：`speckit/t04-step3-swsd-junction-phase2-step5-render-migration`
  - Phase 3：`speckit/t04-step3-swsd-junction-phase3-rcsd-junction-dataclass`
  - Phase 4：`speckit/t04-step3-swsd-junction-phase4-rcsdroad-only-chain`
  - Phase 5：`speckit/t04-step3-swsd-junction-phase5-consistency-verdict`
  - Phase 6：`speckit/t04-step3-swsd-junction-phase6-real-case-regression`
  - Phase 7：`speckit/t04-step3-swsd-junction-phase7-qa-closeout`
- 每个 Phase 一个独立 PR：**Phase N 的 PR 必须 base 在 Phase N-1 已合并的 main 上**，不允许多 Phase 并行。
- Phase 0 PR base 在 main 上；Phase 1 PR 等 Phase 0 合并后再开。
- **不允许整轮一次性大 PR**；不允许跨 Phase 顺手携带未授权改动（`AGENTS.md §4`）。
- PR 体量参考：单 PR diff ≤ 2000 行（含测试）；超出立刻拆 sub-phase。
...
## 已修改
- 文件路径 1：本 PR 的修改目的（一句话）
- 文件路径 2：本 PR 的修改目的（一句话）
...
...
- 本 SpecKit 已知接近阈值的高风险文件（必须 PR 内自检）：
...
每 Phase PR 合并前测试不绿不允许合并。**不允许 `--ignore` / `xfail` 静默跳过任何测试**（如确需跳过，spec.md 同轮显式说明并经用户授权）。

### 8.6 硬停机回报（`AGENTS.md §1` 末段强制）

任何时刻命中 §1.1–§1.7 → 立即停机并发**单独 issue / PR comment**，内容必须包含：
...
每个 PR 的 description 与最终回复必须区分**已修改 / 已验证 / 待确认**三档。"看起来应该可以"不得表述为"已经修复"。

### 8.8 治理工件维护

Phase 0 起在本 SpecKit 目录维护：

- `notes/run-log.md`：每 Phase 的 起止时间 / commit hash / PR 链接 / run_root 路径 / 关键决策点。
- `notes/release-notes.md`：Phase 7 写就绪后的 Release Notes 草稿（已修改 / 已验证 / 待确认 三档）。
- 每个 Phase PR merge 后立即更新 run-log.md（同 PR 或独立小 PR 均可）。

### 8.9 Phase 完成判据（用户验收）

每个 Phase PR 申请合并前，必须满足：
...
不满足任一条 → 用户有权 reject PR；Codex 重做后重新申请。

### 8.10 全任务结束

Phase 7 PR 合并后，Codex 必须发**最终汇总回报**：

- 7 个 PR 的 link 列表 + 每个 PR 关联的 Phase。
