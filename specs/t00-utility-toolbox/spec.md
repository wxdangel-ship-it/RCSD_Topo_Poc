...
对 `patch_all/<PatchID>/Vector/DriveZone.geojson` 做单 Patch 预处理，再形成一个全局 `DriveZone.geojson`。
...
8. 单 Patch 结果仅用于后续全量合并，不强制成为正式持久化输出

### 6.4 全量处理流程

1. 将所有单 Patch 处理后的 `DriveZone` 再做一次全量面合并
2. 全量合并完成后，再进行一次拓扑保持的几何简化
3. 输出一个全局 `DriveZone.geojson`
