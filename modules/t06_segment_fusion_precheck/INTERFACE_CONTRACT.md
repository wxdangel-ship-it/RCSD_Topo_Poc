...
Step2 运行诊断 sidecar 只用于性能和卡点定位，不参与替换率、replacement plan、problem registry 或 Step3 handoff。`t06_step2_heartbeat.json` 记录当前 PID、当前 Segment、group_audit 当前组或后处理阶段；`t06_step2_progress.jsonl` 记录阶段切换、每 1000 个 Segment 的进度和 group_audit 稀疏组级进度；`t06_step2_slow_units.jsonl` 记录耗时超过阈值的 Segment；`t06_step2_slow_groups.jsonl` 记录耗时超过阈值的 group_audit 组；Linux/WSL 环境可用 `kill -USR1 <PID>` 将当前 Python 栈写入 `t06_step2_stackdump.log`，用于不中断长耗时进程定位 CPU-bound 热点。
