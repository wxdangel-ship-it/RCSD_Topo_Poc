...
Tool2 命令脚本输出 Patch join / Kind enrich 阶段进度；Kind enrich 复用 raw Kind token 缓存与 STRtree 查询结果，summary 记录阶段耗时、吞吐与空间候选数。
...
Tool3 命令脚本输出读取、初始化、环岛聚合、复杂链聚合与写出进度；复杂链 component 组装使用 node-to-component 索引归集边，避免按组件反复全量扫描链路边。
