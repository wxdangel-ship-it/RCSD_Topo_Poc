...
- 对 `single_sided_t_mouth`，当目标语义节点仅以小容差贴近 DriveZone 边界，且每个目标节点均有进入 DriveZone 的 incident road 支撑时，Step3 可扩大“候选组件触达目标”的参考容差；该规则只解决贴边目标点误判，不放宽最终 `allowed_space` 必须完全落在 DriveZone 内的硬约束，并必须在 `target_edge_touch_enabled / reason / tolerance_m / target_drivezone_distances_m` 中审计。
