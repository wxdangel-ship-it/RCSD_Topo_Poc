...
插件加载后注册两个 QGIS Dock：左侧 `T11 Relation Tasks` 负责单个 workbook 加载、图层绑定、分页和任务列表；setup 区可折叠，并在任务加载成功后自动收起以给任务列表让出空间。任务管理区提供 `Font` 字号控件，同时作用于任务管理与任务处理两个 Dock。任务列表单行展示人工数据符号、`target_id` 和 `swsd_segment_id`，其中 `❌ -` 表示未填写或仅部分填写、`✅ +` 表示已满足人工完成判定；普通 relation 必须同时具备 `manual_relation_type` 与非 `NULL` 的 `selected_ids` 才显示为已填，`no_valid_relation` 必须配套 `selected_ids=NULL` 才显示为 `NULL` 确认，`uncertain` 保留独立状态；完整状态和人工字段保留在 tooltip。底部 `T11 Relation Processing` 负责当前任务摘要、人工字段编辑、上一条 / 下一条、定位、高亮、selection 写入、清空、`NULL` 与 `uncertain` 标记；第一行保留 `manual_relation_type` 下拉输入框并与 `selected_ids` 同行，下一行提供 relation type 快捷按钮；点击 junction / road 类型按钮或切换到已有类型任务后，插件会激活对应的 RCSDNode / RCSDRoad 绑定图层，便于直接点选后写入 `selected_ids`。点击任务后定位到 SWSD 语义路口或 Segment 上下文，显式居中，默认缩放到约 `1:1500`，并保持匹配 SWSD 要素处于选中状态以便识别。
