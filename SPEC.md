...
- 启动 `t08_preprocess`，完成 Tool1 基础矢量格式转换、Tool2 Road GPKG 预处理、Tool3 Nodes 类型聚合与 Tool4 路口类型错误识别的文档契约、模块内 callable runner、内网执行脚本与最小测试闭环
...
- T01 双向 Segment 构建正式启用 `kind_2 = 128` 的复杂分歧 / 合流穿越审计语义：该值在双向首轮不作为 `seed / terminate / hard-stop`，允许被图搜索穿越；穿越结果必须通过 `kind_2_128_*` 输出字段可追溯，不改变 `through_node_ids` 的 degree-based 语义。
...
- 已登记正式模块 `t08_preprocess` 的 Tool1 基础矢量格式转换、Tool2 Road GPKG 预处理、Tool3 Nodes 类型聚合与 Tool4 路口类型错误识别；Tool1 支持 SHP / GeoJSON 转 GPKG 与 GPKG 转 GeoJSON，输出均写回输入目录下同名文件；Tool2 补充 `patch_id / kind`，Tool3 补充 `kind_2 / grade_2` 并处理环岛、复杂分歧 / 合流 mainnode，Tool4 输出 `nodes_error.gpkg` 记录错误语义路口类型，输出 `EPSG:3857` GPKG
...
- T01 双向 Segment 构建中，`kind_2 = 128` 表达复杂分歧 / 合流路口组合的可穿越审计语义：允许穿越但必须输出 candidate / validation 级 `kind_2_128_*` 统计，用于定位复杂路口穿越对候选规模、拒绝原因与性能的影响。
...
- `t06_segment_fusion_precheck` 当前作为 Active 正式业务模块启动；正式范围仅覆盖 Step1 SWSD 可融合 Segment 识别与 Step2 RCSD Segment candidate 抽取、趋势硬筛、可替换集合和错误分析，不执行 Segment 替换，不重塑路口，不修改 T01 / T05 输出；当前提供模块内 callable runner 与 `scripts/t06_run_innernet_precheck.py` 内网运行包装，不新增 repo 官方 CLI。
- `t08_preprocess` 当前作为 Active 正式预处理模块启动；当前覆盖 Tool1 基础矢量格式转换、Tool2 Road GPKG 预处理、Tool3 Nodes 类型聚合与 Tool4 路口类型错误识别，Tool1 支持 SHP / GeoJSON 转 GPKG 与 GPKG 转 GeoJSON，输出均写回输入目录下同名文件，并采用流式读写输出控制台进度，Tool2 只消费 GPKG 输入、补充 `patch_id / kind` 并输出 `EPSG:3857` GPKG，Tool2 / Tool3 / Tool4 均输出阶段进度与性能 summary，Tool3 只消费 GPKG Nodes/Roads 输入、补充 `kind_2 / grade_2` 并处理环岛、复杂分歧 / 合流 mainnode，Tool4 只消费 GPKG Nodes/Roads 输入、输出 `nodes_error.gpkg` 记录错误语义路口类型；当前提供模块内 callable runner 与 `scripts/t08_tool1_vector_convert.py`、`scripts/t08_tool2_road_preprocess.py`、`scripts/t08_tool3_nodes_type_aggregation.py`、`scripts/t08_tool4_junction_type_repair.py` 内网脚本，Tool4 不自动修复。
