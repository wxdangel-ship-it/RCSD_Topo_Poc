...
- Segment replay 的 T07/T03/T04 无候选不视为打包遗漏：当 T07 stdout 明确无 RCSDIntersection 几何，或 T03/T04 stdout 明确无 eligible candidate，且 CaseID 为 `segment_*` 时，T10 生成空 relation/surface/audit handoff，记录 `segment_no_candidate_handoff=true` 与 `noop_reason=no_eligible_candidates_in_segment_dependency_closure`。
...
- 覆盖 Segment replay 中 T07 无 RCSDIntersection 几何、T03/T04 无 eligible candidate 时的空 handoff 输出和普通 Case 禁用条件。
