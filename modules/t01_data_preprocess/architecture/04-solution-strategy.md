...
本文件是 T01 的详细版需求 / 落地策略说明。它解释 SWSD Segment 如何从输入 `nodes / roads` 落到双向 Segment、单向补段和 Step6 聚合输出。稳定输入输出、入口和参数以 `INTERFACE_CONTRACT.md` 为准；accepted baseline 细节以 `architecture/06-accepted-baseline.md` 为准。
...
- 错：把 all related roads、其它 pair trunk 或预算超限结果纳入 Segment body。
