...
T07 will be a semantic-junction-level extraction of T02 Step1/Step2. It keeps the business meaning of `has_evd / is_anchor / anchor_reason`, uses representative `kind_2` as the gate, and removes every Segment input/output/summary path. This implementation round creates a module-local callable runner, focused tests, and the explicitly authorized innernet wrapper `scripts/t07_run_semantic_junction_anchor_innernet.sh`; no repo official CLI is added.
...
**Constraints**: No Segment processing; no repo official CLI; only the approved innernet wrapper script; no silent geometry/CRS fix; source/script files must stay below 100 KB per repository rule.
...
- **Development**: Implement T07 as module-local callable code under `src/rcsd_topo_poc/modules/t07_semantic_junction_anchor/`; add only the approved innernet wrapper script, and do not add repo CLI, tools, `run.py`, or `__main__.py`.
...
- Entry points: This plan forbids new repo CLI and allows only the approved innernet wrapper `scripts/t07_run_semantic_junction_anchor_innernet.sh`.
...
**Structure Decision**: Start with a compact module-local callable implementation in `runner.py`; split into smaller files in a later round if Step1 / Step2 scope expands. Project-level registration and entrypoint registry are updated in this authorized round.
