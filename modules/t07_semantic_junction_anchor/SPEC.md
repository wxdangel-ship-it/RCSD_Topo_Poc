# T07 模块规格：语义路口 1:1 锚定与可选 Relation 补锚

## 1. 模块定位

T07 是项目“路口 1:1 关系层”的已有路口面锚定模块。它迁移 T02 Step1 / Step2 的语义路口级锚定能力，把 SWSD 已有语义路口与 RCSD 已有路口面 / 语义节点对齐为可交给 T05 的关系证据。Step3 是可选兼容补锚能力，用于显式消费早期或外部方案产出的 `intersection_match_all` 兼容 relation 文件；它不是 T07 主链必须依赖的 T05 回灌，也不是用 T05 主表重新补锚。模块只处理代表 node 的 `has_evd / is_anchor / anchor_reason` 与 T07 版 relation evidence，不处理 Segment、不生成虚拟路口面。

## 2. 业务目标

- 基于 `DriveZone` 判断 SWSD 语义路口是否位于可处理道路面并具备后续锚定 evidence。
- 基于 `RCSDIntersection` 执行已有路口面 1:1 锚定，保留 T02 `fail1 / fail2` 语义。
- 可选基于早期或外部方案产出的 `intersection_match_all.geojson` 兼容 relation 文件，对部分 `is_anchor = no` 的已有路口特征做补锚。
...
- Step3：可选基于 Step2 strict surface 与外部兼容 relation 文件，对 `kind_2 in {4,8,16,128,2048}` 的候选补锚，其中 `128` 只走兼容 relation 成功补锚，`2048` 仅在 Step2 strict surface 已发布时走 surface 推导，否则仍可走兼容 relation 补锚。
...
| 上游 | DriveZone / RCSDIntersection | `DriveZone` 提供 Step1 道路面 evidence；`RCSDIntersection` 提供 Step2 existing surface 锚定面。 |
| 上游 | 早期 / 外部 `intersection_match_all` 兼容 relation 文件 | 可选提供 Step3 relation 补锚来源；T05 Phase2 产物只能作为显式兼容输入，不构成 T07 主业务上游。 |
...
| Step1 has_evd | 代表 node 类型在处理范围内时，要求组内所有 node 命中 `DriveZone` 或满足 `1.5m` 道路面 evidence 容差才写 `yes`；单独命中 `RCSDIntersection` 不得把道路面外语义路口抬成 `has_evd=yes`。 |
...
| Step3 兼容 relation 补锚 | 对候选 `is_anchor = no` 的 existing surface 路口，以及未被 Step2 strict surface 锚定的 `kind_2=128 / 2048` 专项 relation 路口，可选消费兼容 relation 文件中的成功关系。 |
| Step3 基数质检 | 对 1:N、N:1、重复 target 进行压制和回写。 |

## 8. 什么是对

- `has_evd / is_anchor / anchor_reason` 只写代表 node。
- `kind_2 = 2048` 在 Step2 只有满足 single-surface / single-SWSD / single-RCSD strict 条件时才建立 surface 成功关系；否则保持 `no / NULL`，由 T03/T04 或 Step3 可选兼容 relation 补锚继续处理。
...
- 用失败 relation 或不存在的 `base_id` 写锚定成功。
...
- Step3 relation 补锚需要持续与 `intersection_match_all` 兼容字段契约同步，并保持可选定位。
