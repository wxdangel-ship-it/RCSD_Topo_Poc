...
`p01_arm_build` 是 P01 POC 验证模块，当前覆盖 `P01-A1 / Arm 构建` 与 `P01-A2 / Arm 配准与 LogicalArmGroup 构建`。A1 在 SWSD / RCSD / F-RCSD 三套数据中分别构建当前语义路口 Arm；A2 读取 A1 run root，以 F-RCSD FinalArm 为目标承载体建立跨三源 LogicalArmGroup，并输出自动检查与人工目视审查产物。
...
本轮仍属于 P01-A1 修订，不新增正式 CLI，不实现 A2 / A3 / P01-B。
...
输入可为 SWSD `RoadNextRoad.json`、RCSD `RoadNextRoad.geojson` 或同类 JSON / GeoJSON 结构。F-RCSD RoadNextRoad 是业务结果输出，不作为 A1 输入。未提供 RoadNextRoad 时 A1 仍按既有流程运行，movement evidence 输出为空，trunk correction 输出 `not_evaluated_no_road_next_road_input`。
