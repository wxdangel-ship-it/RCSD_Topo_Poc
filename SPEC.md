...
| 路口关系 | T07 / T03 / T04 / T05 | 构建并发布 SWSD-RCSD 语义路口关系。T07 处理已有路口面和补锚，T03 处理交叉 / T 型，T04 处理分歧 / 合流 / 复杂路口，T05 统一发布 relation。 |
...
| 编排证据 | T10 | 组织 Case package、Case replay、full pipeline manifest、T06 funnel、feedback 和 visual check；不替代 T01-T09 算法。 |

T10 v1 Case runner 编排 `T01 -> T07 Step1/2 -> T03 -> T04 -> T05 -> T07 Step3 -> T06 -> T09`。T08 是独立前置预处理、质检和修复模块，不由 Case runner 调用；内网全量总控可把 T08 作为独立阶段串入全量链路。
...
4. T06 不能只按“有 1:1 relation”机械替换 Segment，必须处理 RCSD 端点、方向、道路切分、提前右转、内部短连接、surface 证据和 topology 连通等真实数据差异。
5. T09 必须在 F-RCSD 承载关系上恢复 SWSD 现场通行规则，并保留可追溯的 restriction、Laneinfo 和风险审计链。
