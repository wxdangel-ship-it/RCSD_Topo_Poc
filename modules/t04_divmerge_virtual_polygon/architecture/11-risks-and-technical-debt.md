...
- Anchor_2 official 39-case baseline 是当前唯一正式冻结基线：`39 case / accepted = 35 / rejected = 4`，rejected set 为 `607602562 / 760598 / 760936 / 857993`。
- Anchor_2 23-case 与 30-case 是 official 39-case 的历史子集，只从 official manifest 投影，不再作为独立 batch / visual fingerprint 基线。
- `857993 = rejected` 是人工验收确认后的正确业务结果，不作为待修成 `accepted` 的缺陷。
- `607602562 = rejected` 是当前 official 39-case 目视审计口径下的正确结果；不作为提高 accepted count 的静默修复目标。
...
- 若 acceptance 数字口径冲突，以当前 official 39-case gate `39 / 35 / 4` 为准；23-case `23 / 20 / 3` 与 30-case `30 / 26 / 4` 仅作为 official manifest 的历史子集投影，`7 / 1` 仅保留为 2026-04-22 selected-case legacy 口径。
...
| H-01 视觉基线漂移 | 2026-05-01 已定位：旧 23-case 中 8 个 `final_review.png` 指纹变化不是 pure renderer drift，而是 `surface_scenario / section_reference / case-level bridge / Step6 guard` 改造导致的 intentional geometry / visual baseline drift；用户后续已确认 39-case 目视通过 | PNG raw hash 对几何和 review 层过敏，容易把 intentional geometry drift 误报为业务失败 | 当前不再维护 PNG raw fingerprint hard gate；official 39-case gate 锁 PNG 存在性、render audit、summary / consistency 与人工目视确认 run root |
| 视觉黄金图守门 | Anchor_2 official 39-case 已生成 `final_review.png` 与 render audit | 当前能守住缺图和 SWSD 实体道路渲染缺失；仍不是像素级视觉语义 diff | 后续只在真实视觉需求扩大时再升级像素 / 图层级 diff |
