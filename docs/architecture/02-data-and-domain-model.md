...
| T12 quality hypothesis | SWSD 与原始 1V1 F-RCSD 在通行性上应等价。1V1/T05 mainNode 锚定只展开选中 `base_id` canonical group 的 raw subNode/alias；其它显式 grouped raw node保留但不递归扩组。随后在 identity endpoint 图上按 Road Direction 检查实际 carrier；锚定 alias 距离仅审计，非锚定 spatial portal 仍受范围/标准面门禁。该语义与 portal-constrained semantic carrier、标准路口 portal 和锚点可信度联合质检；semantic carrier 只排除 raw 假断裂，完成排除门禁后仍失败的记录可进入正式问题层，但任何质量结论都不得直接提升为修复规则。 |
