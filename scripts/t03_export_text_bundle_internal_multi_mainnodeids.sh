...
usage() {
  cat <<'EOF'
Usage:
  scripts/t03_export_text_bundle_internal_multi_mainnodeids.sh [options] <mainnodeid>...
...
OUT_ROOT="${OUT_ROOT:-$REPO_DIR/outputs/_work/t03_text_bundle_internal}"
RUN_ID="${RUN_ID:-t03_text_bundle_$(date +%Y%m%d_%H%M%S)}"
OUT_TXT="${OUT_TXT:-}"
...
OUT_TXT="${OUT_TXT:-$OUT_ROOT/$RUN_ID/t03_t04_ready_bundle.txt}"
...
"$PYTHON_BIN" -m rcsd_topo_poc t03-export-text-bundle \
...
if [[ "$DECODE_AFTER_EXPORT" == "1" ]]; then
  "$PYTHON_BIN" -m rcsd_topo_poc t03-decode-text-bundle \
    --bundle-txt "$OUT_TXT" \
    --out-dir "$DECODE_DIR"
