...
本文件是 T05 的架构设计 / 需求具体实现策略说明。模块需求见 `../SPEC.md`，稳定输入输出、业务规则和入口契约见 `../INTERFACE_CONTRACT.md`。

业务上，T05 是路口 1:1 关系层的收口与发布点：T07 解决已有路口面，T03 解决常规虚拟路口，T04 解决复杂虚拟路口，T05 负责把这些来源统一成一个可被 T06 消费的 SWSD-RCSD 语义路口关系主表。Phase 1 先把多来源 surface 统一成路口面索引，Phase 2 再基于 relation evidence 和 copy-on-write RCSD 处理结果发布唯一 `target_id -> base_id` 关系。
