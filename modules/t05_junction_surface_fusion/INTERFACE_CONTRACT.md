...
- 最终成功关系必须通过 cardinality QC：同一 `target_id` 不得挂接多个 `base_id`，同一 `base_id` 不得被多个 `target_id` 挂接；若发现 `1:N`、`N:1` 或重复 success target，写入 `relation_cardinality_errors.csv/json`，记录 `introduced_by_module / source_modules / source_case_ids / scenes / reasons`，并令 `summary.passed = false`。
