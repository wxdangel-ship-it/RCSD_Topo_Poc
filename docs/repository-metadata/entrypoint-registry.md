...
| `t03_pack_failed_cases_first50_innernet.sh` | `scripts/t03_pack_failed_cases_first50_innernet.sh` | repo 级 | T03 首批失败审计 Case 内网提取入口；使用已确认的 POC_QA SWSD/Patch Vector/1V1 FRCSD 路径，固定提取用户指定失败清单的前 50 个 mainnodeid，大小写不敏感预检 `snodeId/enodeId/mainNodeId` 等外部字段，输出自动分片文本 bundle、解包校验目录与传输归档 | `active` | 否 |
