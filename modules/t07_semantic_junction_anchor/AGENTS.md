...
- 本模块继承 T02 Step1 / Step2 的代表 node 写值与空间命中基础口径，但 Step2 对 `kind_2 = 64 / 128 / 2048` 采用 T07 专属分流规则；`kind_2 = 2048` 在 Step2 不建立 SWSD-RCSD surface 关系，Step3 可在 T05 成功 relation 可验证时补写 anchor；模块去除全部 Segment 处理。
...
- Step1 对处理范围内语义路口，必须组内所有 node 均落入、接触 `DriveZone ∪ RCSDIntersection`，或到该合并 evidence 面距离不超过 `1.5m` 才写 `has_evd = yes`；任一组内 node 既未命中也超过容差则写 `has_evd = no`。
...
- Step2 可启用 `RCSDNode` 可消费性门禁；命中的 `RCSDIntersection` 面内没有可用 RCSDNode 语义路口时，不得作为 `is_anchor = yes` 的依据。
- Step3 的 Step2 surface 1V1 推导只处理代表 node `kind_2 in {4, 8, 16}` 的语义路口；T05 relation 主表补充候选处理代表 node `kind_2 in {4, 8, 16, 128, 2048}`，其中 `128 / 2048` 只能在 T05 已发布成功 relation 且 RCSD base 存在时补写 anchor。
...
- 单元测试必须覆盖 Step1 allowed / disallowed `kind_2`、代表 node 写值、多节点组、singleton、`1.5m` evidence 面容差、Step2 `yes / no / fail1 / fail2 / NULL`、Step2 `RCSDNode` 可消费性门禁、`kind_2 = 64 / 128 / 2048` 专项基础规则写 `no`、`kind_2 = 2048` 不参与 Step2 `fail2` 但可通过 Step3 T05 relation 补锚、`kind_2 in {4, 8, 16, 64, 128}` 一面多 SWSD 语义路口时统一 `fail2` 覆盖、Step2 handoff surface/evidence 输出、Step3 候选识别 / relation 成功 / RCSD 存在性校验 / `intersection_match_t07.geojson` 与合并 evidence 输出、Step3 relation 1:N / N:1 基数质检、无 Segment 依赖。
