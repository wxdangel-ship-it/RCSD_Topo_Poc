...
| `modules/p01_arm_build/architecture/*` | P01 模块长期架构真相 | `source_of_truth` | P01 POC 验证模块的长期文档主表面；当前正式范围只覆盖 `P01-A / Arm 构建`，不包含 Arm 配准、Movement、禁行迁移或 P01-B |
| `modules/p01_arm_build/INTERFACE_CONTRACT.md` | P01 稳定契约面 | `source_of_truth` | 固化六类 Node/Road 输入、多 `junction-group`、语义路口组装、InitialArm/FinalArm/Trace/Decision/Issue 输出、review PNG/GPKG/summary/index 与当前无 repo 官方 CLI 的入口状态 |
| `modules/p01_arm_build/README.md` | P01 操作者入口 | `operator_guide` | 说明 P01-A 模块定位、非范围、当前模块内 callable runner 与主要文档 |
| `modules/p01_arm_build/AGENTS.md` | P01 durable guidance | `durable_guidance` | 只保留 P01-A 边界、禁止 Grade/几何右转反推/Movement/P01-B 以及不得新增正式入口的模块规则 |
...
| `specs/p01-arm-build/*` | P01-A 变更工件 | `active_change_artifact` | 记录 P01-A Arm 构建的需求规格、计划与任务拆解，不替代模块长期真相 |
