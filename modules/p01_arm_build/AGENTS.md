...
- P01-Final 以 F-RCSD Road 为承载体，基于 `Source + CRS 归一化 rounded exact geometry` 源 Road 映射、SWSD / RCSD SourceMovementPolicy、同源继承、跨源 primary source 与 RCSD -> SWSD fallback 生成 `frcsd_road_next_road.geojson`。
...
- 不通过几何形态反推右转专用道 / 渠化右转。
- 不通过空间接近替代 F-RCSD Source + CRS 归一化 rounded exact geometry 源 Road 映射。
