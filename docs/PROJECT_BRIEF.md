...
- `p01_arm_build` 当前作为 Active P01 成果模块进入治理；正式范围覆盖 P01-A1 Arm 构建、P01-A2 Arm 配准与 P01-Final F-RCSD RoadNextRoad 还原，输出 Trace / ThroughDecisionAudit / IssueReport / LogicalArmGroup / RawArmAlignment / ArmBuildFeedback / corrected_final_arms / frcsd_road_next_road.geojson / final audit / review PNG / review GPKG / summary / review index，不提供 repo 官方 CLI 或 `scripts/` 常驻命令
