...
| `p01_arm_build` | `modules/p01_arm_build` | `P01-A / Arm 构建` POC 验证模块；三套数据独立构建 Arm，输出 InitialArm / FinalArm / Trace / ThroughDecisionAudit / IssueReport / review PNG / review GPKG / summary / review index | 已建立标准 architecture 文档组、`INTERFACE_CONTRACT.md`、`README.md`、`AGENTS.md` | 模块内 callable runner 落地；不新增 repo CLI、`scripts/` 常驻脚本、模块 `run.py` 或 `__main__.py` | P01 用于区分 POC 验证模块，目录结构与 T0X 模块一致 |
