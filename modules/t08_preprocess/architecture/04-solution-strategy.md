...
2. 交叉路口质检：识别 `kind_2 = 4` 且 `in_degree = 2 / out_degree = 2` 的语义路口；忽略道路方向后四个外侧角度方向不输出错误；仅两条双向 road 或仅两个外侧角度方向且各方向一进一出时输出 `错误交叉路口_非交叉路口`；符合右侧 T 型特征输出 `错误交叉路口_T型路口`，其余输出 `错误交叉路口_非交叉路口`。
