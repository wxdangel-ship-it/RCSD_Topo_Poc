...
| `t10_run_innernet_full_pipeline.sh` | `scripts/t10_run_innernet_full_pipeline.sh` | repo 级 | T10 内网全量端到端总控脚本；默认串联 `T08 -> T01 -> T07 Step1/2 -> T03 -> T04 -> T05 -> T06 Step1/2 -> T06 Step3 -> T11 -> T09`，显式 `RUN_T12=1` 时在 T11 后、T09 前运行 T12，并支持通过 `T12_PROCESSING_CRS` 向 T12 显式透传混合输入的 projected metre CRS；T12 候选运行后可在同一 T10 run root 以新的 `T12_RUN_ID` 和显式复核 CSV 续跑并登记新的 reviewed 输出；所有实际运行阶段写入同一 run root 与 manifest，支持 finalize/resume/stage selection，默认流程业务效果不变 | `active` | 否 |
| `t10_run_frcsd_quality_pipeline.sh` | `scripts/t10_run_frcsd_quality_pipeline.sh` | repo 级 | T10 原始 1V1 F-RCSD 质量检查专用流水线；固定 `RUN_T08=0 / RUN_T12=1`，按 `T01 -> T07 -> T03 -> T04 -> T05 -> T06 -> T11 -> T12 -> T09` 复用通用 full runner，要求显式原始 1V1 F-RCSD Road/Node，混合 CRS 时接受显式 `T12_PROCESSING_CRS`，支持候选阶段后以新 `T12_RUN_ID / T12_REVIEW_DECISIONS` 续跑发布 confirmed，沿用 manifest、summary、resume 和失败退出，不复制模块算法 | `active` | 否 |
