...
| T12 | 对原始 1V1 F-RCSD 执行 audit-only 质量检查，验证 SWSD 通行性等价假设；1V1/T05 mainNode 锚定只展开选中 `base_id` canonical raw alias group，其它显式 grouped raw node不递归扩组；按当前方向 source outgoing / target incoming 进入 raw identity endpoint topology，锚定 alias 距离仅审计，非锚定 spatial fallback 不放宽。portal-constrained semantic carrier 与 T07 Road-surface portal carrier继续作为误报排除门禁；Road-surface 层要求唯一标准面和有向物理 Road。结合锚点可信度自动发布确认与排除层，人工复核仅作可选 QA 覆盖，不执行修复。 |
...
- F-RCSD QA：T06 Step3 结果继续由 T06 正式审计；外部 1V1 匹配生成的 F-RCSD 由 T12 检查 SWSD 等价可达性、road-node integrity、anchored canonical alias 到 raw Direction endpoint 的映射、canonical 候选图、raw verdict 图、portal-constrained semantic carrier、T07 Road-surface portal 和自动高置信发布；人工复核仅作可选 QA 覆盖。
