...
| `t08_tool2_road_preprocess.py` | `scripts/t08_tool2_road_preprocess.py` | repo 级 | T08 Tool2 Road GPKG 预处理，补充 `patch_id` 与原始 `kind`，删除 `kind` 同时具有 `0a` 与 `17` 道路类型属性的 Road 并输出 `event_road_0a_17_tool2.gpkg`，成果输出文件名以 `_tool2` 结尾 | `active` | 否 |
...
| `t08_tool6_nodes_type_qc.py` | `scripts/t08_tool6_nodes_type_qc.py` | repo 级 | T08 Tool6 Nodes 类型质检，基于语义路口入出度、连续分歧合流 T 型候选与交叉路口候选规则输出 `node_error_tool6.csv / node_error_tool6.gpkg / node_error_summary_tool6.json`，CSV 最后一列 `是否修复` 默认 `0`，不改写输入 Nodes/Roads | `active` | 否 |
