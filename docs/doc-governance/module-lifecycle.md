...
| `t07_semantic_junction_anchor` | `modules/t07_semantic_junction_anchor` | T07 Step1 / Step2 / Step3：T02 Step1 / Step2 的语义路口级重构与独立 Step3 relation 补锚；Step1 / Step2 消费 `nodes / DriveZone / RCSDIntersection`，Step3 消费 Step2 后 `nodes`、T05 `intersection_match_all.geojson` 与输入 `RCSDNode`，输出代表 node 的 `has_evd / is_anchor / anchor_reason`，不处理 Segment | `step1-step2-step3 active / callable module runners / innernet scripts registered / no public cli` |
| `t08_preprocess` | `modules/t08_preprocess` | T08 正式预处理模块；当前覆盖 Tool1 基础矢量格式转换、Tool2 Road GPKG 预处理、Tool3 Nodes 类型聚合、Tool4 路口类型修复、Tool5 复杂路口预处理与 Tool6 Nodes 类型质检，T08 成果输出文件名统一以 `_toolX` 结尾；Tool1 支持 SHP / GeoJSON 转 GPKG 与 GPKG 转 GeoJSON，输出写回输入目录并追加 `_tool1`；Tool2 补充 `patch_id / kind` 并删除 `kind` 具有 `17` 主辅路出入口属性的 Road 到事件输出；Tool3 补充 `kind_2 / grade_2`；Tool4 输出完整 Nodes/可选 Roads/audit Nodes，修复错误 T 型、一入一出分合流并可消费 Tool6 人工确认成果；Tool5 构建复杂分歧 / 合流路口并可处理错误 1 对多路口；Tool6 输出 `node_error_tool6.csv / node_error_tool6.gpkg` 作为人工质检输入，不改写输入 Nodes/Roads，均输出 `EPSG:3857` GPKG | `tool1-tool2-tool3-tool4-tool5-tool6 active / innernet scripts registered / tool4 consumes optional tool6 QC / tool6 QC only` |
...
- `t07_semantic_junction_anchor` 当前作为 T07 正式模块启动，范围覆盖语义路口级 `has_evd / is_anchor / anchor_reason` 与独立 Step3 relation 补锚，不处理 Segment；当前不新增 repo 官方 CLI，Step1 / Step2 内网执行通过 `scripts/t07_run_semantic_junction_anchor_innernet.sh` 包装模块内 runner，Step3 通过 `scripts/t07_run_step3_intersection_match_innernet.sh` 独立执行。
- `t08_preprocess` 当前作为 T08 正式预处理模块启动；工具形态属于项目正式组成部分，当前内网执行通过 `scripts/t08_tool1_vector_convert.py`、`scripts/t08_tool2_road_preprocess.py`、`scripts/t08_tool3_nodes_type_aggregation.py`、`scripts/t08_tool4_junction_type_repair.py`、`scripts/t08_tool5_complex_junction_preprocess.py` 与 `scripts/t08_tool6_nodes_type_qc.py`；T08 成果输出文件名统一以 `_toolX` 结尾，Tool4 按契约修复路口类型并可消费 Tool6 人工确认成果，Tool6 只输出人工质检候选，不自动修复。
