...
仓库不提供 P01 repo 官方 CLI、Makefile 目标、模块 `__main__.py` 或模块 `run.py`。当前稳定调用面是模块内 callable runner；内网单 Case 端到端执行另提供 repo 级正式脚本 `scripts/p01_run_innernet_case.sh`。
...
`scripts/p01_run_innernet_case.sh` 只消费内网全量 SH 输入路径与单个 `JUNCTION_GROUP`，不执行文本包打包 / 解包。脚本输出以 `<OUT_ROOT>/<CASE_ID>/` 为 Case 主目录，A1 / P01-Final 与 A2 原始 run root 仅保留在 `_raw/` 下用于追溯。

文本证据包与 dev helper 仅用于取证、复现和开发验收，不登记为正式执行入口。
...
- `review_layers.gpkg`
- `p01_arm_movement_turn_audit.png`（仅 F-RCSD；SWSD / RCSD 保留 JSON / GPKG 审计，不输出该 PNG）
...
以下低价值 PNG 不作为正式业务输出：dataset 级 `p01_arm_review.png`、A1 三源 `p01_arm_compare.png`、F-RCSD `frcsd_road_next_road_review.png`。对应的结构化 JSON / GPKG 仍保留，用于机器审计与问题追溯。
...
- `frcsd_pass_capability_audit.png`
...
内网正式 Case 级脚本为 `scripts/p01_run_innernet_case.sh`。它直接消费全量 SH 数据：

- 默认 `SWSD_NODES=/mnt/d/TestData/SH/SWSD/node.gpkg`
- 默认 `SWSD_ROADS=/mnt/d/TestData/SH/SWSD/road.gpkg`
- 默认 `RCSD_NODES=/mnt/d/TestData/SH/RCSD/RCSDNode.gpkg`
- 默认 `RCSD_ROADS=/mnt/d/TestData/SH/RCSD/RCSDRoad.gpkg`
- 默认 `FRCSD_NODES=/mnt/d/TestData/SH/FRCSD/RCSDNode.gpkg`
- 默认 `FRCSD_ROADS=/mnt/d/TestData/SH/FRCSD/RCSDRoad.gpkg`
- 默认 `SWSD_ROAD_NEXT_ROAD=/mnt/d/TestData/SH/SWSD/road_topo.json`
- 默认 `RCSD_ROAD_NEXT_ROAD=/mnt/d/TestData/SH/RCSD/RCSDRoadNextRoad.geojson`

该脚本要求 `JUNCTION_GROUP=<SWSD_ID>,<RCSD_ID>,<FRCSD_ID>`，或 `CASE_ID + RCSD_JUNCTION_ID + FRCSD_JUNCTION_ID` 三个环境变量。`CASE_SCOPE_BFS_DEPTH` 默认 `8`，`RIGHT_TURN_FORMWAY_VALUE` 默认 `128`，`RUN_A2=0` 可跳过 A2。正式输出落在 `<OUT_ROOT>/<CASE_ID>/FRCSD/frcsd_road_next_road.geojson` 与同目录 audit / issue / decision JSON；阶段性 A1 / A2 运行目录仅在 `<OUT_ROOT>/<CASE_ID>/_raw/` 下保留。
