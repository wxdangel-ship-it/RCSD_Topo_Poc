...
P01 的最终承载体是 F-RCSD Road。模块输出 F-RCSD 道路在路口处允许通行到哪些 F-RCSD 退出道路，并保留完整 source map、source policy、audit 与 issue report。
...
- `InitialArm / FinalArm / corrected_final_arms`：member roads、seed/connector roads、inbound/outbound/bidirectional roads、terminal、build status、risk flags、advance-left fields、trunk fields、advance-right relation refs。
...
- RCSD -> SWSD fallback 只允许 `from_road.Source = 1`、`to_road.Source = 2`、RCSD 无对应 to_arm、SWSD 存在对应 to_arm，且进入 Arm road count 一致。
- 平行支路数量不一致进入 `data_error / manual_review_required`；source 有平行支路而 F-RCSD 没有时写入 audit。
...
- review GPKG / PNG 支持审查 Arm、trunk、corrected trunk、special turns、ArmMovement、RoadMovementEvidence、generated RoadNextRoad、source map 与 issues。
