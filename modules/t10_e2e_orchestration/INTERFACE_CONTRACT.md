...
- 支持文本 bundle 自动分片与解包。
- Case 证据包 v1 只纳入全量外部输入清单，排除模块间中间产物。
...
- `radius_m`：Case 范围半径，单位米，当前声明 CRS 为 `EPSG:3857`。
- `include_files`：是否把外部输入文件复制到 package 目录；默认可只生成 manifest。
...
- `manifest_scope_declared` 状态，避免误表述为空间切片已完成。
...
当前无 repo 官方 CLI、root `scripts/` 或模块独立启动入口。
...
新增稳定入口必须另行授权并同步 `docs/repository-metadata/entrypoint-registry.md`。
