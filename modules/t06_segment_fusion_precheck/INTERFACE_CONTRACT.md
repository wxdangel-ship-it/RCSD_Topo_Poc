...
- Step2 retained RCSDRoad 覆盖审计复用 `min_buffer_road_overlap_ratio`：候选选择可保留 `intersects + overlap length / endpoint` 的宽松入口，但最终 retained Road 若覆盖率低于该比例，不能进入 replaceable。retained RCSD 跑出 SWSD 50m buffer 仍是硬拒绝；`swsd_geometry_not_covered_by_retained_rcsd` 表示 SWSD 反向 coverage gap，在正式 relation 已消费、required graph、candidate graph、方向性均通过，且 retained RCSD corridor 完全位于 SWSD 50m buffer 内时，必须作为 `swsd_buffer_corridor_controlled_release` 发布 replacement plan，并追加 `swsd_geometry_not_covered_by_retained_rcsd / manual_review_required` 风险标记。该受控放行不依赖 failure business audit 的高置信评分，不接受 `rcsd_outside_swsd_buffer_length_m / ratio` 非零。50m 通过后还要使用默认 15m 窄通道复核 Segment 级几何连续性，阈值同为 `10% / 20m`。
...
8. `junc_nodes` 执行 optional 内部通过 + 侧向阻断；孤立 optional junc 可被剪除并输出 dropped / lost attach 审计，retained graph 中出现非 pair leaf endpoint 时必须拒绝并输出 `unexpected_endpoint_node_ids`。
9. retained graph 中不得存在 pair required corridor 内部解释节点以外的额外 T05 mapped semantic nodes；出现时必须拒绝并输出 `unexpected_mapped_semantic_node_ids`。
