...
本轮已经完成日志根因分析、隔离工作树实现、`1885118` 优先门禁及六用例回归。局部业务等价已经成立；六例每例 Step3 wall 均低于当前 `f870a83` 基线，Step3 聚合耗时下降 `52.06%`，已越过本地 50% 参考线；以未改 Step1/2 基线合成的 T06 总计下降 `41.78%`，swap 均为 0。其中 `1885118` Step3 已下降 `56.66%`。5 个 Case 的 peak RSS 下降，最小 Case `74155468` 增加 `536 KB`（`+0.29%`），最大候选 peak RSS 仍只有 `499704 KB`。但尚未取得同环境、同全量数据的内网候选结果，因此“全量 T06 耗时降至当前 50%”和全量 peak RSS 门禁仍为待确认，目标不得标记完成。
...
- ownership：STRtree 使用精确 `dwithin` 候选查询；Segment buffer 仅在第二次命中后缓存，LRU 上限 `512`。
...
六例均 swap=0、exit=0，最大候选 peak RSS 为 `499704 KB`；5 例下降，`74155468` 增加 `536 KB`（`+0.29%`），仍远低于正式全量上限。正式判定仍需确认全量 peak RSS `<=9365992 KB`、swap=0、无 OOM/Killed。

## 7. 自动化验证

- T06 最终全量测试：`485 passed in 46.05s`（JUnit：0 failure、0 error、0 skip；外层 `48.61s`，peak RSS `173340 KB`）。其中新增内网验收门禁定向测试 `6 passed`，覆盖 GNU time 解析、正式阈值、业务根路径归一化、流式断点复用、拓扑审计/兼容业务指标防漂移和 250KiB bundle 导出后解包校验。
