...
| `t08_preprocess` | `modules/t08_preprocess` | Tool1 基础矢量格式转换，支持 SHP / GeoJSON 转 GPKG 与 GPKG 转 GeoJSON，输出均写回输入目录下同名文件，采用流式转换并输出进度；Tool2 Road GPKG 预处理，补充 `patch_id / kind` 并输出阶段性能；Tool3 Nodes 类型聚合，补充 `kind_2 / grade_2` 并处理环岛、复杂分歧 / 合流 mainnode，输出阶段性能；输出 `EPSG:3857` GPKG | 已建立 `INTERFACE_CONTRACT.md`、`README.md`、`AGENTS.md` 与 architecture 文档 | 提供模块内 callable runner 与 `scripts/t08_tool1_vector_convert.py`、`scripts/t08_tool2_road_preprocess.py`、`scripts/t08_tool3_nodes_type_aggregation.py` 内网运行脚本 | 工具形态属于项目正式组成部分；Tool3 以外的 Node 预处理待后续定义 |
