...
- 2026-07-28 T12-only innernet resume 轮次只修改既有正式 profile 入口和专项测试：`t10_run_frcsd_quality_pipeline.sh` 为 `3474` bytes，`test_t10_frcsd_quality_pipeline.py` 为 `5337` bytes，均低于 60KiB/100KB 阈值。未新增入口、依赖或业务算法；既有入口新增正式 `RESUME_RUN_ROOT + RUN_STAGES=t12` 调用合同，自动生成新的 T12 子运行 ID，复用 full runner 的 manifest、失败退出与不覆盖约束；`entrypoint-registry.md`、T12 README 和接口契约已同步。
