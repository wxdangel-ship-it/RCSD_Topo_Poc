...
def _segment(segment_id: str, x0: float) -> dict:
    return {
        "properties": {"id": segment_id},
        "geometry": LineString([(x0, 0), (x0 + 10, 0)]),
    }
...
def test_unreplaced_rcsd_attribution_uses_formal_funnel_priority(tmp_path: Path) -> None:
    ...
    _write(
        ...
        [_segment_row("s5"), _segment_row("s4"), _segment_row("s3")],
    )
    _write(
    ...
    assert summary["unreplaced_rcsd_road_count"] == 5
    assert [item["value"] for item in summary["by_attribution_class"]] == [
