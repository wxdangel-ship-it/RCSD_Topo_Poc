...
- 端到端 case 分析由 T10 以 SWSD 语义路口 ID 与半径组织 Case 证据包；v1 支持 manifest-only、空间切片 Case 包、Case 级 replay、T06 数据漏斗和上游反馈包。
...
| 全量编排 manifest | T10 内网全量总控记录阶段顺序、显式输入输出、日志路径和最终 handoff；`t11` 固定位于 `t06_step3` 与 `t09` 之间，供全量审计优先消费。 |
