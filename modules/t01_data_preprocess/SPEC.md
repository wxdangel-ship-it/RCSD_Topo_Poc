...
- 在 Step5C refreshed 结果之后执行单向补段，补齐仍未构段的单向 road 与受控 dead-end / single-road fallback。
...
- dead-end leaf、final single-road fallback 与 final side-attachment merge。
...
| Step2 | 对候选 pair 做 validation、trunk、segment body 与 rejected 判定，输出 pair-specific road body。 |
| Step3 | 基于 Step2 结果刷新 Nodes/Roads 当前语义。 |
| Step4 / Step5A / Step5B / Step5C | 在 residual graph 上逐轮扩展双向 Segment，每轮后立即 refresh。 |
| 单向补段 | 在 Step5C refreshed 结果上补齐单向 road、dead-end leaf、single-road fallback 与 side-attachment merge。 |
...
- `architecture/04-solution-strategy.md` 需要持续保持详细需求说明职责，避免再次退化为阶段名清单。
