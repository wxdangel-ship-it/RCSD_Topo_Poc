...
| `t06_step3_unreplaced_rcsd_attribution.*` | 未替换 RCSDRoad 反向归因审计。`attribution_*` 保留 `5 > 4 > 3 > 2 > 1 > 6` 漏斗粗口径；`final_attribution_*` 先使用 replacement unit / plan 的 road-level 强证据，再按几何主 Segment 判定最终六类，避免邻近高漏斗 Segment 抢占归因。`ppt_attribution_*` 将最终六类映射到三类汇报口径：`2/4/5` 为 Segment 下面 RCSD 质量导致无法替换，`3` 为 Segment Relation 不满足要求，`1` 为 RCSD 不在 Segment 范围内；mixed 部分覆盖不新增归因类型，仅保留低置信与人工审计标记。 |
