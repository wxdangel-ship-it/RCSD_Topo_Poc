...
- Step2：基于 relation 与 buffer-based 策略构建 RCSDSegment 候选，并在特殊路口组局部替换门控后输出最终 `replaceable` 集合；对单 Segment 失败但具备成组替换可能的路径走廊输出 group replacement 审计证据，并统一发布 replacement plan 与 problem registry。
...
- 对 `junc_nodes` 启用 `kind_2 in {1,4096,8192}` 的 Step1 eligibility 豁免：命中 junc node 不参与 Step1 `has_evd / is_anchor` 判定；进入 Step2 后，未被明确 detached / exempt 的 junc 仍作为 required junction relation / topology 对象，该豁免不适用于 `pair_nodes`。对 `sgrade` 属于 `0-0* / 0-1*` 的高等级 Segment，`has_evd=yes` 且 `is_anchor` 明确非可用的 `pair_nodes.kind_2=2048`、`junc_nodes.kind_2 in {16,2048}` 可进入 Step2 probe 审查；对 `sgrade=0-2双` 且两个 `pair_nodes.kind_2` 均为 `2048` 的虚拟 T 型 pair，也可仅放行 pair 主通道进入 Step2 probe。对 `sgrade=0-*单` 且失败节点属于 `pair_nodes.kind_2=4` 的拆分边界，若 `has_evd=yes / is_anchor=fail1`，或 `has_evd` 明确非 yes 且 `is_anchor` 为空、`no` 或 `fail1`，Step1 可放行到 Step2 probe。上述规则不改变 anchor 事实、不回写 T05 relation、不改变 `junc_kind2_exempt_nodes`；`0-2双` 虚拟 T 扩展不适用于 `0-2单`、混合 `kind_2` pair 或 junc 脱挂。高等级 Segment 中若非特殊路口 junc-only 节点拖垮 `has_evd / is_anchor`，Step1 可将其从 final fusion unit 的 `junc_nodes / semantic_node_set` 中脱挂，写入 `detached_junc_nodes / detached_junc_reasons` 后交给 Step2 只审 pair-to-pair 主通道；该脱挂不适用于 `pair_nodes`，也不适用于 `kind_2 in {64,128}` 特殊语义路口。
- 将 `is_anchor = fail4_fallback` 视为可融合 anchor。
- 消费 T05 Phase 2 `intersection_match_all.geojson`、`rcsdroad_out.gpkg`、`rcsdnode_out.gpkg`。
- 基于 SWSD Segment 50m buffer、RCSDRoad `intersects + 阈值`、RCSDNode `covers/within` 生成 buffer-based RCSDSegment 审查成果，作为 Step2 唯一正式构建策略；RCSD retained corridor 允许局部跑出 50m buffer，但 pair-to-pair 主通路必须与 SWSD Segment buffer 有有效交集。
...
- `swsd_directionality=single` 已选主 corridor 后，若候选 RCSDRoad 与已选 corridor road 具有同一 canonical required-pair 端点对、方向兼容，且仍满足 buffer / visual corridor 约束，可作为同语义路口 sibling subnode parallel edge 保留；该规则只补已选端点对的并行 sibling edge，不允许新增任意 required-to-required shortcut。
- retained RCSDSegment 中的每条 RCSDRoad 必须与 SWSD Segment buffer 有有效交集；若某条完整 RCSDRoad 完全不经过 buffer，必须以 `retained_road_buffer_overlap_insufficient` 拒绝。retained RCSD 不要求完全位于 SWSD 50m buffer 内；retained RCSD 与 SWSD 的双向 50m buffer 覆盖差异、reverse coverage gap 和局部跑出 buffer 进入 `manual_review_required` 风险审计。50m 覆盖通过后，仍必须执行默认 15m 窄通道视觉连续性复核；若 SWSD 主线未被 retained RCSD 窄 buffer 覆盖或 retained RCSD 已明显脱离 SWSD 窄 buffer，任一 mismatch 比例超过 `10%` 或长度超过 `20m` 时写入风险审计，不单独作为 Step2 硬拒绝。
- 不再执行旧 pair-to-pair BFS 路径搜索、主轴 / 粗长度趋势或唯一性筛选；buffer 候选连通分量必须先收缩为覆盖 pair nodes + required junction nodes 的可解释 corridor 子图；`swsd_directionality=single` 时，source/target 必须由 SWSDRoad `snodeid / enodeid / direction` 推导，并据此构建一条覆盖 pair nodes + required junction nodes 的 RCSD 有向 corridor；`swsd_directionality=dual` 时执行 RCSD retained graph 双向可达硬审计。
...
- 对 `swsd_sgrade` 属于 `0-0* / 0-1*` 的高等级 Segment，若 T05 原始 pair relation 已完整且 50m buffer 失败可由全图拓扑诊断解释为裁剪窗口不足，Step2 可在不回写 pair anchor 的前提下执行受限重审：单向 Segment 不再整体放大候选 buffer，而是按 SWSDRoad 推导出的 RCSD pair 方向在全 RCSDRoad 有向图中联通两个 pair 路口；该 path 必须经过 50m SWSD Segment buffer 内的 RCSD core，且 path / SWSD 长度比例、首尾离开 50m core 的纵向长度、75m/100m 几何参考覆盖均通过安全门槛，才可作为 `single_graph_first_longitudinal_retry` 进入 replaceable。双向 Segment 仍要求全图 required nodes 连通且全图双向可达；若 50m buffer-only probe 给出非人工复核的高置信候选 pair 集合，且失败主因为 `directionality_mismatch_fixable + rcsd_not_bidirectional_for_swsd_dual`，Step2 可遍历候选 pair 并在当前 Segment 内构造 effective relation 后执行 `75m / 100m / 125m` adaptive buffer 重审，仍失败时可执行 `dual_graph_first_bidirectional_retry`：只有恰好一个候选 pair 通过正式双向硬审计时才可替换；正反两个 RCSD 有向 path 均必须存在、均经过 50m core、均满足 path / SWSD 长度比例，并保持 required junction 相对拓扑；否则仅复用 T05 原始 pair relation 做几何覆盖失败、`required_semantic_nodes_not_connected_in_buffer` 或 `rcsd_not_bidirectional_for_swsd_dual` 的受限重审。重审通过仍必须满足方向、叶子端点、required junction 相对拓扑、有效 buffer 穿行与特殊组硬审计；通过后在 candidates / replaceable / failure business audit 中记录 `adaptive_buffer_status / adaptive_buffer_distance_m / adaptive_buffer_source_reason`。
- 对 final `nodes.gpkg.kind_2=64` 的环岛路口与 `kind_2=128` 的复杂路口执行特殊组局部替换门控：按 `pair_nodes + junc_nodes` 包含该语义路口识别关联 Segment；组内已通过硬审计的关联 Segment 允许进入 Step2 replaceable，未通过的关联 Segment 保留 SWSD carrier。只有组内全部关联 Segment 均通过可替换判定时，Step2 才发布特殊组内部 RCSD Road/Node；局部环岛不得引入 RCSD 环岛内部端点间 Road，必须保留 SWSD 环岛内部 Road 和 nodes。局部复杂路口中，RCSD 可替换的连通 Road 由标准 Segment 或 path-corridor group action 整体替换，不可替换 SWSD road 继续保留，端点通过 T05 语义路口组表达同一路口。
...
- `kind_2 in {1,4096,8192}` 只对 `junc_nodes` 的 Step1 eligibility 生效：命中 junc node 从 Step1 `has_evd / is_anchor` 检查集合中移除，但仍保留在 `junc_nodes / semantic_node_set` 输出中；`pair_nodes` 命中这些 `kind_2` 也不豁免。对 `sgrade` 属于 `0-0* / 0-1*` 的高等级 Segment，`pair_nodes.kind_2=2048` 与 `junc_nodes.kind_2 in {16,2048}` 在 `has_evd=yes` 且 `is_anchor` 明确不可用时可作为 Step2 probe 放行节点；对 `sgrade=0-2双` 且两个 `pair_nodes.kind_2` 均为 `2048` 的虚拟 T 型 pair，仅 pair 主通道可同样进入 Step2 probe；对 `sgrade=0-*单` 的拆分边界，仅 `pair_nodes.kind_2=4` 的 `fail1` 多 RCSD 场景或明确 non-yes EVD 且 anchor 为空、`no`、`fail1` 的场景可进入 Step2 probe。该放行仍不是 anchor 豁免，Step2 中未被 detached / exempt 的 `junc_nodes` 统一按 required junction 审计处理，pair 或 required junction relation 缺失必须经 buffer-only probe 和正式硬审计重试证明后才可替换。
- `intersection_match_all.geojson` 中只有 `status = 0` 且 `base_id > 0` 的 relation 可用。
- `base_id` 必须是 RCSD 语义路口主 node id。
- Step2 构建 buffer candidate graph 时，必须先按 `rcsdnode_path` 的 `mainnodeid / subnodeid` 做语义节点归一化：`id` 若有有效 `mainnodeid` 则归一到 `mainnodeid`，`subnodeid` 列表中的物理节点也归一到所属 `mainnodeid`；pair required nodes、required / exempt junc nodes 与 RCSDRoad `snodeid / enodeid` 必须使用同一 canonical key 判定连通与裁剪。
...
- RCSDRoad `direction` 用于 retained graph 有向可达硬审计：`swsd_directionality=dual` 时 pair 两端必须正反向均可达；`swsd_directionality=single` 时必须存在一条按 SWSDRoad 推导方向、覆盖 pair nodes + required junction nodes 的有向 corridor；component coverage、seed pruning、可解释 corridor 子图与 leaf endpoint 仍按 canonical 无向图执行。
- 未被 detached / exempt 的 `junc_nodes` 在 RCSD 抽取中是 required junction relation / topology 对象；retained RCSD graph 的叶子端点只能是 `pair_nodes` 对应的 RCSD semantic nodes。
- buffer-based RCSDSegment 审查中，required semantic nodes 来自 `pair_nodes` relation 和 required `junc_nodes` relation；`junc_kind2_exempt_nodes` 若有有效 relation，作为 exempt audit nodes 保留。required junc relation 缺失、无效或在 retained corridor 中拓扑缺失时必须输出 `failed_junc_nodes / dropped_junc_nodes / dropped_junc_relation_nodes / lost_attach_road_ids / promoted_attach_road_ids / blocked_attach_road_ids / attach_promotion_status / attach_promotion_reason / isolated_attach_loss_count / junc_attach_loss_reason`。
- 孤立 exempt junc 被剪除后，Step2 可在 final replaceable 集合内执行全局无冲突 promotion：`lost_attach_road_ids` 中的 RCSDRoad 若未被其它 replaceable Segment 的主通道 `rcsd_road_ids` 占用，且未被多个 replaceable Segment 同时申请，可追加到当前 Segment 的最终 `rcsd_road_ids`；主通道审计字段 `retained_rcsd_road_ids` 不随 promotion 改写。
- 额外 T05 mapped semantic nodes 与 exempt junc 必须按 seed-based pruning 判定为 `inner_nodes / out_nodes`；目标 Segment required junction 节点必须保留并参与相对拓扑审计；RCSD 自身未锚定节点允许作为通路节点；已锚定到其它 SWSD 语义路口的额外 mapped semantic node 只有在破坏目标 Segment 相对拓扑、形成错误跨 Segment 消费或构成完全绕开 SWSD buffer 的 pair-to-pair 通路时才硬拒绝。
...
- 对单向 required-pair 主通道，若候选中存在与已选 corridor road 同 canonical 端点对的 sibling subnode parallel edge，且该 edge 支持同一通行方向并通过 buffer / visual corridor 约束，Step2 可一并保留；若只是另一条 required-to-required shortcut、方向不兼容或脱离参考走廊，仍必须拒绝补入。
- 当两个 `ready/replace` plan 在相同端点区间形成并行通道时，Step2 的唯一归属优先级固定为 `正式锚定关系 > required junction 有序相对位置 > 几何距离/视觉偏差`。当前 Segment 的 `rcsd_road_ids` 必须自身包含按 `rcsd_pair_nodes + rcsd_junc_nodes` 顺序连通的有向通道；只有锚定覆盖和相对顺序均无法区分时，才允许用距离或视觉偏差择优。释放通道后的另一 plan 仍须以自身 Road 保持 pair、全部非 detached/exempt required junction、方向和覆盖成立。该策略不得生成 relation、释放 rejected Segment，或用相邻 plan 的 Road 代替当前 Segment 的锚点覆盖。
- Step2 retained RCSDRoad 覆盖审计复用 `min_buffer_road_overlap_ratio` 作为有效 buffer 穿行检查：候选选择可保留 `intersects + overlap length / endpoint` 的宽松入口，但最终 retained Road 若完全不经过 SWSD Segment buffer，不能进入 replaceable。retained RCSD 不要求完全位于 SWSD 50m buffer 内；`swsd_geometry_not_covered_by_retained_rcsd` 与 `retained_geometry_outside_swsd_buffer_scope` 表示人工后审风险，在正式 relation 已消费、required graph、candidate graph、方向性均通过，且不存在 pair-to-pair 完全绕开 SWSD buffer 的连续通路时，必须作为风险发布 replacement plan，并追加对应 coverage issue 与 `manual_review_required` 风险标记。50m 通过后还要使用默认 15m 窄通道复核 Segment 级几何连续性，阈值同为 `10% / 20m`。
...
- Step2 特殊组局部替换门控只使用已正式定义的 `kind_2=64/128`：`64` 表示环岛路口，`128` 表示复杂路口。关联 Segment 范围为 Step2 输入融合单元中 `pair_nodes + junc_nodes` 包含该语义路口的所有 Segment；切片证据包按切片内可见 Step2 输入执行并通过 `t06_special_junction_group_audit.*` 暴露组完整性与 `passed / partial / blocked` 状态。
- 特殊组门控的“组内已覆盖 Segment”由 Step2 当前轮正式可执行结果构成：普通 `replaceable` Segment 与已通过 `group_probe_status=passed`、`group_probe_repair_owner=T06_path_corridor_group_replacement` 的 path-corridor group 覆盖 Segment 都可满足组完整性；但 path-corridor group 覆盖 Segment 不进入普通 `replaceable` 白名单，仍必须由 Step2 replacement plan 以 `execution_scope=path_corridor_group` 发布后供 Step3 执行。组内只有部分覆盖时，特殊组审计输出 `gate_status=partial`，不得移除已通过硬审计的普通 `replaceable` Segment。
- 特殊组门控全通过时，`t06_special_junction_group_audit.*` 必须输出该 SWSD 特殊路口映射到 RCSD 后的 `rcsd_junction_id`、RCSD 语义组内 Node，以及端点同归一到该 RCSD 语义组的内部 RCSDRoad，用于 Step3 统一替换审计。`gate_status=partial / blocked` 时这些内部 RCSD Road/Node 只能作为审计上下文，不得进入 `special_junction_group_internal` replacement plan。
...
- Step3 不重新判定特殊路口组或 path-corridor group 是否可替换；Step3 只执行 Step2 replacement plan 中 `execution_scope=special_junction_group_internal` 与 `execution_scope=path_corridor_group` 的 ready action，并将 `path_corridor_group` 作为组级原子替换 action 加入 F-RCSD。组级原子性仅约束整组执行/回退，不构成一条 RCSD Road 对多个 Segment 的所有权。旧运行缺少 replacement plan 时，可兼容读取同目录 `t06_special_junction_group_audit.*` 与 `t06_segment_group_replacement_audit.*`，但 summary 必须记录 `replacement_plan_source=legacy_step2_artifacts`。
...
- Step3 surface-aware visual conflict release 只能产出纯 `relation_status=replaced` 的 RCSD 替换；若 release 后 relation 仍为 `replaced+retained_swsd` 或 `retained_swsd`，必须将该 visual conflict plan 回滚为 `hold`，避免由 `visual_consistency_road_conflict_with_primary_replacement_plan` 产生新的混合挂接。
...
- Step3 relation 中的 `frcsd_road_ids` 是该 Segment 在最终 F-RCSD 中实际可消费的 carrier 清单。`source=1` RCSD Road 只允许进入其唯一 owner Segment 的 `frcsd_road_ids / owned_frcsd_road_ids`；特殊路口内部 Road 与 multi-Segment connectivity Road 无 Segment owner，分别通过 `related_special_junction_internal_road_ids` 与 `related_connectivity_road_ids / connectivity_group_ids` 提供关联上下文。`retained_swsd / replaced+retained_swsd` 时可指向 `source=2` 保留 SWSD carrier，但必须通过 `frcsd_road_source_values / source_mix`、状态和风险标记暴露来源。若 surface topology materialize 出 split road，所有非 `retained_swsd` relation 都必须把 pre-split road id 同步为最终存在的 split road id。
...
- 提右来源判定只统计两端相邻的非 `advance_right` 普通 Segment：两侧均 replaced 且 5m 内存在几何一致 RCSD 提右时不得保留 SWSD 提右；一侧 replaced、一侧 retained/failed 时保留 SWSD 提右，并只在 replaced 侧对该侧正式 selected RCSD Road执行 split/reuse attachment。T01 独立提右 Segment 的 `segmentid` 不得把 mixed attachment 自动收紧为 1m；仍须满足候选唯一、方向可解释和契约化最大间距。显式 split/reuse attachment 的 mainnode 映射优先于 retained identity/peer relation 同步。
...
`t06_step1_summary.json` 在读取 T05 audit 时必须记录：

- `input_paths.intersection_match_path`
- `input_paths.manual_relation_anchor_override_audit_path`
- `manual_relation_anchor_override_source`
- `manual_relation_anchor_override_candidate_node_count`
- `manual_relation_anchor_override_node_count`
...
- `t06_rcsd_segment_candidates.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
- `t06_rcsd_segment_replaceable.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
- `t06_rcsd_segment_rejected.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
- `t06_rcsd_buffer_segments.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
- `t06_rcsd_buffer_segment_rejected.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
- `t06_rcsd_buffer_only_probe.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
- `t06_rcsd_repair_candidates.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
- `t06_rcsd_segment_failure_business_audit.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
- `t06_special_junction_group_audit.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
- `t06_segment_group_replacement_audit.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
- `t06_segment_replacement_plan.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
- `t06_segment_replacement_problem_registry.gpkg/csv`（`json` 在 `write_json_outputs=true` 时输出）
...
- `t06_step2_slow_groups.jsonl`（仅 `progress=True`）
...
特殊路口组 gate 可读取同一轮 pre-gate `t06_segment_group_replacement_audit` 中已通过的 path-corridor group 覆盖 Segment 作为组完整性证据；该证据只用于判断 `passed / partial / blocked` 和是否发布特殊组内部 RCSD 上下文，不改变 `t06_rcsd_segment_replaceable` 的普通 Segment 边界。

`t06_segment_replacement_plan` 是 Step2 到 Step3 的正式执行边界。它把普通 replaceable Segment、全通过特殊路口组内部 RCSD 实体、path-corridor group replacement 统一表达为 `plan_status=ready` 的 action；Step3 只能执行 plan 中的 RCSDRoad / RCSDNode / Segment group，不得重新补充 RCSD path 或重判可替换性。特殊路口组 `partial` 只允许其标准 Segment / path-corridor group action 进入 plan，不发布 `special_junction_group_internal`。
...
Step2 运行诊断 sidecar 只用于性能和卡点定位，不参与替换率、replacement plan、problem registry 或 Step3 handoff。`t06_step2_heartbeat.json` 记录当前 PID、当前 Segment、group_audit 当前组、后处理阶段或当前正在写出的文件；`t06_step2_progress.jsonl` 记录阶段切换、每 1000 个 Segment 的进度、group_audit 稀疏组级进度，以及每个 Step2 输出的 `start/end/skipped`；`t06_step2_slow_units.jsonl` 记录耗时超过阈值的 Segment；`t06_step2_slow_groups.jsonl` 记录耗时超过阈值的 group_audit 组；Linux/WSL 环境可用 `kill -USR1 <PID>` 将当前 Python 栈写入 `t06_step2_stackdump.log`，用于不中断长耗时进程定位 CPU-bound 热点。
...
- `t06_frcsd_road.gpkg/csv`
- `t06_frcsd_node.gpkg/csv`
- `t06_step3_replacement_units.gpkg/csv`
- `t06_step3_junction_rebuild_audit.gpkg/csv`
- `t06_step3_removed_swsd_roads.csv`
- `t06_step3_removed_swsd_nodes.csv`
- `t06_step3_added_rcsd_roads.csv`
- `t06_step3_added_rcsd_nodes.csv`
- `t06_step3_unreplaced_rcsd_roads.gpkg/csv`
- `t06_step3_unreplaced_rcsd_attribution.gpkg/csv`
- `t06_step3_unreplaced_rcsd_attribution_summary.json`
- `t06_step3_id_collision_audit.gpkg/csv`
- `t06_step3_swsd_frcsd_segment_relation.gpkg/csv`
- `t06_step3_semantic_junction_groups.gpkg/csv`
- `t06_step3_topology_connectivity_audit.gpkg/csv`
- `t06_step3_authoritative_transition_closure_audit.gpkg/csv`（仅 final topology hard-gate 回退后存在受影响的 mixed-source transition 候选时产生非空记录）
- `t06_step3_surface_topology_audit.gpkg/csv`（仅调用方提供 T03/T04/T05/T07 surface 或 T04 audit 时输出）
...
- `t06_step3_surface_aware_replacement_plan_<label>.json`（surface-aware 兜底最终 replay plan；中间候选 plan 可在最终 plan 落地后清理）
- `t06_step3_progress.jsonl / t06_step3_heartbeat.json / t06_step3_stackdump.log`（仅 `--progress` 执行时输出；用于长耗时 Step3 的阶段心跳、主线程栈采样与 `SIGUSR1` 手工 stack dump，不参与业务判定）
- `t06_step3_summary.json`
- `t06_step3_detail_metrics.json`
- `t06_step3_output_manifest.json`

标准 Step3 CLI 默认 `suppress_feature_json_outputs=true`，不写出逐 feature 几何 JSON；GPKG/CSV 是正式审计和目视检查载体。需要旧版 feature JSON 调试时，调用方必须显式关闭该开关。`t06_step3_summary.json` 使用紧凑 schema，只保留下游常用输入路径、执行计划来源、核心漏斗、关系状态、拓扑状态和 surface-aware 计数级结果；完整文件清单、文件大小和大体量指标写入 `t06_step3_detail_metrics.json / t06_step3_output_manifest.json`。
...
`t06_step3_topology_connectivity_audit.*` 每行必须新增 `final_topology_category / final_topology_object_key / counts_in_final_frcsd_topology_fail / topology_road_lineage_id / topology_endpoint_index`。`counts_in_final_frcsd_topology_fail=true` 只允许以下最终图 hard fail：

- `segment_transition`：`segment_pair_nodes_not_connected / dual_segment_pair_nodes_not_bidirectional / segment_road_endpoints_not_connected / segment_road_directed_path_missing / junction_incident_segment_mapped_points_diverged`，以及 `retained_swsd_endpoint_closure` 层的 final mainnode/端点闭合 fail；
- `independent_attachment`：`final_road_node_integrity` 层 fail、`patch_road_attachment` 层 fail、`advance_right_leaf_endpoint_unattached`。

`segment_relation_failed / segment_corridor_coverage_dropped_after_replacement / segment_road_endpoint_mapping_missing / junction_incident_segment_mapping_missing / formal_replacement_source_consistency` 等对象即使 `audit_status=fail`，也不得计入 `final_frcsd_topology_fail_count`；它们分别由 replacement、construction/coverage、relation evidence 和 source consistency 审计承接。summary 必须同时输出兼容 `topology_connectivity_fail_count`、显式 `topology_audit_fail_row_count`、正式 `final_frcsd_topology_fail_count`，以及 `final_frcsd_segment_transition_fail_count / final_frcsd_independent_attachment_fail_count`。
`advance_right_endpoint_connectivity` 中能够由 `t06_step3_rcsd_advance_right_closure_audit.*` 证明为 `final_rcsd_advance_right_leaf_endpoint_has_unselected_native_rcsd_neighbor` 的 RCSD 原生边界叶端必须标记为 accepted exception，并在 Road 属性 `t06_accepted_native_boundary_node_ids` 中逐端点记录，不得用 Road 级布尔值豁免另一端；对应端点不得设置 `counts_in_final_frcsd_topology_fail=true`。旧输出只有 `t06_accepted_native_boundary_leaf` 时保留兼容读取。输入 SWSD 原生叶端单列 inherited input。正式发布不得保留 T06 引入的 Segment transition fail，无法闭合时必须在 topology-safe plan 中受控回退关联 replaced Segment。
`t06_step3_surface_aware_plan_release_audit.json` 必须输出 `final_topology_hard_gate`：至少包含 `status / iteration_count / rerun_count / rolled_back_count / rolled_back_plan_ids / iterations / final_decision`。每轮 `iterations` 必须记录 repairable、mapped、unmapped、inherited 数量、失败对象和新增回退 plan；回退 plan 使用 `plan_status=blocked / execution_action=hold / source_reason=final_frcsd_topology_hard_gate_failed / upstream_owner=T06_step3_final_topology_hard_gate`，并以 `final_topology_hard_gate_failure_node_ids` 记录触发该 plan 回退的正式失败 SWSD node。门禁最多两轮；source=2 inherited attachment 不进入回退集合，repairable unmapped 或最终残留必须输出 `failed_unresolved_formal_topology`。

hard-gate 首轮回退后，允许在第二轮回退决策前执行受限 mixed-source transition closure。候选只能来自“当前 hard-gate 回退 Segment 的 pair/junc node 减去 `final_topology_hard_gate_failure_node_ids`”，并且必须在标准 retained-carrier mainnode 同步后重新构建的当前 F-RCSD topology 中仍为 `segment_transition` 正式 fail。只有当前 remaining replaced relation 的 mapped root 唯一等于 T05 `status=0 / base_id>0` 权威 root、T05 base node 已进入最终 F-RCSD、当前 node 未命中 Patch/T04 冲突且 source=2 node 到权威 RCSD node 距离不超过 12m 时，才可把 retained source=2 endpoint 通过 `mainnodeid` 闭合到该 root；不得恢复已由原始正式 fail 直接触发回退的 Segment，不得新增 Road、修改几何或改变 Segment 替换计数。动作必须写入 `t06_step3_authoritative_transition_closure_audit.*`，retained relation endpoint 标记 `mapping_status=identity_authoritative_t05_mainnode_synced`，relation 风险标记 `authoritative_t05_transition_mainnode_synced`；收口后必须重新构建 topology audit，再决定是否需要下一轮 hard-gate 回退。
F-RCSD Road 中由 T06 发布的 `source=1` RCSD Road 必须包含 `t06_swsd_segment_ids` 列表字段，最终只允许单值或空值：`owner_type=single_segment` 写唯一 `owner_segment_id`；`special_junction_internal / multi_segment_connectivity / reality_change` 等无正式 Segment owner 的 Road 写空列表。任何 Road 出现两个及以上 Segment id 均为 hard fail。`path_corridor_group` 的组级原子执行不得转化为多 Segment Road 所有权。
...
`t06_step3_unreplaced_rcsd_attribution` 是 `t06_step3_unreplaced_rcsd_roads` 的归因增强层，不改变 Step3 替换结果。它保留 `attribution_*` 粗口径，用 50m Segment buffer 和 `5 > 4 > 3 > 2 > 1 > 6` 漏斗优先级记录历史对照：`5` 表示已在可替换范围但 Step2/Step3 未最终落地；`4` 表示已在所有路口 Relation 的 Segment 范围但不可替换；`3` 表示在 T06 有证据 Segment 范围但未达到所有路口 Relation；`2` 表示在 SWSD Segment 范围但无有效 T06 替换证据；`1` 表示不在 SWSD Segment 范围；`6` 表示无法落入上述任何归因范围，通常应接近 0 并进入人工审计。

最终汇报口径必须使用 `final_attribution_*`。最终口径先使用 road-level 强证据：若未替换 RCSDRoad 被 `t06_step3_replacement_units.rcsd_road_ids` 或 ready `t06_segment_replacement_plan.rcsd_road_ids` 精确引用但未进入 F-RCSD，则输出 `final_attribution_class=5_replaceable_scope_unreplaced` 且 `final_attribution_confidence=exact`；若只被 blocked plan 精确引用，则输出 `final_attribution_class=4_relation_scope_not_replaceable` 且 `final_attribution_confidence=exact`。缺少上述强证据时，必须先按几何主归属选择 `final_primary_segment_id`，排序依据为 20m buffer 覆盖率、50m buffer 覆盖率、距离和 Segment id；只有主归属达到有效覆盖门槛时，才按该 Segment 的 T06 状态输出最终归因，不得让邻近弱命中的高漏斗 Segment 抢占归因。主归属覆盖不足时输出 `1_outside_swsd_segment_scope`；主 Segment 无 Step1 evidence 时输出 `2_swsd_scope_no_t06_evidence`；主 Segment 未进入 relation scope 时输出 `3_evidence_scope_relation_incomplete`。主 Segment 在 relation scope 内但没有 road-level 强证据时，必须继续区分：若主 Segment 已在可替换范围且没有 Step2/problem registry failure reason，则输出 `5_replaceable_scope_unreplaced`；若主 Segment 未进入可替换范围且 failure reason 属于 required semantic nodes / anchor 语义闭合类，则输出 `3_evidence_scope_relation_incomplete`；`rcsd_not_bidirectional_for_swsd_dual` 等 RCSD 方向性/承载能力不足仍输出 `4_relation_scope_not_replaceable`。若主 Segment 与贴近的次要 Segment 指向不同漏斗状态，仍按几何主 Segment 输出粗归因，但 `final_attribution_confidence=low` 且 `ppt_review_flag=mixed_partial_segment_coverage`。

PPT 汇报口径必须使用 `ppt_attribution_*`：`4_relation_scope_not_replaceable / 5_replaceable_scope_unreplaced` 映射到 `1_segment_rcsd_quality_unreplaceable`，中文名为“Segment下RCSD质量导致无法替换”；`2_swsd_scope_no_t06_evidence / 3_evidence_scope_relation_incomplete` 映射到 `2_segment_replacement_prerequisite_unsatisfied`，中文名为“Segment侧替换前提不满足导致无法替换”；`1_outside_swsd_segment_scope` 映射到 `3_rcsd_outside_segment_scope`，中文名为“RCSD不在Segment范围内导致无法替换”；mixed 部分覆盖样本不新增归因类型，仅保留 `ppt_review_flag=mixed_partial_segment_coverage` 便于抽查。输出字段必须保留 `attribution_class / attribution_subclass / attribution_owner / matched_*_segment_ids / step1_reject_reasons / step2_reject_reasons / plan_statuses / step3_relation_statuses`，并新增 `coarse_attribution_class / coarse_attribution_subclass / final_attribution_class / final_attribution_subclass / final_attribution_owner / final_attribution_reason / final_attribution_confidence / final_attribution_basis / final_primary_segment_id / final_primary_scope / final_primary_distance_m / final_primary_cover20_ratio / final_primary_cover50_ratio / final_primary_effective_match / final_competing_segment_id / final_competing_distance_m / final_competing_cover20_ratio / final_competing_cover50_ratio / final_plan_statuses / final_step3_relation_statuses / final_step1_reject_reasons / final_step2_reject_reasons / final_problem_registry_reasons / competing_final_attribution_classes / ppt_attribution_class / ppt_attribution_label / ppt_source_attribution_class / ppt_review_flag`。summary 必须给出 total-RCSD 分母、未替换 RCSD 里程、RCSD 替换率，以及 `by_attribution_* / by_final_attribution_* / by_ppt_attribution_class / by_ppt_review_flag` 聚合。
...
稳定字段至少包含：`swsd_segment_id / relation_status / relation_reason / swsd_pair_nodes / swsd_junc_nodes / junc_kind2_exempt_nodes / detached_junc_nodes / swsd_road_ids / removed_swsd_road_ids / retained_detached_swsd_road_ids / frcsd_road_ids / owned_frcsd_road_ids / related_special_junction_internal_road_ids / connectivity_group_ids / related_connectivity_road_ids / pruned_non_owner_frcsd_road_ids / parallel_corridor_peer_road_ids / frcsd_road_source_values / rcsd_pair_nodes / rcsd_junc_nodes / junction_c_ids / group_replacement_plan_ids / group_replacement_source_segment_ids / group_replacement_segment_ids / swsd_to_frcsd_node_map / source_mix / risk_flags`。

`t06_rcsd_road_ownership` 对每条原始 RCSD Road 输出唯一判定。`owner_type=single_segment` 时 `owner_segment_id` 必须唯一且非空；`owner_type=special_junction_internal / multi_segment_connectivity / reality_change` 时 `owner_segment_id` 必须为空，关联 Segment 只写入 `related_segment_ids`。`special_junction_ids` 记录正式 replacement plan 中 `execution_scope=special_junction_group_internal` 的来源。Step3 summary 必须输出 `final_rcsd_road_single_segment_assignment_count / final_rcsd_road_unassigned_count / final_rcsd_road_multi_segment_assignment_count`，其中 multi count 必须为 0。
...
- `run/<step_dir>/...`
  - 默认包含 Step1 / Step2 summary，以及存在的 JSON / CSV 审计输出；内网 no-json 产物中缺失的 Step2 JSON 记为可选缺失。
...
`candidates` 为 buffer 成功构建的 RCSDSegment 候选；`replaceable` 为经过全部硬审计与特殊路口组局部替换门控后的最终可替换集合。非特殊组场景下 `replaceable.rcsd_road_ids` 等于 buffer 构建出的最小 corridor `retained_rcsd_road_ids`；特殊组 `partial` 时，原本通过硬审计的候选继续保留在 `replaceable`，未通过的关联 Segment 保持 rejected 或 retained SWSD。`rejected` 输出保留 `swsd_sgrade / swsd_directionality / failed_pair_nodes / failed_junc_nodes / junc_kind2_exempt_nodes`，用于定位高等级、单向 Segment 的 pair relation 硬失败、required junc relation 失败与豁免集合，同时记录 buffer 构建失败 reason。

当输入来自 P02 局部实验时，T06 消费 T05 基于完整 RCSDRoad/RCSDNode 发布的 copy-on-write 结果；其中可包含 `modules/p02_wuhan_local_experiment/endpoint_overrides/p02_confirmed_endpoint_overrides.csv` 登记并通过独立审计的 9 项用户确认覆盖。没有正式 relation 的 Segment 不得进入 replacement plan；T06 自身不改写端点、不读取 `NodeLid/CrossLid` 生成新 alias、不使用几何最近点或同坐标拼接 Road，也不因 P02 例外放宽方向、连通、buffer 或 required junction 硬审计。
...
`execution_scope` 当前稳定取值包括 `standard_segment / special_junction_group_internal / path_corridor_group`。`standard_segment` 必须来自 Step2 replaceable，`execution_action=replace`；`special_junction_group_internal` 必须来自 `gate_status=passed` 的全通过特殊路口组，`execution_action=include_context`；`path_corridor_group` 必须来自 passed group replacement probe，`execution_action=replace`。未在 plan 中发布为 `plan_status=ready` 的审计证据不得被 Step3 执行为替换。

`parallel_corridor_peer_road_ids` 仅为相邻 plan 通道的审计字段。Step3 不得用它满足当前 Segment 的 pair/required-junction 连通、coverage 或 SWSD 删除门禁，也不得写入当前 Segment 的 `owned_frcsd_road_ids / frcsd_road_ids`；最终 topology audit 不能替代该 Segment 自身的锚点覆盖校验。
...
默认读取 Step2 同目录 `t06_segment_replacement_plan.json`（若存在），再兼容读取 `geojson/gpkg`；旧结果缺少 replacement plan 时才回退读取 `t06_special_junction_group_audit.json` 与 `t06_segment_group_replacement_audit.gpkg`。需要指定其它审计文件时可使用 `--step2-special-junction-group-audit` 或 `--step2-group-replacement-audit`。
...
Step2 replacement plan 负责前置处理准确 T05 relation 下的 retained-junction 20m gate：当触发节点在 `intersection_match_all.geojson` 中存在 `status=0 / base_id>0`，且 `base_id` 与当前 plan 的 RCSD node canonical 后一致时，`MAX_RETAINED_JUNCTION_ATTACHMENT_GAP_M=20m` 只能产生 `junction_alignment_to_retained_swsd_exceeds_topology_gate / junction_alignment_t05_relation_release / manual_review_required` 风险标记，不得把 ready plan 改为 blocked。该 relation-backed 直接释放规则不适用于缺失 relation、relation status 非 0、base_id 无效、canonical 后不一致、pair anchor mismatch 被判为非权威、或 replacement plan 间映射到同一 SWSD junction 但 RCSD node 彼此分裂且无可解释 bridge 的场景。

在视觉、Road 冲突、路口对齐和 group gate 全部执行后，Step2 可对正式 replaceable 的 `standard_segment` 执行一次受限后置锚定复核。候选必须满足 `len(swsd_pair_nodes)=2`、`rcsd_pair_nodes` 为两个互异节点、全部非 detached/exempt Junc 在正式 `rcsd_junc_nodes` relation 中具有 RCSD 映射、`rcsd_road_ids` 非空，且其 RCSD Road 与所有当前 `plan_status=ready / execution_action=replace` 的标准或 group plan 无交集；`optional_junc_rcsd_nodes` 只是 buffer graph 子集，不作为正式锚定完整性判据。可降级的 source reason 仅包括：`junction_alignment_to_retained_swsd_exceeds_topology_gate`；failure business audit 明确给出非人工 `high_confidence_pair_anchor_candidate / side_preserving_missing_pair_anchor_completion` 的 `pair_anchor_mismatch_replacement_plan_anchor_not_authoritative`；以及 blocked plan 之间共享至少一个 RCSD Road、但不与 ready plan 冲突的 `junction_alignment_between_replacement_plans_diverged`。复核通过后必须恢复 `ready/replace`，将原阻断原因保留在 risk flags，并追加 `postplan_anchor_gate_deferred_to_step3_topology`、证据类型与 peer Segment 审计字段。Step3 必须用同一 candidate plan 关闭全部后置复核、但保留其它相同 surface/visual release 的结果作为同轮 topology 基准；candidate 相对该基准新增普通 Segment hard fail 时，必须将命中 Segment 回退为 `plan_status=blocked / execution_action=hold / source_reason=postplan_anchor_gate_failed_topology_gate`。直接关联回退后若仍存在无法由 fail key 唯一映射到 Segment 的非局部新增普通 Segment hard fail，必须对其余没有 `junction_alignment_surface_audit_release` 独立证据的后置候选执行 residual rollback，输出 `residual_rolled_back_plan_ids`，并保证后置锚定 gate 的新增 fail 为 0。`advance_right_endpoint_connectivity` 由独立提右 Step3 流程负责：仍保留在全局 `final_added_fail_*`，同时写入 `postplan_anchor_gate.non_postplan_added_fail_*`，但不得回退普通 Segment 后置候选。该规则不得释放 Step2 rejected、path-corridor、T04 reject、Patch 冲突、缺少完整锚定或与 ready plan 争抢 Road 的对象；Step3 的 `retained_swsd_not_attached_side_road_only` 严格 2b gate 和 topology connectivity audit 继续生效。

正式 Topology fail 基准比较必须使用逐层稳定的 `final_topology_object_key`，且只比较 `counts_in_final_frcsd_topology_fail=true` 的对象。Segment 内通道按 Segment + reason；Segment Road 按 Segment + SWSD Road + reason；共享路口按 SWSD node + incident Segment 集合；retained endpoint 按 Segment + SWSD Road/Node；patch 按 SWSD Road/Node + RCSD Road lineage；提右按 original Road lineage + endpoint index。不得再使用会把同 Segment 多条 Road或无 Segment relation 的多条提右 Road折叠为一个对象的通用 key。

当 Step3 调用方提供 surface 输入时，脚本仍可在不改变调用参数的前提下对旧 Step2 plan 中 `source_reason=junction_alignment_to_retained_swsd_exceeds_topology_gate` 的行执行 surface-aware 条件释放，作为兼容兜底而非当前 relation-backed 或 Step2 受限后置复核的决策主路径。释放条件只接受两类证据：触发超距节点在 surface topology audit 中为 pass，且 reason 属于 `auto_closed / auto_closed_surface_1v1 / auto_closed_t04_patch_1v1 / auto_closed_step2_junc_1v1 / auto_closed_relation_mapped_boundary_1v1 / auto_closed_selected_replacement_endpoint`；或触发超距节点是 plan 的 `swsd_pair_nodes -> original_rcsd_pair_nodes` 原始端点映射。T04 reject、Patch 冲突、多候选、缺少可解释 endpoint、以及未携带 Step2 后置复核审计证据的其它 blocked plan 都不能被释放。释放后的 plan 必须重跑 Step3 与 topology audit；其它新增 hard fail 仍必须回退为 `plan_status=blocked / execution_action=hold`，并记录 `source_reason=junction_alignment_surface_release_failed_topology_gate`。若相对传入 `--t06-run-root` 下已有 Step3 baseline 仍存在不可降级新增 fail，必须写入 `external_final_added_fail_count / external_final_added_fail_keys`，不得把内部 release 回退通过误表述为外部 baseline 零新增。
...
内网脚本默认 `--no-write-json-outputs`，Step2 写出 GPKG/CSV、summary 和 progress sidecar；如需要兼容旧调试链路，可显式加 `--write-json-outputs` 生成 Step2 JSON feature dump。
...
- 仅当 Segment 为高等级口径（`sgrade` 以 `0-0` 或 `0-1` 开头），或属于 `sgrade=0-*单` 的拆分边界，且失败节点属于 `pair_nodes`、该节点 `has_evd = yes`、`is_anchor = fail1`、`kind_2 = 4` 时，Step1 允许该 Segment 进入 Step2 probe。`sgrade=0-*单` 的拆分边界若 `pair_nodes.kind_2=4`、`has_evd` 明确非 yes 且 `is_anchor` 为空、`no` 或 `fail1`，也只允许进入 Step2 probe，不视作 anchor 已成功。
...
- 该规则只负责消费 T07/T05 已正式表达的 SWSD 1:N RCSD 语义路口关系，不放宽 50m buffer、方向性、连通性、retained Road 覆盖率或特殊路口组局部替换门控。
...
5. `junc_nodes.kind_2 in {1,4096,8192}` 的节点不参与 Step1 `has_evd / is_anchor` 判定；Step2 中未被 detached / exempt 的 `junc_nodes` 按 required junction 审计处理。同值 `pair_nodes` 仍按原规则判定并映射。高等级 Segment 中 `pair_nodes.kind_2=2048`、`junc_nodes.kind_2 in {16,2048}` 只能作为 Step2 probe 放行，不得被视作 Step1 anchor 豁免或 T05 relation 成功。`sgrade=0-*单` 的 `pair_nodes.kind_2=4` 拆分边界放行同样只进入 Step2 probe，不回写 EVD/anchor 事实。高等级 Segment 的非特殊 junc-only 节点若因 `has_evd / is_anchor` 失败拖垮主通道，Step1 可输出 `detached_junc_nodes / detached_junc_reasons` 并从 final `junc_nodes / semantic_node_set` 中移除；`pair_nodes` 与 `kind_2 in {64,128}` 特殊路口不得脱挂。
6. Step2 不执行旧 pair-to-pair BFS 路径搜索、主轴 / 粗长度趋势或唯一性筛选；buffer 候选连通分量必须先收缩为覆盖 pair nodes + required junction nodes 的可解释 corridor 子图；`swsd_directionality=single` 时必须由 SWSDRoad `snodeid / enodeid / direction` 推导 pair source/target，并按该方向构建覆盖 pair nodes + required junction nodes 的 RCSD corridor，否则以 `rcsd_directed_path_missing` 或 `swsd_single_direction_*` 拒绝；特殊语义路口 subnode 端点导致初始有向 corridor 失败时，只允许保持原方向执行本地 corridor 受限释放，不允许翻转 Segment 方向、改写字段语义或回写 T05 relation；`swsd_directionality=dual` 时 retained RCSD graph 必须 pair 两端双向可达，否则以 `rcsd_not_bidirectional_for_swsd_dual` 拒绝；pair anchor 自动补全缺失端点时不得使用 buffer-only 候选顺序覆盖 SWSD pair 侧，必须按 relation 失败侧重排候选。
7. SWSD pair 两端相同或映射后 RCSD pair 坍缩到同一个语义路口时，Step2 必须拒绝并输出 `swsd_pair_nodes_not_distinct` 或 `rcsd_pair_nodes_not_distinct`。
8. 未被 detached / exempt 的 `junc_nodes` 执行 required junction relation / topology 审计；exempt junc 可被剪除并输出 dropped / lost attach 审计，retained graph 中出现非 pair leaf endpoint 时必须拒绝并输出 `unexpected_endpoint_node_ids`。
9. retained graph 中允许 RCSD 自身未锚定节点作为通路节点；额外 T05 mapped semantic nodes 只有在破坏目标 Segment 相对拓扑、形成错误跨 Segment 消费或构成完全绕开 SWSD buffer 的 pair-to-pair 通路时才拒绝并输出 `unexpected_mapped_semantic_node_ids`。
10. 所有解析、映射、buffer 构建失败都有明确 reason。
11. 输入文件不被原地修改。
12. buffer-based RCSDSegment 审查必须按 `formway` bit7/128 识别提前右转 road；二度链接保留、required corridor 保留和排除结果必须在 summary / 输出中可审计。
13. `swsd_directionality=dual` 不能因短 required-to-required connector 通过双向审计；retained corridor 内部 `formway & 1024 != 0` 调头 road 必须保留。
14. Step2 retained RCSDRoad 必须满足 `min_buffer_road_overlap_ratio` 覆盖审计；不满足时以 `retained_road_buffer_overlap_insufficient` 拒绝，并在 `failed_metric_value` 中记录低覆盖 Road ID 与最小覆盖率。
15. 高等级受限重审默认保留 T05 原始 pair relation；single 必须以 RCSD 有向图联通 pair 路口并经过 50m buffer core，不得消费 repair candidate；dual 只有在 buffer-only probe 给出非人工复核高置信候选 pair 集合，且候选 pair 遍历试算后恰好一个通过正式双向 extractor / adaptive buffer / dual graph-first 硬审计时，才允许在当前 Segment 内使用该候选 pair。dual graph-first 必须保持 required junction 相对拓扑并排除完全绕开 SWSD buffer 的 pair-to-pair 通路。重审通过时必须输出审计距离、方向分组与来源失败原因。
16. Step3 必须优先消费 Step2 replacement plan，只执行 `plan_status=ready` 的普通 Segment replace、全通过特殊路口组内部 include_context 与 path-corridor group replace action；删除被替换 SWSDRoad 与其端点 SWSDNode，保留未替换 SWSD 数据并写 `source=2`，引入 retained RCSD 数据、plan 发布的全通过特殊路口组内部 RCSD 数据与 path-corridor group RCSD 数据并写 `source=1`。特殊路口组 `partial` 时不得引入内部 RCSD Road/Node，保留 SWSD 内部 road，并由 `t06_step3_semantic_junction_groups.*` 表达替换端点与保留端点同属语义路口。若 replaceable 的 junc 集合相对 T01 原始 Segment 发生 detached junc 缩减，detached junc 触达的原 SWSDRoad 必须保留为 `source=2` 局部 carrier，Segment relation 必须标记 `replaced+retained_swsd` 并输出 identity node map，避免 T09 显式 restriction 因局部未锚定而丢失。
17. Step3 必须按 C 聚合重建语义路口关系；若原 main node 被删除，必须重新选择 main node，并让 C 内 Node 继承原 main node 的 `kind / grade / kind_2 / grade_2 / closed_con`。
