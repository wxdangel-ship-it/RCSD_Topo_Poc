...
| F-RCSD 质检 | T12 | 对原始 1V1 匹配生成的 F-RCSD 做 SWSD 可达性等价假设检查，以 raw endpoint topology、标准路口 portal 和锚点可信度自动发布高置信问题及排除证据；人工复核仅作可选 QA 覆盖，不修改 T06 或目标路网。 |
...
7. T12 必须把“SWSD 与原始 1V1 F-RCSD 的通行性应等价”作为待验证质量假设；只有通过 raw endpoint topology、标准路口 portal、方向/几何和锚点可信度门禁的候选才能自动进入 confirmed 正式问题层，外部 review decisions 仅作可选 QA 覆盖，任何结果都不得据此 silent fix。
...
- 用 T12 持续验证原始 1V1 F-RCSD 与 SWSD 的可达性等价假设，自动发布高置信正式问题，并冻结 raw topology、标准路口 portal、锚点可信度和误报排除证据。
