...
T10 面向 RCSD_Topo 端到端业务流程编排与 Case 级证据组织。项目级业务数据链仍保持 T06 F-RCSD 直接进入 T09；T10 v1 的默认审计编排范围为 `T01 -> T07 Step1/2 -> T03 -> T04 -> T05 -> T06 -> T11 -> T09`。显式启用 T12 时，编排变为 `T01 -> T07 Step1/2 -> T03 -> T04 -> T05 -> T06 -> T11 -> T12 -> T09`。T12 与 T11 都是 audit-only，不向 T09 回写业务输入。T08 作为独立前置预处理、质检与修复模块，不由 T10 v1 callable 或 Case runner 调用；内网全量总控脚本可把 T08 作为独立前置阶段纳入全量执行链路。F-RCSD 质量检查专用入口固定跳过 T08、启用 T12。T07 Step3 是可选兼容 relation 补锚阶段，不属于 T10 v1 默认编排。
...
- 支持通过 `--run-t12` 显式插入 T12；默认关闭，保持既有阶段与业务 handoff。
...
Case runner 不改变 T01-T09 / T11 / T12 算法，只负责：
...
- `t12_frcsd_quality_audit/`（仅在显式启用 T12 时存在）
- `t09_swsd_field_rule_restoration/`

`t10_innernet_full_pipeline_manifest.json` 保留兼容的 flat `inputs / outputs`，同时必须提供阶段级 `stage_order / stages`。每个 `stages.<stage_id>` 至少包含 `stage_id / module_id / status / stdout_log / inputs / outputs / params / execution_context`，用于统一表达 T08、T01、T07 Step1/2、T03、T04、T05、T06 Step1/2、T06 Step3、T11、可选 T12、T09 的显式 handoff；显式启用 T07 Step3 时，manifest 才登记 `t07_step3` 阶段。`RUN_T08=0` 时不得把未运行的 T08 登记到 `stage_order / stages`。下游审计应优先消费 `stages`，仅在兼容旧产物时读取 flat `outputs`。

全量 runner 在创建 manifest 时必须先写入 `status=running / passed=false`；进程退出时必须写入 `status / passed / exit_code / finished_at_utc / duration_seconds`。T11 与 T09 阶段均完成，且 T11 candidates/summary、T06 F-RCSD Road/Node、T09 `frcsd_restriction.gpkg` 均存在时，`status=passed`；仅当 manifest 声明 `t12_enabled=true` 时，还要求 T12 stage passed 及 candidates/confirmed/summary 存在。任一阶段命令失败或必要输出缺失时，退出 trap 必须把顶层状态写为 `failed`。
...
阶段续跑模式不创建新 T10 run root，不重跑未列入 `RUN_STAGES` 的前序阶段；它优先读取既有 manifest 中的 `inputs / outputs` 作为 handoff，缺失时才回退到固定目录结构。支持阶段名：`t08_preprocess / t01 / t07_step12 / t03 / t04 / t05 / t06_step12 / t06_step3 / t11 / t12 / t09`；`t07_step3` 只作为显式兼容补锚阶段支持。`RUN_STAGES=t12` 只有在 `RUN_T12=1` 且显式原始 1V1 F-RCSD 输入齐备时才允许执行。最终完整 T10 通过要求 T06 F-RCSD Road/Node、passed 的 T11 stage 与 candidates/summary、T09 `frcsd_restriction.gpkg` 同时存在；manifest 声明 `t12_enabled=true` 时还要求 T12 必要产物。
...
- T12 输入覆盖变量：`FRCSD_1V1_ROADS_PATH`、`FRCSD_1V1_NODES_PATH`、可选 `T12_REVIEW_DECISIONS`、可选 `T12_CASE_MANIFEST`；前两者必须指向原始 1V1 F-RCSD，不得使用 T06 Step3 输出替代。`T12_CASE_MANIFEST` 仅用于裁剪 Case 的显式边界排除，全图运行必须留空，并进入总 manifest 与 T12 stage 审计记录。
...
- `stop_after`：T10 E2E Case runner 截断阶段，包含可选 `t12` 与 `t11`。
...
5. Case evidence package manifest 排除 T01-T09 / T11 / T12 模块间中间产物。
