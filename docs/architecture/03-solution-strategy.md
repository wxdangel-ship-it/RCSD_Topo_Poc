...
| T12 | 对原始 1V1 F-RCSD 执行 audit-only 质量检查，验证 SWSD 通行性等价假设，输出候选、确认、排除与人工复核层，不执行修复。 |
...
- F-RCSD QA：T06 Step3 结果继续由 T06 正式审计；外部 1V1 匹配生成的 F-RCSD 由 T12 检查 SWSD 等价可达性、road-node integrity、方向图、局部 portal 替代路径和复核发布。
