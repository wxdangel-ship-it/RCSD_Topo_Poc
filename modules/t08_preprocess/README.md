...
| 文档 | 承载内容 |
|---|---|
| `SPEC.md` | 模块需求：业务目标、范围、上下游、输入输出、Tool1-10 关键步骤、对错边界。 |
| `INTERFACE_CONTRACT.md` | 稳定接口：Tool1-10 输入输出、脚本入口、参数和验收契约。 |
...
| `architecture/03-solution-strategy.md` | Tool1-10 的需求具体实现策略。 |
...
T08 成果输出文件名默认在扩展名前以 `_toolX` 结尾，`X` 为工具编号。Tool1 转换成果是命名特例，使用输入文件同 stem、不同格式后缀；Tool10 按用户指定固定写 `<Patch>/Traj/raw_dat_pose.gpkg`；两者 summary 仍分别按 `_tool1 / _tool10` 命名。
