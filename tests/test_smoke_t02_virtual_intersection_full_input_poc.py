...
@pytest.mark.smoke
def test_smoke_t02_virtual_intersection_full_input_poc() -> None:
    run_id = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    root = Path("outputs/_work/smoke_t02_virtual_intersection_full_input_poc") / f"{run_id}_{os.getpid()}"
    ...
    assert exit_code == 0
    run_dir = outputs_dir / "smoke_full_input"
    case_dir = run_dir / "cases" / "100"
    assert (run_dir / "preflight.json").is_file()
    assert (run_dir / "summary.json").is_file()
    assert (run_dir / "perf_summary.json").is_file()
    assert (run_dir / "virtual_intersection_polygons.gpkg").is_file()
    assert (run_dir / "_rendered_maps" / "100.png").is_file()
    assert (case_dir / "virtual_intersection_polygon.gpkg").is_file()

    summary_doc = json.loads((run_dir / "summary.json").read_text(encoding="utf-8"))
    assert summary_doc["mode"] == "specified_mainnodeid"
    assert summary_doc["success_count"] == 1
    assert summary_doc["selected_case_ids"] == ["100"]
