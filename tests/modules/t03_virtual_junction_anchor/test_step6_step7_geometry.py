...
def _run_case(case_root: Path, step3_root: Path, case_id: str) -> tuple[FinalizationContext, object, object]:
    ...
    return finalization_context, step6_result, step7_result


def _road_covered_length(finalization_context: FinalizationContext, step6_result: object, road_id: str) -> float:
    road = next(
        road
...
def test_finalization_accepts_case_a_when_step6_is_clean(tmp_path: Path) -> None:
    ...
    finalization_context, step6_result, step7_result = _run_case(case_root, step3_root, "100001")

    status_doc = build_step6_status_doc(finalization_context, step6_result)
    ...
    assert _road_covered_length(finalization_context, step6_result, "road_h") <= 38.0
    assert _road_covered_length(finalization_context, step6_result, "road_v") <= 38.0


def test_finalization_accepts_association_support_only_cases_after_step6_convergence(tmp_path: Path) -> None:
    case_root = tmp_path / "cases"
    step3_root = tmp_path / "step3"
    build_center_case_b(case_root, step3_root, case_id="100002")

    _finalization_context, step6_result, step7_result = _run_case(case_root, step3_root, "100002")

    assert step6_result.geometry_established is True
    assert step7_result.step7_state == "accepted"
    assert step7_result.visual_review_class == VISUAL_V1
    assert step7_result.root_cause_layer is None
    assert step7_result.reason == "step7_accepted_after_support_only_convergence"
...
def test_finalization_keeps_visual_v2_as_visual_audit_only_when_geometry_is_risky(tmp_path: Path) -> None:
    case_root = tmp_path / "cases"
    step3_root = tmp_path / "step3"
    build_center_case_a(case_root, step3_root, case_id="100004")

    finalization_context, step6_result, _step7_result = _run_case(case_root, step3_root, "100004")
    risky_step6 = replace(
        step6_result,
        problem_geometry=True,
        review_signals=("polygon_has_holes",),
    )
    risky_step7 = build_step7_result(finalization_context, risky_step6)
    ...
    assert risky_step7.reason == "step7_accepted_with_visual_risk"


def test_finalization_rejects_cases_blocked_by_upstream_association(tmp_path: Path) -> None:
    case_root = tmp_path / "cases"
    step3_root = tmp_path / "step3"
    ...
    _finalization_context, step6_result, step7_result = _run_case(case_root, step3_root, "100003")
...
def test_finalization_single_sided_case_applies_directional_cut_before_rejecting(tmp_path: Path) -> None:
    ...
    _finalization_context, step6_result, step7_result = _run_case(case_root, step3_root, "100006")
