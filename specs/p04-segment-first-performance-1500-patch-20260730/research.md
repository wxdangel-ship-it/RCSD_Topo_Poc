...
`perf_opt11_1885118_20260730T1400`：

- wall：`738.55s -> 161.05s`，降低约`78.2%`，提速约`4.59x`；
- process CPU：最终`118.53s`，平均进程CPU占用`73.6%`；
- peak RSS：最终`528060416 bytes`，约`503.6MiB`；
...
优化后的P04专项测试为`277 passed`。以既有1532 Patch失败运行
`97689s`与同机6-Patch冻结基线做保守线性折算，候选约为`21302s`
（约`5.92h`）；该数字只是复跑候选估算，不替代约1500 Patch内网正式
