...
- 唯一新增正式入口为 `scripts/t08_tool11_patch_data_organization.py`。
- 核心逻辑位于 T08 callable；脚本只负责参数解析、进度和退出码。
...
- 覆盖默认 6 Patch 实验集、嵌套目录、空目录、FRCSD 白名单、缺失项聚合、冲突保护、显式覆盖、CLI 参数和 summary。
...
- **FR-020**：所有业务路径 MUST 由参数提供，不得写死内网目录。
