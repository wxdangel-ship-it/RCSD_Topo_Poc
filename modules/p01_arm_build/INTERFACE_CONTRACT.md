...
输入可为 SWSD `RoadNextRoad.json`、RCSD / F-RCSD `RoadNextRoad.geojson` 或同类 JSON / GeoJSON 结构。未提供某源 RoadNextRoad 时，该源 A1 movement evidence 输出为空，trunk correction 输出 `not_evaluated_no_road_next_road_input`。P01-Final 最终产物不复制输入 F-RCSD RoadNextRoad，而是输出新生成的 `frcsd_road_next_road.geojson`。
