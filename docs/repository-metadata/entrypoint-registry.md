...
| `t07_run_semantic_junction_anchor_innernet.sh` | `scripts/t07_run_semantic_junction_anchor_innernet.sh` | repo 级 | T07 内网语义路口级 Step1 / Step2 执行脚本；Step1 只读取 `nodes / DriveZone`，Step2 再读取 `RCSDIntersection` 并可选读取 RCSDNode 辅助判断其消费性；调用模块内 callable runner 输出代表 node 的 `has_evd / is_anchor / anchor_reason`，不处理 Segment | `active` | 否 |
...
| `t08_tool1_vector_convert.py` | `scripts/t08_tool1_vector_convert.py` | repo 级 | T08 Tool1 基础矢量格式转换，支持 SHP / GeoJSON / FGB 转 GPKG 与 GPKG 转 GeoJSON，转换成果写回输入目录下并使用同 stem、不同格式后缀，采用流式转换并输出进度，支持可选目标 EPSG 与 summary 输出 | `active` | 否 |
...
| `t08_tool11_patch_data_organization.py` | `scripts/t08_tool11_patch_data_organization.py` | repo 级 | T08 Tool11 Patch 级数据整理；参数化扫描全量 Patch，递归复制 SWSD/RCSD、仅复制三个 FRCSD 白名单 GeoJSON，逐文件 SHA-256 校验后发布统一全量根，并可选发布指定实验 Patch 独立根；默认拒绝覆盖并输出 `_tool11` summary | `active` | 否 |
| `t08_tool11_run_innernet.sh` | `scripts/t08_tool11_run_innernet.sh` | repo 级 | T08 Tool11 内网 WSL 固定场景入口；默认使用已确认的原始根和全量根执行全量-only 整理，不校验或生成实验 Patch；显式设置实验输出根才启用固定 6 Patch 实验模式。自动转换 Windows 路径、调用正式 Tool11 Python 入口并持久记录控制台日志，只有 `OVERWRITE=1` 才替换已有成果 | `active` | 否 |
...
| `t03_pack_failed_cases_first50_innernet.sh` | `scripts/t03_pack_failed_cases_first50_innernet.sh` | repo 级 | T03 首批失败审计 Case 内网提取入口；从已确认 T10 full run manifest 精确读取当次 T03 正式交接输入（T07 Step2 Nodes、T01 Roads、DriveZone、RCSDRoad/RCSDNode）及同次 DivStripZone，固定提取用户指定失败清单的前 50 个 mainnodeid，预检全部代表节点的 T03 候选状态并输出来源清单、自动分片文本 bundle、解包校验目录与传输归档 | `active` | 否 |
...
| `t10_run_e2e_cases.sh` | `scripts/t10_run_e2e_cases.sh` | repo 级 | T10 Case 级端到端执行入口；默认串联 `T01 -> T07 Step1/2 -> T03 -> T04 -> T05 -> T06 -> T11 -> T09`；显式 `--run-t12` 时在 T11 后、T09 前运行 audit-only T12，并把当前 T03 run root 与 T07 Step1/2 run root 交给 T12，登记 Segment/Junction 独立输出，但不改变 T06/T11/T09 业务 handoff；同时输出阶段审计与 T06 数据漏斗，并支持 feedback iteration no-regression guard | `active` | 否 |
| `t10_run_innernet_full_pipeline.sh` | `scripts/t10_run_innernet_full_pipeline.sh` | repo 级 | T10 内网全量端到端总控脚本；默认串联 `T08 -> T01 -> T07 Step1/2 -> T03 -> T04 -> T05 -> T06 Step1/2 -> T06 Step3 -> T11 -> T09`，显式 `RUN_T12=1` 时在 T11 后、T09 前运行 T12，传入当前 T03 root 与 T07 Step1/2 root，不依赖可选 T07 Step3；支持通过 `T12_PROCESSING_CRS` 显式透传混合输入的 projected metre CRS；T12 运行后可在同一 T10 run root 以新的 `T12_RUN_ID` 和显式 Segment 复核 CSV 续跑并登记 Segment/Junction 输出；所有实际运行阶段写入同一 run root 与 manifest，支持 finalize/resume/stage selection，默认流程业务效果不变 | `active` | 否 |
| `t10_run_frcsd_quality_pipeline.sh` | `scripts/t10_run_frcsd_quality_pipeline.sh` | repo 级 | T10 原始 1V1 F-RCSD 质量检查专用流水线；固定 `RUN_T08=0 / RUN_T12=1`，按 `T01 -> T07 -> T03 -> T04 -> T05 -> T06 -> T11 -> T12 -> T09` 复用通用 full runner，要求显式原始 1V1 F-RCSD Road/Node；T12 同时发布 Segment 与 Junction，混合 CRS 时接受显式 `T12_PROCESSING_CRS`，支持新 `T12_RUN_ID / T12_REVIEW_DECISIONS` 续跑 Segment 覆盖，沿用 manifest、summary、resume 和失败退出，不复制模块算法 | `active` | 否 |
| `t12_run_frcsd_quality_audit.py` | `scripts/t12_run_frcsd_quality_audit.py` | repo 级 | T12 原始 1V1 F-RCSD Segment/Junction 质量审计正式 Python 入口；读取 SWSD、原始 1V1 F-RCSD、RCSDIntersection、T05/T06 交叉证据、可选 T03 root 与 T07 Step1/2 root 及 DriveZone/review decisions，按 S01-S03/J01-J04 输出 Segment 线几何族（`LineString/MultiLineString`）与 Junction Point 的 candidate/confirmed/excluded/evidence/summary 分层结果；旧 T07 Step3 参数仅作弃用定位兼容，不读取 cardinality 错误，不修改输入或执行修复 | `active` | 否 |
| `t12_rerun_frcsd_junction_quality_innernet.sh` | `scripts/t12_rerun_frcsd_junction_quality_innernet.sh` | repo 级 | T12-only 内网重跑入口；要求 `T10_RUN_ROOT` 并使用环境变量接收既有正式 T01/T03/T05/T06/T07、SWSD、原始 1V1 F-RCSD 和 RCSDIntersection 路径，做前置校验后调用 T12 Python 正式入口；默认新结果保持 T10 标准目录 `<T10_RUN_ROOT>/t12_frcsd_quality_audit/t12_full`，既有同名目录完整移动到 `<T10_RUN_ROOT>/history/t12_frcsd_quality_audit/`；新批次失败时保留失败目录并恢复原标准批次，成功时才提交目录切换；验证 Segment/Junction 必要输出并打印历史路径、进度、计数和耗时，不覆盖或删除历史结果 | `active` | 否 |
