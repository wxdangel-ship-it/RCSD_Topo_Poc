...
| `t06_rcsd_segment_replaceable.*` | 通过全部硬审计和特殊组局部替换门控的白名单。 |
...
| `t06_special_junction_group_audit.*` | 环岛 / 复杂路口组完整性与 `passed / partial / blocked` 审计。 |
...
| `t06_step3_authoritative_transition_closure_audit.*` | hard-gate 直接回退后 mixed-source 边界的候选、T05 base/root、距离、阻断原因和实际 mainnode 收口动作。 |

`t06_step3_topology_connectivity_audit.*` 是多类审计证据集合，不再用全部 fail 行直接代表最终错误。正式最终质量只消费 `counts_in_final_frcsd_topology_fail=true` 的 `segment_transition / independent_attachment` 对象，并按 `final_topology_object_key` 去重；其余 relation、coverage、source 与映射证据问题由对应专项指标承接。
`t06_step3_authoritative_transition_closure_audit.*` 不能作为通用 surface fallback 或替换释放白名单；它只证明 hard-gate 直接回退后新产生的 mixed-source 级联 fail 是否满足严格收口条件。直接失败节点、Patch/T04 冲突、无有效 T05 base、remaining replaced root 分歧或距离超 12m 必须保留为 skipped 证据。
