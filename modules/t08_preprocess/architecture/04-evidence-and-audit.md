...
| Tool11 | 全量根、实验根、逐 Patch/逐文件路径、大小、SHA-256、忽略项、失败、发布和性能 summary。 |
...
Tool11 summary 必须同时覆盖成功和失败。逐文件记录角色、相对路径、源/主输出/实验输出绝对路径、大小和 SHA-256；逐 Patch 记录三个源目录、目标目录、空目录、FRCSD 忽略项、实验标记和错误。根级审计记录非 Patch 条目、根路径重叠检查、覆盖模式、发布/回滚状态。性能记录扫描、复制校验、发布、总耗时和吞吐。任何哈希不一致、源文件复制期间变化或部分发布都不能标记 `passed`。
