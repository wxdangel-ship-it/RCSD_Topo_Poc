...
| `t10_run_innernet_full_pipeline.sh` | `scripts/t10_run_innernet_full_pipeline.sh` | repo 级 | T10 内网全量端到端总控脚本；不走 Case package，串联 `T08 -> T01 -> T07 Step1/2 -> T03 -> T04 -> T05 -> T07 Step3 -> T06 Step1/2 -> T06 Step3 -> T09` 既有模块入口或 callable，所有阶段输出到 `outputs/_work/t10_innernet_full_pipeline/<RUN_ID>/` 并写入 manifest；支持 `FINALIZE_EXISTING` 补写既有 run 完成状态，并支持 `RESUME_RUN_ROOT / RESUME_FROM_STAGE / RUN_STAGES` 从既有 run 的指定阶段续跑 | `active` | 否 |
