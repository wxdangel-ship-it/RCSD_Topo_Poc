...
- 输入字段治理约束：
  ...
  - `Road.kind` 已正式启用；当前确认其字段语义为道路种别，单个 token 使用 `XXXX` 形式表达，前两位为道路等级、后两位为道路类型，多个道路种别用 `|` 分隔。当前在 T08 Tool4 中启用后两位道路类型：任一 token 后两位为 `0a`（大小写不敏感）时，该 Road 视为辅路；在 T08 Tool5 错误 1 对多 T-pair 虚拟连通补充判定中启用规范化后的完整 `Road.kind` 等值匹配；在 T01 双向 Segment 构建中启用前两位道路等级，用于分歧 / 合流 / 低等级交叉路口的局部续行优先策略，不通过几何或局部样本反推缺失 `Road.kind`。
  - `RCSDRoad.formway` 已正式启用；在 T03 / T06 中当前确认 `1024` bit 表示调头口，判定表达式为 `(formway & 1024) != 0`，字段名大小写不敏感；T06 仅将其用于 retained corridor 内部调头 road 保留，不作为几何反推规则。
  - T09 消费 T08 Tool8 `arrow` / 原始 SW `Arrow_Dir` 时，字母型箭头码按大小写不敏感方式归一到小写码表；已确认 `A` 等同于 `a`，表示 `straight`。数字 `0` 与字母 `o/O` 仍按码表区分为不同语义。
