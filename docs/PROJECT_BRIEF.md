...
- `t06_segment_fusion_precheck` 当前作为 Active 正式业务模块；正式口径覆盖 Step1 SWSD 可融合 Segment 识别与 Step2 buffer-based RCSDSegment 构建、兼容可替换集合和错误分析，不执行 Segment 替换或路口重塑
...
- T06 Step2 以 SWSD Segment 50m buffer 生成 RCSDSegment 审查输出；RCSDRoad 使用 `intersects + 阈值` 筛选，构图前以 `formway` bit7/128 识别提前右转 road，若提前右转 road 两端均与非提前右转候选 road 形成二度链接则保留，否则排除；`junc_kind2_exempt_nodes` 只作为 optional allowed 审计节点；候选连通分量必须先收缩为覆盖 required semantic nodes 的最小 corridor 子图，不能直接作为 retained 成果；额外 T05 mapped semantic nodes 必须按 seed-based pruning 判定为 `inner_nodes / out_nodes`，retained graph 中仍存在 required / optional allowed 以外 mapped semantic nodes 时必须拒绝；同一 RCSD base node 若同时映射到本 Segment 外 SWSD 语义节点，也视为额外 mapped semantic node；`swsd_directionality=dual` 时 retained graph 必须 pair 两端双向可达，`swsd_directionality=single` 时必须构建一条覆盖全部 required semantic nodes 的 pair 端到另一端有向 corridor，不得把无向 corridor 与有向 pair path 做并集；Step2 不再执行旧 pair-to-pair BFS 路径搜索、SWSD 单向方向推导、主轴 / 长度趋势或唯一性筛选
- T06 Step2 RCSD 建图使用 `rcsdnode_out.gpkg` 的 `id / mainnodeid / subnodeid` 归一化到 canonical RCSD semantic node id；relation required nodes 与 RCSDRoad `snodeid / enodeid` 按同一 canonical key 判定连通
