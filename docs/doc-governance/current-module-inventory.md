...
| `t06_segment_fusion_precheck` | `modules/t06_segment_fusion_precheck` | Step1 SWSD 可融合 Segment 识别；Step2 RCSD Segment candidate 抽取、buffer-based RCSDSegment 审查输出、趋势硬筛、可替换集合与错误分析 | 本轮按新模块启动要求建立 `INTERFACE_CONTRACT.md`、`README.md`、`AGENTS.md` 与 architecture 文档 | 提供模块内 callable runner 与 `scripts/t06_run_innernet_precheck.py` 内网运行包装；不新增 repo CLI、`tools/`、`Makefile` 或模块 `run.py` / `__main__.py` | 当前范围不执行 Segment 替换，不重塑路口，不修改 T01/T05 输出 |
