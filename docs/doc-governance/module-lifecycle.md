...
| `t08_preprocess` | `modules/t08_preprocess` | T08 正式预处理模块；当前覆盖 Tool1 基础矢量格式转换、Tool2 Road GPKG 预处理与 Tool3 Nodes 类型聚合，Tool1 支持 SHP / GeoJSON 转 GPKG 与 GPKG 转 GeoJSON，输出均写回输入目录下同名文件，补充 `patch_id / kind / kind_2 / grade_2` 并输出 `EPSG:3857` GPKG；Tool3 以外的 Node 预处理待后续定义 | `tool1-tool2-tool3 active / innernet scripts registered / other node preprocessing deferred` |
