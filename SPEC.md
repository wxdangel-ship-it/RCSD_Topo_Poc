...
- T06 Step1 正式启用 final `nodes.gpkg.kind_2` 的 junc-node 豁免语义：仅当 `junc_nodes` 中节点的 `kind_2 in {1,4096,8192}` 时，该 junc node 不参与 Step1 的 `has_evd / is_anchor` eligibility 判定并视为通过；该语义不适用于 `pair_nodes`，不改变 `junc_nodes / semantic_node_set` 输出，也不跳过 Step2 relation mapping。
...
- T06 Step1 中 final `nodes.gpkg.kind_2 in {1,4096,8192}` 只作为 `junc_nodes` 的 eligibility 豁免字段：命中节点不参与 `has_evd / is_anchor` 判定并视为通过；`pair_nodes` 仍必须按原规则判定，Step2 仍必须校验 T05 relation。
