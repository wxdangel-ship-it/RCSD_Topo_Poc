# T01 Step2 kind_2=128 Performance Fix - Plan

## Scope

修复 T01 双向 Segment 在复杂 `kind_2 = 128` 场景下的两类性能问题：Step1/S2 candidate search 因复杂 mainnode 聚合丢失组内独立分歧 / 合流边界而候选放大，以及 Step2 trunk 验证在复杂局部全局追溯时运行时间不可控。

## Implementation Plan

1. 增加 path search budget 数据结构。
2. 将 `_enumerate_simple_paths` 改为预算感知枚举。
3. 在 Step1/S2 semantic graph 中识别 `kind_2 = 128` 复杂 mainnode 组：
   - 组内物理 node 使用自身 `node.id`，不使用 `mainnodeid` 聚合。
   - 组内物理 node 使用 raw `kind / grade` 作为有效规则字段。
4. 在 `_evaluate_trunk_choices` 内识别复杂穿越 pair：
   - `pair.crosses_kind_2_128`
   - `len(pair.kind_2_128_node_ids)` 达到阈值
   - `len(pruned_road_ids)` 达到阈值
5. 对复杂组合先构造 `kind2_128_local_corridor`：
   - 使用 Step1 已确认的 forward / reverse support path 作为进入 / 退出 port corridor。
   - 小型 case 保留既有精确判定回退。
   - 命中终止阈值的大型复杂组合不再回退到全局 simple-path 枚举。
6. 剩余复杂穿越 pair 使用受限 path budget。
7. budget exhausted 时直接返回：
   - `reject_reason = trunk_search_budget_exceeded`
   - support info 包含预算配置、消耗计数、pair 复杂度与阶段。
8. 在 Step2 summary 增加预算超限计数。
9. 增加单元测试与 XS 性能复测。

## Risk Control

- 不改变普通 pair 的 path 枚举默认行为。
- 仅在复杂 `kind_2 = 128` mainnode 组上启用物理 node 级 Step1/S2 规则恢复。
- 仅在复杂 `kind_2 = 128` pair 上启用局部 corridor 或预算限制。
- 局部 corridor 不扩展 `through_node_ids`。
- 预算超限作为 rejected candidate 输出，不生成 segment body。
- 结果可通过 `pair_validation_table.csv`、`rejected_pair_candidates.csv`、`segment_summary.json` 审计。
