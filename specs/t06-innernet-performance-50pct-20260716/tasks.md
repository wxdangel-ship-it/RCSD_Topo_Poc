...
- [ ] T027 内网从 T06 Step1/2 开始执行，记录精确阶段 wall/CPU/RSS。
- [ ] T028 全量稳定 CSV/GPKG、CRS、schema、geometry、topology/audit 对比通过。
- [ ] T029 Step3 `<=16103.973s`，T06 两个独立 group 的 wall 求和 `<=21464.149s`。
- [ ] T030 peak RSS `<=9365992 KB`、swap=0、无 OOM/Killed。
...
- T025：最终 T06 全量测试 `485 passed in 46.05s`，0 failure、0 error、0 skip；其中新增内网验收门禁定向测试 `6 passed`。六例最终文件集一致，全部 CSV/GPKG 不变；`1885118` 真实成果再次完成 `91/91` 文件校验，`73/73` 个稳定业务成果无差异，两个允许变化的 compact summary JSON 已由两侧一致的权威 topology audit CSV 重算一致，54 个非拓扑兼容业务键与冻结基线相等。T06 核心 `src/tests/scripts` 共 `162` 个文件，加 `3` 个 SpecKit 一次性验收脚本后总计 `165` 个，`>=61440 bytes` 为 `0`。
- T027..T031：等待优化代码进入内网后执行同数据全量复验。旧运行 Step1/2、Step3 外层和 T06 总计由日志边界分别推算为 `5432.081s`、`37496.218s`、`42928.299s`；候选必须用 helper 产生的独立 `/usr/bin/time` 精确值验收。
