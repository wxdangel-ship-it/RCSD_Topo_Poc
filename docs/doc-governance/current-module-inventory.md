...
- `Active POC / 成果模块`：`p01_arm_build`、`p02_wuhan_local_experiment`、`p05_neural_road_generation`
...
| `p05_neural_road_generation` | Active POC / 成果模块 | 方案A神经网络F-RCSD carrier决策实验模块。冻结T01 Junction—Segment/PhysicalMovement骨架，只研究carrier评分、异常线索和fallback；Dataset-P1已GO，P2-P3-P2 scorer为NO-GO，P2-P3-P3已通过40个ADVANCE_RIGHT invalid-access硬安全资格并判定下一表征必需。整图49+2与context fallback通过，未接正式规则链或生产；旧M1/M2R/R2/PTO/JSG-PTO为历史实验。 |
...
- POC 验证：P01 承载异构路口通行能力 POC，不替代 T09；P02 承载武汉局部人工锚定实验，不替代 T08/T01/T05/T06；P05 承载冻结业务骨架下的神经网络 carrier 决策实验，不替代 T01-T06。
