...
- `pair_anchor_formal_retry.py`：候选 pair 的 as-is / reversed 正式试算与唯一 outcome 判定；支持单候选 pair anchor mismatch 与单向 `multi_anchor_ambiguous` 的受限消歧。
...
- `adaptive_buffer_retry.py`：高等级 single / dual Segment 的受限重审准入；只判断原始 pair relation 不变时是否允许进入 single graph-first 或 dual adaptive buffer 重审。
- `single_graph_connectivity_retry.py`：高等级单向 Segment 的 RCSD graph-first 纵向联通 helper；只在全图有向 path 经过 50m buffer core 且长度/首尾/几何参考门槛通过时产出 RCSDSegment。
...
- Step3 以 Step2 replaceable 为唯一替换白名单，不重新做 RCSD Segment 搜索或特殊组放行判定；先按 Segment 聚合删除 / 引入集合，若 replaceable final `junc_nodes` 相对 T01 原始 `junc_nodes` 存在 detached junc，则保留其触达的原 SWSDRoad 作为 `source=2` 局部 carrier，再消费 Step2 passed 特殊路口组审计中的内部 RCSDRoad / RCSDNode 作为组级补充，最后按语义路口 C 聚合重建 mainnodeid 与继承属性，避免逐 Segment 覆盖同一 C 造成不一致。
