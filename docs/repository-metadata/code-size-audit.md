...
| `src/rcsd_topo_poc/modules/t06_segment_fusion_precheck/step3_segment_replacement.py` | `99803` bytes | Step3 主流程已重新逼近 100 KB；本轮继续把 retained carrier mainnode 同步与拓扑审计判定放在 helper，主文件只保留编排调用 | 后续 Step3 新增替换策略、拓扑审计或 carrier 处理禁止回填主文件，必须继续下沉到专用 helper |
