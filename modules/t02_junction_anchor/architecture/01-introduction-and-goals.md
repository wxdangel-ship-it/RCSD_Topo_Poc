...
- 当前角色：
  ...
  - 面向“有资料但未锚定”路口的 stage3 `virtual intersection anchoring`
  - 面向单 `mainnodeid` 复核与外部复现的文本证据包支撑入口
- 上游关系：
...
1. 为双向 Segment 相关路口锚定提供稳定的 stage1 gate 与后续 stage2 / stage3 落点
2. 将“是否有有效资料”“是否已命中稳定锚点”“未锚定时如何构造虚拟路口面”拆成清晰、可治理的三阶段
3. 为 stage3 提供可解释的局部虚拟路口面、RC 关联与批次汇总能力，并为单 case 复核保留文本证据包支撑
...
- `specs/t02-junction-anchor/*` 与 `specs/t02-virtual-intersection-batch-poc/*` 继续保留为变更工件，不替代长期模块真相。
