...
- 当前只允许用户明确授权的两个正式入口：2026-07-14 授权的核心入口 `scripts/p02_run_wuhan_internal_case.py`，以及 2026-07-15 授权、只做 WSL 环境检查与转调的内网包装入口 `scripts/p02_run_wuhan_innernet_case.sh`；不得借此新增 repo CLI、Makefile 目标、模块 `run.py`、模块 `__main__.py` 或其它 P02 长期入口。
