...
def test_arbitrate_pair_options_drops_selected_segment_body_overlap_only_after_selection() -> None:
    left = step2_arbitration.PairArbitrationOption(
        option_id="LEFT::opt_01",
        pair_id="LEFT",
        a_node_id="A",
        b_node_id="B",
        trunk_mode="counterclockwise_loop",
        counterclockwise_ok=True,
        warning_codes=(),
        candidate_channel_road_ids=("left_trunk", "shared_segment"),
        pruned_road_ids=("left_trunk", "shared_segment"),
        trunk_road_ids=("left_trunk",),
        segment_candidate_road_ids=("left_trunk", "shared_segment"),
        segment_road_ids=("shared_segment",),
        branch_cut_road_ids=(),
        boundary_terminate_node_ids=(),
        transition_same_dir_blocked=False,
        support_info={
            "pair_support_road_ids": ["left_trunk"],
            "endpoint_priority_grades": [2, 1],
        },
    )
    right = step2_arbitration.PairArbitrationOption(
        option_id="RIGHT::opt_01",
        pair_id="RIGHT",
        a_node_id="C",
        b_node_id="D",
        trunk_mode="counterclockwise_loop",
        counterclockwise_ok=True,
        warning_codes=(),
        candidate_channel_road_ids=("right_trunk", "shared_segment"),
        pruned_road_ids=("right_trunk", "shared_segment"),
        trunk_road_ids=("right_trunk",),
        segment_candidate_road_ids=("right_trunk", "shared_segment"),
        segment_road_ids=("shared_segment",),
        branch_cut_road_ids=(),
        boundary_terminate_node_ids=(),
        transition_same_dir_blocked=False,
        support_info={
            "pair_support_road_ids": ["right_trunk"],
            "endpoint_priority_grades": [1, 1],
        },
    )
    ...
    outcome = step2_arbitration.arbitrate_pair_options(
        ...
        tjunction_anchor_node_ids=set(),
    )

    assert outcome.selected_options_by_pair_id == {"LEFT": left}
    decision_by_pair_id = {decision.pair_id: decision for decision in outcome.decisions}
    assert decision_by_pair_id["LEFT"].arbitration_status == "win"
    assert decision_by_pair_id["RIGHT"].arbitration_status == "lose"
    assert decision_by_pair_id["RIGHT"].lose_reason == "selected_segment_body_overlap"
    assert any(
        record.pair_id == "RIGHT"
        and record.conflict_pair_id == "LEFT"
        and record.conflict_types == ("selected_segment_body_overlap",)
        and record.shared_road_ids == ("shared_segment",)
        for record in outcome.conflict_records
    )
