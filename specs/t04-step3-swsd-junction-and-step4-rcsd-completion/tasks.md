...
- [x] 2026-05-04 cleanup：明确 Anchor_2 official 39-case 是唯一正式基线，23/30 历史子集只从 official manifest 投影，不再作为独立 batch / PNG fingerprint gate。
...
- [x] 在 `_runtime_step3_topology_skeleton.py` 实现：
  ...
  - [x] `_walk_arm_to_neighbor_semantic_junction(seed_branch, local_roads, member_node_ids, semantic_mainnodeids) -> tuple[tuple[str,...], str, str, str | None, bool]` —— 返回 `(connector_road_ids, terminal_node_id, terminal_kind, neighbor_junction_id, continuation_through_micro_junction)`。
    - 默认沿语义节点组 degree==2 passthrough chain；
    - 2026-05-04 修订：遇语义节点组 degree>=3 且不属于当前 member node 时立即作为 semantic boundary 停止，不允许角度连续穿透；
    - 终止条件：语义节点组 `degree>=3 且不属于 member_node_ids` / 语义节点组 `degree==1` / 退出 patch。
    - 2026-05-04 追加修订：语义节点组按 `mainnodeid` 聚合，无有效 `mainnodeid` 时按节点自身 `id` 成组；有 `mainnodeid` 与无 `mainnodeid` 必须使用同一进入 / 退出道路度数口径。
...
- [x] 单元测试 `tests/modules/t04_divmerge_virtual_polygon/test_step3_swsd_semantic_junction.py`：
  ...
  - [x] 语义节点组 degree>=3 semantic boundary 不穿透 → `terminal_kind = semantic_neighbor` 且越界 road 不进入当前 connector。
...
- [x] 2026-05-04 修订守门：RCSD 语义路口 connector 与 SWSD 对称，只允许语义节点组 `degree == 2` passthrough；遇语义节点组 `degree >= 3` 立即作为 `semantic_neighbor` 停止。
...
### 6.1 Official baseline 与 legacy projection gate（必须通过）

- [x] 跑 official 39-case baseline gate：`pytest tests/modules/t04_divmerge_virtual_polygon/test_step7_final_publish.py::test_anchor2_39case_official_surface_scenario_gate -x`。
  - [x] `accepted = 35 / rejected = 4` 不漂移；`607602562 / 760598 / 760936 / 857993 = fail4`，`699870 = yes`。
- [x] 跑 legacy 23-case projection gate：`pytest tests/modules/t04_divmerge_virtual_polygon/test_step7_final_publish.py::test_anchor2_full_20260426_baseline_gate -x`。
  - [x] 只校验它是 official 39-case manifest 子集，并投影得到 `accepted = 20 / rejected = 3`。
- [x] 跑 legacy 30-case projection gate：`pytest tests/modules/t04_divmerge_virtual_polygon/test_step7_final_publish.py::test_anchor2_30case_surface_scenario_baseline_gate -x`。
  - [x] 只校验它是 official 39-case manifest 子集，并投影得到 `accepted = 26 / rejected = 4`。
...
- [x] 全 39-case 自动核查（必须全绿）：
  ...
  - [x] 全 39-case 总耗时低于 `summary.json.performance.threshold_seconds_total = 240.0`；历史 30-case 不再作为性能对比基线。
...
- [x] 把 run_root 路径登记到 `architecture/10-quality-requirements.md` 的 *Anchor_2 official 39-case baseline gate* 段落作为新参考。
- [x] **不**新增 PNG fingerprint hash 断言；本轮新基线作为人工审计参考存在，不进 pytest。
...
- official 39-case gate 与 legacy 23/30 projection gate 的 pass/fail。
- 39-case 总耗时与性能阈值对比。
...
- Phase 6：跑 `test_anchor2_39case_official_surface_scenario_gate` + `test_anchor2_full_20260426_baseline_gate` + `test_anchor2_30case_surface_scenario_baseline_gate`；后两者是 legacy projection gate，不再独立重跑 23/30 batch。
...
- official 39-case gate 与 legacy 23/30 projection gate 的最终 pass 证据。
