...
def test_case_584141_keeps_final_allowed_space_inside_drivezone(tmp_path: Path) -> None:
    context, _template_result, case_result = load_real_case_bundle("584141")
    run_root = tmp_path / "run"
    write_case_outputs(run_root=run_root, context=context, case_result=case_result)

    status_doc = json.loads((run_root / "cases" / "584141" / "step3_status.json").read_text(encoding="utf-8"))
