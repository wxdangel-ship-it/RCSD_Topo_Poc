...
T08 输入是原始 SWSD / RCSD / patch / restriction / Laneinfo / Traj 数据。T08 输出被 T01、T03、T04、T05、T06、T09 使用；其中 Tool7 restriction 与 Tool8 arrow 是 T09 的重要证据，Tool9 RCSD 清理结果会影响 T05 relation 与 T06 替换质量，Tool10 为 Patch 级轨迹消费提供统一 `LineStringZ` 输入，Tool11 为全量流程和可选实验流程提供统一 Patch 目录包。
...
| Tool11 | Patch 数据整理 | 将原始 SWSD/RCSD 全树和 FRCSD 三文件白名单整理为统一 Patch 包，并可选发布实验子集。 |
...
6. Tool11 独立扫描全量 Patch，把源目录映射为统一 `SWSD / RCSD / FRCSD`，完成哈希验证后发布全量根；只有显式配置实验根时才同步发布实验子集。
