...
- Tool2 只做属性关联、空间匹配和 `0a + 17` 事件 Road 删除，不改写 Road 几何拓扑。
...
Tool2 必须记录 `event_road_0a_17_count`，事件 Road 输出必须可追溯被删除 Road 的 `id / patch_id / kind` 与几何。
...
Tool6 必须记录 `错误分歧合流路口 / 错误交叉路口` 计数、CSV `是否修复` 默认值、连续分歧合流 suppressed 候选、字段解析与参数阈值。
