...
def test_real_case_505078921_splits_complex_three_merge_and_keeps_internal_cuts_out(
    ...
    assert step5_doc["case_terminal_cut_constraints"]["length_m"] == pytest.approx(
        65.46328053552214,
        abs=1e-6,
    )
    assert max(
        unit_result["unit_terminal_cut_constraints"]["length_m"]
        for unit_result in step5_doc["unit_results"]
    ) < 50.0
