...
| `t08_preprocess` | `modules/t08_preprocess` | T08 正式预处理模块；当前覆盖 Tool1 基础矢量格式转换、Tool2 Road GPKG 预处理、Tool3 Nodes 类型聚合与 Tool4 路口类型错误识别，Tool1 支持 SHP / GeoJSON 转 GPKG 与 GPKG 转 GeoJSON，输出均写回输入目录下同名文件，补充 `patch_id / kind / kind_2 / grade_2`，Tool4 输出 `nodes_error.gpkg` 记录错误类型，均输出 `EPSG:3857` GPKG | `tool1-tool2-tool3-tool4 active / innernet scripts registered / tool4 detects errors only` |
...
- `t08_preprocess` 当前作为 T08 正式预处理模块启动；工具形态属于项目正式组成部分，当前内网执行通过 `scripts/t08_tool1_vector_convert.py`、`scripts/t08_tool2_road_preprocess.py`、`scripts/t08_tool3_nodes_type_aggregation.py` 与 `scripts/t08_tool4_junction_type_repair.py`，Tool4 只识别错误并输出 `nodes_error.gpkg`，不自动修复。
