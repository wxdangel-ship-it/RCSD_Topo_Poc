...
- Step2 中 `kind_2 = 64 / 128` 直接写 `is_anchor = NULL / anchor_reason = NULL`，不纳入冲突规则。
- Step2 中 `kind_2 = 2048` 仅当组内所有 node 均命中同一个且唯一的 `RCSDIntersection` 时写 `is_anchor = yes / anchor_reason = t`，否则写 `NULL / NULL`，不纳入冲突规则。
