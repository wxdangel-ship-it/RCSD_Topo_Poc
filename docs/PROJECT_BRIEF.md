...
- T06 Step2 以 SWSD Segment 50m buffer 生成 RCSDSegment 审查输出；RCSDRoad 使用 `intersects + 阈值` 筛选，构图前以 `formway` bit7/128 排除提前右转 road，`junc_kind2_exempt_nodes` 只作为 optional allowed 审计节点；Step2 不再执行 pair-to-pair BFS 路径搜索、方向趋势、主轴 / 长度趋势或唯一性筛选
