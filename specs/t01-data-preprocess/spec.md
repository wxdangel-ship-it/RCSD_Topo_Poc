...
## 1. 文档状态
- 状态：`accepted baseline / Skill v1.0.0 formalization`
- 当前阶段：`Skill v1.0 formalization and baseline freeze`
- 本文用途：
  - 固化当前 T01 accepted baseline 的正式业务语义
  - 说明官方 Step1-Step5 口径与内部实现映射
  - 说明 XXXS freeze baseline 约束
...
### 2.2 当前实现映射
- Step1 对应：`t01-step1-pair-poc`
- Step2 对应：`t01-step2-segment-poc`
- Step3 对应：`t01-s2-refresh-node-road`
- Step4 对应：`t01-step4-residual-graph`
- Step5 对应：`t01-step5-staged-residual-graph`
- 官方端到端入口对应：`t01-run-skill-v1`
...
- 合法 Segment 不要求物理上永远是两条完全分离的 road
- 当前 accepted 语义允许：
  - 先分后合
  - 合后再分
  - 分合混合
  - 共享双向 road 的混合通道
- 只要整体仍满足合法 `A -> B` 与 `B -> A` 通道，即可成立 trunk
...
- 历史边界同时作用于：
  - pair 搜索阶段
  - segment 收敛阶段
- 对当前轮而言，历史高等级边界 mainnode 具有：
  - `seed`
  - `terminate`
  - `hard-stop`
- 搜索命中历史边界时，应记为 terminal candidate，然后停止继续穿越
...
- 工作图：
  - Step4 refreshed road 中剔除已有 `segmentid` 的 road
- `seed / terminate` 并入：
  - `S2 + Step4` 历史边界端点
...
- 未经用户明确认可，不得更新该 freeze baseline
