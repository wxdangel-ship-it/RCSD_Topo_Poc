...
T10 面向 RCSD_Topo 端到端业务流程编排与 Case 级证据组织。项目级业务数据链仍保持 T06 F-RCSD 直接进入 T09；T10 v1 的审计编排范围为 `T01 -> T07 Step1/2 -> T03 -> T04 -> T05 -> T06 -> T11 -> T09`，其中 T11 是 audit-only 必经阶段，不向 T09 回写业务输入。T08 作为独立前置预处理、质检与修复模块，不由 T10 v1 callable 或 Case runner 调用；内网全量总控脚本可把 T08 作为独立前置阶段纳入全量执行链路。T07 Step3 是可选兼容 relation 补锚阶段，不属于 T10 v1 默认编排。
...
- 支持从 T10 Case package 启动 Case 级端到端执行：`T01 -> T07 Step1/2 -> T03 -> T04 -> T05 -> T06 -> T11 -> T09`。
...
- 支持多个 SegmentID 一次打包，解包后按 `<SegmentID>/` 一级目录重组；Segment package 的正式 Segment 身份记录在 `scope.swsd_segment_id`。
- 支持文本 bundle 自动分片与解包。
...
- Segment package 的 `spatial_slice` 必须以既有 T10 run root 反查到的 T01 Segment 几何生成固定 `200m` buffer 空间范围；T10/T06 matched rows 只作为 evidence reference 进入 manifest，不扩张空间范围，不暴露人工半径参数。

### 1.2 当前非目标

- 不改变项目级主业务链。
- T10 v1 callable 与 Case runner 不调用 T08。
- 不修改 T01-T09 / T11 模块算法。
- 不新增 repo CLI、`Makefile` 目标、模块 `run.py` 或模块 `__main__.py`。
- root `scripts/t10_pack_innernet_cases.sh` 是当前正式内网 Case 证据包打包入口。
- root `scripts/t10_pack_innernet_segments.sh` 是当前正式内网 Segment 证据包打包入口。
...
### 2.3 Segment package request

Segment 证据包输入：

- `swsd_segment_id` / `swsd_segment_ids`：SWSD SegmentID，可一次输入多个；每个 Segment 输出一个独立 `<SegmentID>/` 一级用例目录。
- `t10_run_root`：既有 T10 E2E 或 full-pipeline run root，用于反查 T01 Segment、T06 problem registry、replacement plan、replaceable 和 Step3 relation 证据。
- `t01_segment_path`：可选。未提供时，T10 从 `t10_run_root` 的 visual check summary、full pipeline manifest 或 run root 内 `segment.gpkg` 自动发现。
- `include_files`：是否物化外部输入文件；`true` 时默认生成空间切片。
- `materialization_mode`：可选，允许值：
  - `spatial_slice`：正式默认；按 T01 Segment 几何的 `200m` buffer 生成局部 GPKG 切片，并审计被选中道路的端点节点依赖。
  - `manifest_only`：只写 manifest，不扫描或复制全量输入内容。

Segment package 的 CaseID 是 runner 兼容 ID，格式为 `segment_<SegmentID>`；不得把它解释为 SWSD semantic junction id。正式 Segment 身份必须读取 `scope.swsd_segment_id`。多 Segment package 的目录名固定为 `<SegmentID>/`，不是 `segment_<SegmentID>/`；如需对新布局单独执行某个 Segment，`scripts/t10_run_e2e_cases.sh --case-id` 传入 `<SegmentID>` 目录名。

### 2.4 Case suggest request

`suggest` 的输入分两类：

- `prepared_swsd_nodes`：必选，用于建立 SWSD 语义路口 inventory。
- `selector_evidence`：可选，用于从 T08/T05/T06/T11/T09 等审计或错误文件中筛出问题候选。
...
- `run_id`：可选。未指定时自动生成。
- `stop_after`：可选阶段名，用于受控截断执行。允许值为 `t01 / t07 / t03 / t04 / t05 / t06_step12 / t06_step3 / t11 / t09_step12 / t09_step3`。
- `continue_on_error`：是否在某个 Case 阶段失败后继续记录后续 Case 或后续阶段阻断状态。

Case runner 必须优先消费 Case package 中 `external_inputs/<slot>/<slot>_slice.gpkg`。当 package 仅为 manifest-only 时，才回退到 manifest 记录的 source path。Segment package 同样使用该 package layout，因此无需修改 T01-T09 / T11 算法；但当 `case_id=segment_*` 且 Segment geometry `200m` buffer 范围内 T07/T03/T04 合法无候选时，runner 必须把该阶段登记为 `segment_no_candidate_handoff=true` 的显式空 handoff，继续后续链路，不得回退到人工半径或复制全量无关候选。

Case runner 不改变 T01-T09 / T11 算法，只负责：
...
Segment package replay 的空 handoff 只适用于 Segment geometry `200m` buffer 范围内的合法无候选：T07 stdout 必须明确无可用 `RCSDIntersection` 或 `RCSDNode` 几何记录，或 T03/T04 stdout 必须明确为无 eligible candidate，且 CaseID 必须为 `segment_*`。该 handoff 必须复制上游 nodes、生成空 relation/surface/audit 文件、记录 `noop_reason=no_eligible_candidates_in_segment_geometry_buffer`，并不得用于普通 semantic-junction Case，也不得把失败阶段的部分输出提升为正式 handoff。
...
- `t04` 阶段必须输出 `t04_nodes`，并同步登记为 `final_swsd_nodes`；T05 / T06 / T11 / T09 必须消费该节点事实，不得回退到 T07 Step2 nodes。
...
### 3.3 Segment evidence package outputs
...
每个 Segment case manifest 必须记录：

- `package_type = t10_segment_evidence`。
- `scope.scope_type = swsd_segment`。
- `scope.case_id = segment_<segment_id>`。
- `scope.case_id_semantics = swsd_segment_package_case_id`。
- `scope.swsd_segment_id`。
- `scope.center / bounds / buffer_m / segment_bounds / segment_endpoint_node_ids`。
- `segment_evidence.t10_run_root`。
- `segment_evidence.t01_segment_path`。
- `segment_evidence.artifacts[]`：T06 replaceable、replacement plan、problem registry、Step3 relation 等证据文件路径、匹配行数和最多 20 条匹配行。
- `spatial_slice_summary.selection_mode = swsd_segment_geometry_buffer`，且 `spatial_slice_summary.buffer_m = 200.0`。

Segment package 只物化外部输入切片。T10/T06 中间产物只能作为 evidence reference 进入 manifest，不复制到 package payload，不提升为新的 handoff。

### 3.4 Case suggestion outputs
...
每个完整 Case 必须包含 `cases/<case_id>/t11/`，其下实际产物位于 `run_<timestamp>/`。Case manifest 的 `handoffs` 至少登记 `t11_run_root / t11_candidates_csv / t11_candidates_gpkg / t11_manual_template_csv / t11_summary_json`。
...
### 3.6 Innernet full pipeline outputs
...
`t10_innernet_full_pipeline_manifest.json` 保留兼容的 flat `inputs / outputs`，同时必须提供阶段级 `stage_order / stages`。每个 `stages.<stage_id>` 至少包含 `stage_id / module_id / status / stdout_log / inputs / outputs / params / execution_context`，用于统一表达 T08、T01、T07 Step1/2、T03、T04、T05、T06 Step1/2、T06 Step3、T11、T09 的显式 handoff；显式启用 T07 Step3 时，manifest 才登记 `t07_step3` 阶段。下游审计应优先消费 `stages`，仅在兼容旧产物时读取 flat `outputs`。

全量 runner 在创建 manifest 时必须先写入 `status=running / passed=false`；进程退出时必须写入 `status / passed / exit_code / finished_at_utc / duration_seconds`。T11 与 T09 阶段均完成，且 T11 candidates/summary、T06 F-RCSD Road/Node、T09 `frcsd_restriction.gpkg` 均存在时，`status=passed`；任一阶段命令失败或必要输出缺失时，退出 trap 必须把顶层状态写为 `failed`。`t10_innernet_full_pipeline_summary.json` 至少记录 `run_id / run_root / status / passed / exit_code / stage_statuses / missing_final_outputs / t06_frcsd_road / t06_frcsd_node / t11_candidates_csv / t11_summary_json / t09_frcsd_restriction / manifest`。
...
该模式只读取既有 `t10_innernet_full_pipeline_manifest.json`、T06 Step3 F-RCSD Road/Node、passed 的 T11 stage 与 T11 candidates/summary，以及 T09 `frcsd_restriction.gpkg`，不重跑模块算法，不修改中间阶段产物。legacy run 若缺 T11，必须先以 `RUN_STAGES=t11` 补跑并登记；否则 summary 写为 `status=failed` 并列出 `missing_final_outputs`。
...
```bash
RESUME_RUN_ROOT=<existing_run_root> RUN_STAGES=t11 bash scripts/t10_run_innernet_full_pipeline.sh
```

阶段续跑模式不创建新 T10 run root，不重跑未列入 `RUN_STAGES` 的前序阶段；它优先读取既有 manifest 中的 `inputs / outputs` 作为 handoff，缺失时才回退到固定目录结构。支持阶段名：`t08_preprocess / t01 / t07_step12 / t03 / t04 / t05 / t06_step12 / t06_step3 / t11 / t09`；`t07_step3` 只作为显式兼容补锚阶段支持。最终完整 T10 通过要求 T06 F-RCSD Road/Node、passed 的 T11 stage 与 candidates/summary、T09 `frcsd_restriction.gpkg` 同时存在。
...
- 最终 `t06_frcsd_road`、`t06_frcsd_node`、`t11_candidates_csv`、`t11_summary_json` 与 `t09_frcsd_restriction` 路径。
...
`scripts/t10_pack_innernet_cases.sh` 的 CaseID 含义固定为 SWSD semantic junction id。脚本读取 T10 v1 外部输入 slot，生成多 Case package，并导出可自动分片的文本 bundle。解包后目录结构按 `cases/<case_id>/` 恢复。

`scripts/t10_pack_innernet_segments.sh` 的输入含义固定为 SWSD SegmentID。脚本必须读取 `T10_RUN_ROOT` 作为证据来源，生成多 Segment package，并导出可自动分片的文本 bundle。解包后目录结构按 `<SegmentID>/` 一级目录恢复，可直接交给 `scripts/t10_run_e2e_cases.sh` 执行本地 replay。
...
`scripts/t10_pack_innernet_segments.sh` 额外支持：

- `SEGMENT_IDS`：未提供位置参数时使用，支持逗号分隔。
- `T10_RUN_ROOT`：必选，既有 T10 run root，用于 Segment 证据反查。
- `T01_SEGMENT`：可选，显式指定 T01 `segment.gpkg`；未提供时从 `T10_RUN_ROOT` 自动发现。
- 其它外部输入 slot、`INCLUDE_FILES / MATERIALIZATION_MODE / OUT_ROOT / BUNDLE_ROOT / MAX_TEXT_SIZE_BYTES / DECODE_AFTER_EXPORT / TESTDATA_ROOT` 与 Case 入口一致。

Segment 入口不得暴露 `RADIUS_M`；若调用环境中存在同名变量，Segment 入口必须忽略它。Segment 空间范围固定为 T01 Segment geometry 的 `200m` buffer。
...
- `RESUME_FROM_STAGE`：阶段续跑的起始阶段；支持从 `t11` 开始，未设置 `RUN_STAGES` 时按 `t11 -> t09` 继续。
...
该全量 runner 只消费并串联既有模块脚本或 callable，不新增 T01-T09 / T11 的算法接口。
...
- `swsd_segment_ids`：Segment package 的 SegmentID 列表。
- `t10_run_root`：Segment package 的证据来源 run root。
- `t01_segment_path`：Segment package 可选 T01 Segment 显式路径。
...
- `stop_after`：T10 E2E Case runner 截断阶段，包含 `t11`。
...
5. Case evidence package manifest 排除 T01-T09 / T11 模块间中间产物。
6. `suggest` 只能把 selector evidence 映射为候选 Case，不把 inventory-only 清单表述为问题。
7. 多 Case bundle 解包后按 `cases/<case_id>/` 恢复目录。
8. 多 Segment bundle 解包后按 `<SegmentID>/` 一级目录恢复，并在 manifest 中记录 `scope.swsd_segment_id`。
9. 新增正式入口必须登记到 `docs/repository-metadata/entrypoint-registry.md`。
10. GIS / 拓扑 QA 五项在 summary 或模块质量文档中有明确状态。
11. `spatial_slice` 模式不得复制全量外部输入；Case package 目录只能物化半径窗口内的局部 GPKG，Segment package 目录只能物化 Segment geometry `200m` buffer 选中的局部 GPKG。
12. `spatial_slice` 必须审计道路端点节点依赖完整性，不允许 silent fix。
13. Segment package 必须从 T10 run root 记录 T01 Segment source 和 T06 evidence references，并用 T01 Segment geometry `200m` buffer 选择外部输入；不得用 T06 evidence rows 扩张空间范围，也不得暴露人工半径参数。
14. Case runner 必须输出每阶段显式输入、输出、命令、状态与日志路径。
15. T06 已运行时必须输出 `t10_t06_funnel.json/csv/md`。
16. T05 之后不得默认强制执行 `t07_step3`；T06/T11/T09 默认消费 T04 输出的 `final_swsd_nodes` 事实，显式运行可选 T07 Step3 时也只能额外审计 Step3 `nodes.gpkg`，不得覆盖正式节点 handoff。
