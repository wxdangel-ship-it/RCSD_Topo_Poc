...
| `t04_divmerge_virtual_polygon` | `modules/t04_divmerge_virtual_polygon` | `Step1-4` doc-first formalization；消费包含 `divstripzone.gpkg` 的 case-package，输出 admission/local-context/topology/event-unit interpretation 与 Step4 review PNG、flat mirror、index、summary | `step1-step4 refactor active / step4 review outputs required / no public cli` |
...
- `t04_divmerge_virtual_polygon` 当前作为 T04 新模块进入 Active，正式范围只到 `Step1-4`，其输出是后续 Step5-7 的上游中间结果与 Step4 审计工件。
