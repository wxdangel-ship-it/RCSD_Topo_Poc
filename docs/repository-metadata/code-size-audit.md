...
| `src/rcsd_topo_poc/modules/t06_segment_fusion_precheck/step3_group_replacement.py` | `9876` bytes | 新拆出的 Step3 path-corridor group replacement 消费 helper，负责读取 Step2 group audit、合并重叠 group component，并生成 Segment assignment | 保持只承接 group audit 到 Step3 assignment 的解析与分组，不承接 Road/Node 删除和 F-RCSD 输出 |
| `src/rcsd_topo_poc/modules/t06_segment_fusion_precheck/step3_segment_replacement.py` | `61757` bytes | Step3 主流程，消费 Step2 replaceable、passed special junction group 与 path-corridor group assignment，负责 SWSD/RCSD copy-on-write 输出与 junction C 重建 | 后续 Step3 新增替换策略优先下沉到专用 helper，不在主文件内扩展搜索/审计逻辑 |
