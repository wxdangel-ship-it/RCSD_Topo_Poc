...
T07 从 T02 中抽取语义路口级需求，只保留 `nodes` 语义路口集合，不再让 Segment 引用关系参与候选扩展、结果写值或统计。Step3 进一步消费 Step2 surface、T05 `intersection_match_all.geojson` 与输入 `RCSDNode`，用于建立 T07 的 SWSD-RCSD 语义路口关系。
...
- T07 最终 SWSD-RCSD 语义路口关系 `intersection_match_t07.geojson`。
