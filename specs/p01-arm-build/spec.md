...
提前右转 road 分两类处理：路口前提前右转不进入当前语义路口，不进入 Arm `member_road_ids / seed_road_ids / connector_road_ids / trunk_road_ids`，必须进入 `AdvanceRightTurnRelation` 或 issue；路口内提前右转按 `direction` 进入当前语义路口，进入 Arm `member_road_ids / seed_road_ids`，不进入 `trunk_road_ids`，不进入 `AdvanceRightTurnRelation`。提前左转 road 可以进入 Arm member，但必须从 trunk 中排除。

`--right-turn-formway-value` 只用于 legacy 显式右转 / 渠化右转排除兼容；不定义 bit7 提前右转。bit7 不得被静默写入 `excluded_right_turn_road_ids`。
...
LocalArmCandidate 使用方向性 seed corridor 分组：先按 seed role 与局部趋势形成同向 lane bundle，再按 inbound/outbound 同向配对和 outbound 顺时针连接下一 inbound 的道路设计关系形成 Arm 候选，并按 source InitialArm 合并以保证一个 InitialArm 只归属一个 FinalArm。该规则用于避免纯角度聚类把相邻真实 Arm 过度合并，或把同一远端道路走廊的进入 / 退出方向过度切碎。

FinalArm 形成后输出 `ArmCorridorEvidence`，沿 seed 外侧同向、非 bit7 的拓扑 continuation 记录远端走廊支持道路、节点、角度和终端。该证据不改变 `member_road_ids`，只供 A2 配准、Movement 方向判断和目视审计使用，避免仅用路口前短 Road 判断跨源绑定或转向类型。
...
主干道路 / 平行支路只覆盖部分目标退出 Road 时，默认进入 `data_error_partial_target_coverage`；如果主干 evidence 完整覆盖目标 trunk road，只是没有覆盖其它退出支路，则输出 `trunk_only_allowed` 并只投影到目标 trunk。advance-left left-receiving only、advance-left trunk only 与 uturn trunk only 是合法特殊范围。
...
- primary source 没有可生成规则时，可以使用 alternate source 的 Arm role / corridor ordinal 证据进行低置信投影，生成规则标记为 `alternate_source_role_ordinal_projection`，并在 audit / issue 中保留 primary source rule 不可用与 corridor ordinal 证据。

生成范围：

- `full_allowed`：进入道路角色下全部 F-RCSD from road 生成到目标 Arm 全部退出 Road。
- `trunk_only_allowed`：仅生成到目标 Arm trunk road，用于 trunk evidence 完整覆盖目标 trunk 或 uturn 等明确特例。
