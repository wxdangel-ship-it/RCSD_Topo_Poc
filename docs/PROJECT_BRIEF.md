...
- `t01_data_preprocess` 双向 Segment 构建允许穿越 `kind_2 = 128` 复杂分歧 / 合流路口组合，但必须输出 `kind_2_128_*` 审计字段；该审计不改变 `seed / terminate / hard-stop` 规则，也不扩展 `through_node_ids` 语义
...
- `t08_preprocess` 当前作为 Active 正式预处理模块启动；当前覆盖 Tool1 基础矢量格式转换、Tool2 Road GPKG 预处理、Tool3 Nodes 类型聚合与 Tool4 路口类型错误识别，Tool1 支持 SHP / GeoJSON 转 GPKG 与 GPKG 转 GeoJSON，输出均写回输入目录下同名文件，Tool2 补充 `patch_id / kind`，Tool3 补充 `kind_2 / grade_2` 并处理环岛、复杂分歧 / 合流 mainnode，Tool4 输出 `nodes_error.gpkg` 记录错误类型，输出 `EPSG:3857` GPKG；Tool4 不自动修复
