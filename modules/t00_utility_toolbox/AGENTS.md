...
- 模块角色：项目内工具集合模块
- 当前承接 Tool1 至 Tool5 的固定脚本和共享底层能力
...
- 新工具进入 `T00` 前，先补规格与契约
- Tool2 / Tool3 / Tool4 / Tool5 的修改必须以 `spec.md` 和 `INTERFACE_CONTRACT.md` 为准
- `README.md` 只承担入口说明，不替代长期源事实

## 6. 禁止事项

- 不得把 `T00` 演化成业务生产模块
- 不得未经确认擅自扩展 Tool1 至 Tool5 的范围
- 不得绕过 `spec` 直接编码扩写
...
- Tool2 / Tool3 的几何处理统一在 `EPSG:3857`
...
- Tool4：固定脚本 + 属性关联 + unmatched / conflict 输出
