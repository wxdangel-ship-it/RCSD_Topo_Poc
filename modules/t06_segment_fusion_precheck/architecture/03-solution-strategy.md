...
- Step2 replacement plan 必须前置处理准确 T05 relation 下的 retained-junction 20m gate：触发节点的 T05 `status=0 / base_id>0` 与 plan RCSD node canonical 后一致时，20m 只追加 `junction_alignment_to_retained_swsd_exceeds_topology_gate / junction_alignment_t05_relation_release / manual_review_required` 风险，不得 hard block。Step3 wrapper 的 surface-aware gate release 只保留为旧 plan 或 surface-only 兜底；候选释放仍必须通过 topology audit，新增 hard fail 必须回退并在 summary 暴露。
...
- `t06_step3_surface_aware_plan_release_audit.json`，用于解释旧 plan / surface-only retained-junction gate 条件释放、topology 回退和外部 baseline 新增 fail 对照；准确 T05 relation 释放应优先体现在 Step2 `t06_segment_replacement_plan.*` 风险标记中。
