...
- 支持多个 SegmentID 一次打包，解包后按 `cases/segment_<segment_id>/` 重组；Segment package 的正式 Segment 身份记录在 `scope.swsd_segment_id`。
- 支持文本 bundle 自动分片与解包。
...
- Segment package 的 `spatial_slice` 必须以既有 T10 run root 反查到的 T01 Segment 几何和 T06 evidence dependency ids 形成证据闭包；不得要求或暴露人工半径参数。
...
- `swsd_segment_id` / `swsd_segment_ids`：SWSD SegmentID，可一次输入多个；每个 Segment 输出一个独立 `cases/segment_<segment_id>/` 用例目录。
...
- `materialization_mode`：可选，允许值：
  - `spatial_slice`：正式默认；按 T01 Segment 几何、Segment 端点和 T06 evidence rows 中的 SWSD/RCSD 节点/道路依赖生成局部 GPKG 切片。
  - `manifest_only`：只写 manifest，不扫描或复制全量输入内容。

Segment package 的 CaseID 是 runner 兼容 ID，格式为 `segment_<SegmentID>`；不得把它解释为 SWSD semantic junction id。正式 Segment 身份必须读取 `scope.swsd_segment_id`。
...
- `scope.center / bounds / segment_bounds / segment_endpoint_node_ids`。
...
- `spatial_slice_summary.dependency_context`：从 T01 Segment 与 matched evidence rows 解析出的 SWSD/RCSD 节点和道路依赖。
...
Segment 入口不得暴露 `RADIUS_M`；若调用环境中存在同名变量，Segment 入口必须忽略它。
...
8. 多 Segment bundle 解包后按 `cases/segment_<segment_id>/` 恢复目录，并在 manifest 中记录 `scope.swsd_segment_id`。
9. 新增正式入口必须登记到 `docs/repository-metadata/entrypoint-registry.md`。
10. GIS / 拓扑 QA 五项在 summary 或模块质量文档中有明确状态。
11. `spatial_slice` 模式不得复制全量外部输入；Case package 目录只能物化半径窗口内的局部 GPKG，Segment package 目录只能物化 evidence dependency closure 选中的局部 GPKG。
12. `spatial_slice` 必须审计道路端点节点依赖完整性，不允许 silent fix。
13. Segment package 必须从 T10 run root 记录 T01 Segment source 和 T06 evidence references，并用 evidence dependency closure 选择外部输入；不得只按 SegmentID 或人工半径猜测外部输入范围。
