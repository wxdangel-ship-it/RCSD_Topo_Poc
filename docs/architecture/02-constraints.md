...
- 输入字段治理约束：
  ...
  - `Road.kind` 已正式启用；当前确认其字段语义为道路种别，单个 token 使用 `XXXX` 形式表达，前两位为道路等级、后两位为道路类型，多个道路种别用 `|` 分隔。当前在 T08 Tool4 中启用后两位道路类型：任一 token 后两位为 `0a`（大小写不敏感）时，该 Road 视为辅路；在 T01 双向 Segment 构建中启用前两位道路等级，用于分歧 / 合流 / 低等级交叉路口的局部续行优先策略，不通过几何或局部样本反推缺失 `Road.kind`。
