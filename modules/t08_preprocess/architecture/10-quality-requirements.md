...
- Tool4 只识别路口类型错误，不修复 `kind_2`，不输出或改写 Roads。
...
Tool2 / Tool3 / Tool4 命令脚本必须输出阶段进度，并在 summary 记录耗时与吞吐；GPKG 输出必须复用共享直接 SQLite GeoPackage 写出路径。
Tool3 复杂分歧 / 合流聚合必须使用 Road 拓扑链路搜索，并记录候选数、链路数、聚合组与更新节点数；复杂链 component 组装不得按组件反复全量扫描链路边。
Tool4 命令脚本必须输出阶段进度，summary 必须记录语义路口数、错误数、错误类型分布、方向异常数、阶段耗时、吞吐与 Road 读取模式；Road GPKG 优先走只读取必要字段的 SQLite 轻量读取，拓扑阶段不得长期持有完整 Road 几何对象。
