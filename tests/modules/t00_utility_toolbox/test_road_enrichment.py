...
def test_tool4_joins_patch_id_and_splits_unmatched(tmp_path: Path) -> None:
    ...
    assert [feature["properties"]["patch_id"] for feature in output_doc["features"]] == ["1001", "1002"]
    assert {
        feature["properties"]["unmatched_reason"] for feature in unmatched_doc["features"]
    } == {"conflicting patch_id candidates", "no rc_patch_road match"}
...
def test_tool5_enriches_kind_with_buffer_contains_logic(tmp_path: Path) -> None:
    ...
    _write_feature_collection(
        ...
        features=[
            ...
            },
            {
                ...
                "geometry": {"type": "LineString", "coordinates": [[0.0, 10.0], [10.0, 10.0]]},
            },
            {
                "type": "Feature",
                "properties": {"id": "3", "patch_id": "1003"},
                "geometry": {"type": "LineString", "coordinates": [[0.0, 20.0], [10.0, 20.0]]},
            },
        ],
    )
    _write_feature_collection(
        ...
        features=[
            ...
            },
            {
                ...
                "geometry": {"type": "LineString", "coordinates": [[0.5, 0.0], [9.5, 0.0]]},
            },
            {
                "type": "Feature",
                "properties": {"Kind": ""},
    ...
    summary = run_road_kind_enrich(
        RoadKindEnrichConfig(
            ...
            output_path=output_path,
            target_epsg=3857,
            default_input_crs_text="EPSG:3857",
    ...
    assert output_features[0]["properties"]["kind"] == "1201|1202|1301"
