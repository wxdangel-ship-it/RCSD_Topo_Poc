...
- T12 Junction 层必须把 T03 rejected 限制为正式 eligible 候选并以原始 1V1 F-RCSD 重验，只有 terminal-collapse 或 unmatched-support 完整强门禁通过才可确认；`6m/50m` 距离只作 endpoint 投影、局部 support 检索与审计。同 endpoint 的其它 Segment Road 不得自动并入当前 Junction support。T07 只直接发布正式 1:N/N:1 稳定失败，duplicate 只审计。Segment LineString 与 Junction Point 的结果、计数和根因 evidence 独立，任何输入不得 repair、snap 或 silent fix。
