...
> 当前项目级生命周期中，T02 已 Retired。本文档保留一次历史状态研究，用于说明旧实现和旧文档面如何形成；其中“当前”均按当时研究时点理解。
...
- 当前阶段：`retired`
- 说明：
  - repo 级生命周期当前已将 T02 登记为 Retired。
  - T02 历史上存在 stage1 `DriveZone / has_evd gate`、stage2 `anchor recognition / anchor existence` 与 stage3 `virtual intersection anchoring` baseline 实现。
  - 单 `mainnodeid` 文本证据包作为历史 stage3 复核与外部复现支撑工具保留。
- 研究目标：
  - 解释 T02 模块级 architecture 文档、历史实现、契约和 README 的形成背景
  - 把 T02 从早期“变更规格主导”的记录收口到“历史模块文档面主导”
...
- stage1、stage2、stage3 都曾具备可运行入口、正式输出或 baseline 输出、审计留痕与最小测试。
- 当时主要脱节不再是“是否有实现”，而是部分文档仍停留在“虚拟路口属于受控实验层”的旧口径。
...
- stage3 baseline 之后，是否继续推进到正式产线级全量批处理能力
