...
6. P0X POC 验证模块与 T0X 模块使用同一目录结构和治理链路，仅通过模块 ID 区分验证模块与正式业务模块
7. P01-Final 采用 SWSD / RCSD 源侧通行规则抽象与 F-RCSD 道路角色投影；精确源 Road 映射只进入审计证据，不作为最终 RoadNextRoad 生成前提
...
- 该兼容约束只表达当前工程兼容事实，不冻结 RCSD 长期数据标准
