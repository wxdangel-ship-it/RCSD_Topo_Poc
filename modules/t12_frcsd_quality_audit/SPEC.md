...
- 将 canonical 宽召回候选、raw endpoint 主判定与 portal-constrained semantic 误报排除严格分层，自动发布通过标准路口与锚点可信度门禁的高置信质量问题；人工复核仅作可选 QA 覆盖。
...
- T05 成功锚点、`grouped_rcsdnode_ids`、FRCSD raw Road endpoint、用于宽召回及受限误报排除的 `mainNodeId/subNodeId`，以及 RCSDIntersection 标准路口面。
...
- 候选、自动确认、自动排除、可选复核覆盖，以及 raw/canonical/portal-constrained semantic carrier 空间证据。
...
4. 比较 raw local/full directed/undirected carrier。raw local directed 失败后，检查 portal-constrained semantic local directed carrier：路径必须包含至少一条物理 Road 并满足方向、长度、增量和走廊偏离阈值；T07 非 raw 端点必须与 portal 同属唯一 RCSDIntersection 标准面，非 T07 非 raw 端点必须与 portal 同 canonical group 且间距不超过 portal radius；内部每个 alias transition 间距也不得超过 portal radius。只有全部门禁通过，semantic carrier 才能覆盖该方向的 raw failure 并自动 excluded。
5. 完成 semantic 误报排除后仍失败、且具有 T07 标准面信用或 T03/T03 正式锚点信用的 candidate 自动 confirmed；raw 或 portal-constrained semantic 等价 carrier、以及锚点信用不足自动 excluded。
6. 可选 review contract 可以覆盖自动决定，并完整保留原规则与外部来源。

## 6. 什么是对

- canonical 图中归并到同一 FRCSD 语义节点的零长度路径不能作为 carrier。semantic 排除证据必须包含物理 Road，且端点和每个 alias transition 均通过显式门禁；它只允许排除 raw 假断裂，不能单独确认质量问题。
