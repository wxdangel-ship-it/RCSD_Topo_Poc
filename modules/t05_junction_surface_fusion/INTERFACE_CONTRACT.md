...
- T11 manual relation audit CSV 是人工审计补充输入，只消费 `manual_relation_type in {1v1_rcsd_junction, 1vN_rcsd_junction, 1v1_rcsd_road, 1vN_rcsd_road}` 且 `selected_ids` 非空、非 `NULL` 的行。正向 T11 行可覆盖同 target 的 T07/T03/T04 机器 evidence；空白、`NULL`、`no_valid_relation`、`uncertain` 行只保留在 T11 审计表，不进入 T05。
- T11 relation-only target 可以没有 Phase 1 surface；T05 以 `T11_MANUAL:<target_id>` 构造临时 relation context，只用于本次 Phase 2 relation/junctionization 输出，不回写 Phase 1 surface。
...
`--t11-manual-relation` 是可选人工审计输入，用于实验性消费 T11 人工关系 CSV。该参数只读取正向人工关系类型，不消费 `selected_ids=NULL` 或空白行；人工审计 CSV 本身仍由 T11 保留，用于后续继续标注。
