...
- T12 不允许把 SWSD 与原始 1V1 F-RCSD 的等价假设直接用作修复；候选只有在 raw endpoint failure 无法被受信 portal-constrained semantic carrier 解释，并通过标准路口 portal、方向/几何和锚点可信度门禁后，才能自动进入正式问题层。canonical 零长度折叠、无物理 Road 或不受信端点不能排除问题，全部 raw/semantic 排除与可选 review override 证据必须保留。
