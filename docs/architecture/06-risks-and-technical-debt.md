...
| T12 候选被误当正式问题 | 自动可达性检查仍可能受 canonical 节点折叠、路口归组、局部 portal 或数据覆盖影响 | raw endpoint 图为主；canonical semantic carrier 仅在包含物理 Road、端点受标准面/正式 portal 约束且内部 alias 间距受限时排除假断裂；锚点可信度和方向/几何继续门禁，外部 review 只作可选 QA 覆盖 |
