...
Step2 replacement plan 负责前置处理准确 T05 relation 下的 retained-junction 20m gate：当触发节点在 `intersection_match_all.geojson` 中存在 `status=0 / base_id>0`，且 `base_id` 与当前 plan 的 RCSD node canonical 后一致时，`MAX_RETAINED_JUNCTION_ATTACHMENT_GAP_M=20m` 只能产生 `junction_alignment_to_retained_swsd_exceeds_topology_gate / junction_alignment_t05_relation_release / manual_review_required` 风险标记，不得把 ready plan 改为 blocked。该规则不适用于缺失 relation、relation status 非 0、base_id 无效、canonical 后不一致、pair anchor mismatch 被判为非权威、或 replacement plan 间映射到同一 SWSD junction 但 RCSD node 彼此分裂且无可解释 bridge 的场景。

当 Step3 调用方提供 surface 输入时，脚本仍可在不改变调用参数的前提下对旧 Step2 plan 中 `source_reason=junction_alignment_to_retained_swsd_exceeds_topology_gate` 的行执行 surface-aware 条件释放，作为兼容兜底而非当前 relation-backed 决策主路径。释放条件只接受两类证据：触发超距节点在 surface topology audit 中为 pass，且 reason 属于 `auto_closed / auto_closed_surface_1v1 / auto_closed_t04_patch_1v1 / auto_closed_step2_junc_1v1 / auto_closed_relation_mapped_boundary_1v1 / auto_closed_selected_replacement_endpoint`；或触发超距节点是 plan 的 `swsd_pair_nodes -> original_rcsd_pair_nodes` 原始端点映射。T04 reject、Patch 冲突、多候选、缺少可解释 endpoint、以及非该 gate 的 blocked plan 都不能被释放。释放后的 plan 必须重跑 Step3 与 topology audit；其它新增 hard fail 仍必须回退为 `plan_status=blocked / execution_action=hold`，并记录 `source_reason=junction_alignment_surface_release_failed_topology_gate`。若相对传入 `--t06-run-root` 下已有 Step3 baseline 仍存在不可降级新增 fail，必须写入 `external_final_added_fail_count / external_final_added_fail_keys`，不得把内部 release 回退通过误表述为外部 baseline 零新增。
