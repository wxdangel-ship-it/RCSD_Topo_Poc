...
| T12 质量审计证据 | 显式记录原始 1V1 F-RCSD、SWSD、RCSDIntersection、T05/T06 交叉证据、CRS、参数、运行环境、耗时，以及候选种类、selected base canonical group、显式 grouped raw node、实际 raw alias portal、Direction role、alias distance audit、非锚定 spatial fallback、canonical 候选、raw endpoint carrier、portal-constrained semantic carrier 的端点/内部 alias 门禁、F-RCSD 反向载体、双端锚点区间、逐 raw RCSD Road 当前/其它 Segment 归属、SWSD 反向替代路径、自动 decision、confirmed/excluded 和可选 Segment review override 分层输出；Junction 层另记录 T03/T07 run identity 与指纹、eligibility、target projection、局部 support ownership、support/global endpoint degree、component、替代 carrier、跨层/几何/Direction、T07 conflict group，并以独立 Point 结果和 Road/endpoint/projection/conflict evidence 发布。 |
...
| P05 Target A 证据 | 分离记录 immutable inference feature store 与 label store；路口 feature 只来自 T01/SWSD、原始 DriveZone/RCSDIntersection、raw RCSD 及 truth-free 相对几何/拓扑，T07/T03/T04/T05/T06 终态只进入 label/mask/weight/metric。先记录 T07 Step1/Step2、T03/T04 surface/relation evidence、T05 unique relation/graph-consumable/junctionization、完整锚定对象及零危险路口门，再允许记录 Segment/AR/RoadGraph 指标。每阶段保留 Case-group OOF、seed/config/checkpoint/hash、最差 fold/Case、CRS/拓扑/资源和零 silent fix。 |
