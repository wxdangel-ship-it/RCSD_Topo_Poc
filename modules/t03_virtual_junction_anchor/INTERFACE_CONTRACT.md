...
- 当前正式成果：
  ...
  - `t03_swsd_rcsd_relation_evidence.csv / t03_swsd_rcsd_relation_evidence.json`
...
- `nodes` 至少需具备：
  ...
  - `kind_2`
  - `grade_2`
  - `grade`（若输入提供则保留，后续 T05 `level` 可消费；缺失不阻塞 T03）
  - `closed_con`（若输入提供则保留，后续 T05 `is_highway` 可消费；缺失不阻塞 T03）
...
- 只允许更新当前批次 selected / effective case 对应代表 node 的 `is_anchor`：
  ...
  - 若最终 `intersection_match_t03` 校验发现同一个 SWSD 语义路口对应多个 RCSD 语义路口，则取消该 SWSD 的 T03 最终 relation，并将该代表 node 回退为 `is_anchor = no`
...
- 每个 selected case 至少输出一条 summary row，稳定字段包括：
  - `target_id`
  - `case_id`
  - `junction_type`
  - `template_class`
...
- `association_class = B` 表示 support-only，有 RCSD 但不构成 RCSD 语义路口；必须输出 `rcsd_present_not_junction / status_suggested = 1`。
- `visual_review_class / V1~V5 / business_outcome_class` 不得决定 relation evidence 的成功状态；该 evidence 必须以 formal `step7_state` 和 Step4 association 证据为准。
